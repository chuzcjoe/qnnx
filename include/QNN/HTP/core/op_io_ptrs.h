...
class OpIoPtrs {
 ...
 public:
  ...
  // NOTE: should be able to remove this one when prepare code is properly separated - used in
  // Op::clone
  OpIoPtrs(Graph &g, Op const *op_to_clone_in, OpId new_op_id_in,
           clonemode clone_mode_in = output_realloc);
  ...
  inline GraphStatus ophook(GraphStatus (OpHookBase::*mfp)(OpIoPtrs const &, Op &) const,
                            Op &target_op) const {
