...
ReplFunc wrap_as_replfunc(oExp::opexpr<V, T> &&opr) {
  return ReplFunc::create(
      [op{std::move(opr)}](Replacement &rpx, OpDef const &) -> OpRef { return op.eval(rpx); });
}
template <oExp::OpVnt V, typename T>
...
inline ReplFunc wrap_as_replfunc(ReplFunc &&rep) { return std::move(rep); }
inline ReplFunc wrap_as_replfunc(ReplFunc const &rep) { return rep; }
...
// EJP: FIXME: Instead of separate optim_config things that require several changes in several
// places, we need to plumb through a way to get an option out of graph.options that comes from
// options.def
...
// of the struct offset; i.e. the oExp can be built without an instance of optim_config_values
// existing.
//
...
API_EXPORT OpRef graph_gen_Const_int32_common_wrapper(GraphPrepare &graph_in, const OpDef &old,
                                                      const OutputDef &out_def, const uint8_t *data,
                                                      size_t data_len);
...
template <>
API_EXPORT OpRef graph_gen_Const_scalar_wrapper<DType::Int32>(GraphPrepare &graph_in,
                                                              const OpDef &old,
                                                              NN_INT32_T constval);
template <>
API_EXPORT OpRef graph_gen_Const_scalar_wrapper<DType::Float32>(GraphPrepare &graph_in,
                                                                const OpDef &old, float constval);
POP_VISIBILITY()
...
template <typename opdef_map_t, typename OpId_t>
inline bool exists(opdef_map_t const &m, const OpId_t &test) {
...
template <template <typename, typename, typename, typename> class C, typename K, typename V,
...
template <typename T>
inline auto map_to_size_t(T &&x) {
  return map_to_sizet_helper<T>::convert(std::forward<T>(x));
}

inline size_t eval_size(oExp::ECtx &, size_t size) { return (size_t)size; }
inline size_t eval_size(oExp::ECtx &e, ReplFuncInt const &f) { return (size_t)(f(e)); }

template <typename... Ts>
inline ReplFunc gen_Shape_inner(Ts... sizes) {
...
///
template <typename... Ts>
inline ReplFunc gen_Shape(Ts... sizes) {
...
template <typename... Ts>
API_EXPORT OpRef gen_Shape_immed(const OpDef &some_op, Ts... sizes);
...
template <DType DT>
inline OpRef gen_ConstScalar_imm(const OpDef &old,
                                 typename dtype_traits<DT>::element_type constval) {
  auto &g = old.graph();
...
template <typename T>
inline ReplFunc gen_ConstScalar_f32(T &&expr) {
...
template <typename T>
inline ReplFunc gen_ConstScalar_i32(T &&expr) {
...
//
template <typename... Ts>
struct all_are_int_helper {
  static_assert(sizeof...(Ts) == 0, "template problem");
  static constexpr bool value = true;
};
template <typename T1, typename... Ts>
struct all_are_int_helper<T1, Ts...> {
...
template <typename... Ts>
inline constexpr bool all_are_int() {
...
template <typename... Ts>
inline ReplFunc gen_ConstArr_vals_i32(Ts &&...values) {
 ...
 *
 * The first parameter is always Replacement &; the second is a Split_Context const & (obtained via
 * the 'split_tag' parmeter to the AUTOSPLIT_SHAPEFN_APPLY' and the remaining parameters are
 * obtained from the AUTOSPLIT_SHAPEFN_APPLY, and may be OpRef (mapped from "OperandTag" in the
 * SHAPEFN_APPLY), or scalar values.
 *
 * The return value may be an OpRef representing a new graph object; instead, the function may
 * return a QuickShape object representing a shape, and the framework will convert this to an
 * OpDef_Shape.
 *
 *
 * @{
...
///
API_EXPORT QuickShape simpledim_split_start(Replacement &rpx, Split_Context const &splitinfo,
...
/// This is used within CHANGEDIM_SLICE
///
API_EXPORT QuickShape simpledim_split_size(Replacement &rpx, Split_Context const &splitinfo,
                                           OpRef const &orig, int dim);
...
///
API_EXPORT QuickShape simple_split_start(Replacement &rpx, Split_Context const &splitinfo,
                                         OpRef const &orig);
...
/// This is used within TYPICAL_SLICE
///
API_EXPORT QuickShape simple_split_size(Replacement &rpx, Split_Context const &splitinfo,
                                        OpRef const &orig);

// :::EXTERNAL_SHAPEFN::: {  qshape conv_valid_split_start(split,op,op); }

/// @brief make 'start' shape for splitting input to 'valid' convolution, where the input is being
/// split along height or width
///
/// Generates shape {0, SPLIT_START * stride_h, 0, 0 }
/// or
/// Generates shape {0, 0, SPLIT_START * stride_w, 0 }
API_EXPORT QuickShape conv_valid_split_start(Replacement &rpx, Split_Context const &splitinfo,
                                             OpRef const &Act, OpRef const &stride);

// :::EXTERNAL_SHAPEFN::: {  qshape conv_valid_split_size(split,op,op,int,int); }

/// @brief make 'size' shape for splitting input to 'valid' dilated convolution, where the input is
/// being split along height or width
///
...
///
API_EXPORT QuickShape conv_valid_split_size(Replacement &rpx, Split_Context const &splitinfo,
                                            OpRef const &Act, OpRef const &stride, int window,
                                            int dilation);
...
API_EXPORT QuickShape conv_split_start_valid(Replacement &rpx, Split_Context const &splitinfo,
                                             OpRef const &Act, OpRef const &weights,
                                             OpRef const &stride);

// :::EXTERNAL_SHAPEFN::: {  qshape conv_split_size_valid(split,op,op,op); }

/// @brief make 'size' shape for splitting input to 'valid' convolution, where the input is being
/// split along height
///
...
///
API_EXPORT QuickShape conv_split_size_valid(Replacement &rpx, Split_Context const &splitinfo,
                                            OpRef const &Act, OpRef const &weights,
                                            OpRef const &stride);
...
/// @brief make 'size' shape for splitting input to 'valid' dilated convolution, where the input is
/// being split along height
///
/// Generates shape {0, inrows, 0, 0 }
///
/// where inrows = stride_h * (SPLIT_SIZE-1) + (filter_h - 1) * dilation + 1
///
API_EXPORT QuickShape conv_split_size_valid_dil(Replacement &rpx, Split_Context const &splitinfo,
                                                OpRef const &Act, OpRef const &weights,
                                                OpRef const &stride, OpRef const &dilation);

// :::EXTERNAL_SHAPEFN::: {  qshape pool_split_start_valid(split,op,op,op); }

/// @brief make 'start' shape for splitting input to 'valid' Xpool, where the input is being split
/// along height
///
/// Generates shape {0, SPLIT_START * stride_h, 0, 0 }
API_EXPORT QuickShape pool_split_start_valid(Replacement &rpx, Split_Context const &splitinfo,
                                             OpRef const &Act, OpRef const &window,
                                             OpRef const &stride);

// :::EXTERNAL_SHAPEFN::: {  qshape pool_split_size_valid(split,op,op,op); }

/// @brief make 'size' shape for splitting input to 'valid' Xpool, where the input is being split
/// along height
///
/// Generates shape {0, inrows, 0, 0 }
///
...
///
API_EXPORT QuickShape pool_split_size_valid(Replacement &rpx, Split_Context const &splitinfo,
                                            OpRef const &Act, OpRef const &window,
                                            OpRef const &stride);
...
API_EXPORT QuickShape offset_into_concat(Replacement &rpx, Split_Context const &splitinfo,
 ...
 * These are functions which may be used with SHAPEFN_APPLY.
 *
 * The first parameter is always Replacement &; the remaining parameters are obtained from the
 * SHAPEFN_APPLY, and may be OpRef (mapped from "OperandTag" in the SHAPEFN_APPLY), or scalar
 * values.
 *
 * The return value may be an OpRef representing a new graph object; instead, the function may
 * return a QuickShape object representing a shape, and the framework will convert this to an
...
API_EXPORT QuickShape shape_add_on_axis(Replacement &rpx, OpRef const &start, OpRef const &amt,
                                        int axis);

// :::EXTERNAL_SHAPEFN::: {  qshape conv_same_padded_size(op,op,op); }
///@brief find padded shape for input to 'same' convolution
///
/// For a 'same' convolution, produce a shape the same as 'Act', but expanded in H and W dimensions
/// to allow for the padding needed (as determined by the given filter shape and stride)
///
API_EXPORT QuickShape conv_same_padded_size(Replacement &rpx, OpRef const &Act,
...
API_EXPORT QuickShape conv_same_padded_size_dilation(Replacement &rpx, OpRef const &Act,
                                                     OpRef const &weights, OpRef const &stride,
...
///
/// For a 'same' convolution, produce a shape which indicates how the input needs to be padded on
/// top and left to be processed as 'valid' - as determined by the given filter shape and stride.
/// The resulting shape will be
///
///   { 0, top_padding,  left_padding, 0 }
///
API_EXPORT QuickShape conv_same_before(Replacement &rpx, OpRef const &Act, OpRef const &weights,
                                       OpRef const &stride);
...
API_EXPORT QuickShape conv_same_before_dilation(Replacement &rpx, OpRef const &Act,
                                                OpRef const &weights, OpRef const &stride,
                                                OpRef const &dilation);
...
///
/// For a 'same' Xpool, produce a shape the same as 'Act', but expanded in H and W dimensions to
/// allow for the padding needed (as determined by the given window shape and stride)
///
...
///
/// For a 'same' Xpool, produce a shape which indicates how the input needs to be padded on top and
/// left to be processed as 'valid' - as determined by the given window shape and stride. The
/// resulting shape will be
///
///   { 0, top_padding,  left_padding, 0 }
///
API_EXPORT QuickShape pool_same_before(Replacement &rpx, OpRef const &Act, OpRef const &window,
                                       OpRef const &stride);
...
///
API_EXPORT QuickShape conv_s2d_shape(Replacement &rpx, OpRef const &Act, OpRef const &filter,
                                     OpRef const &stride);

// :::EXTERNAL_SHAPEFN::: {  qshape pad_total_for_qnn(op,op); }

///@brief use input pad_amount to calculate padded offset for input from 'QNN_Conv' to 'valid'
///convolution
///
/// For a 'QNN_Conv' convolution, produce a shape the same as 'Act', but expanded in H and W
/// dimensions, which is determined by the input pad_amount: [[h_before, h_after], [w_before,
/// w_after]]
///
API_EXPORT QuickShape pad_total_for_qnn(Replacement &rpx, OpRef const &Act,
...
///
API_EXPORT QuickShape pad_total_for_qnn_round(Replacement &rpx, OpRef const &Act,
                                              OpRef const &Stride, OpRef const &pad_amount,
                                              OpRef const &rounding_mode);

// :::EXTERNAL_SHAPEFN::: {  qshape pad_before_for_qnn(op,op); }

///@brief use input pad_amount to get the (top/left margin) for input from 'QNN_Conv' to 'valid'
///convolution
///
...
///   { 0, top_padding,  left_padding, 0 }
///
API_EXPORT QuickShape pad_before_for_qnn(Replacement &rpx, OpRef const &Act,
                                         OpRef const &pad_amount);

// :::EXTERNAL_SHAPEFN::: {  qshape explicit_pad_for_qnn(op,op); }

API_EXPORT OpRef explicit_pad_for_qnn(Replacement &rpx, OpRef const &output,
                                      OpRef const &pad_amount);
...
API_EXPORT QuickShape shape_after_transpose(Replacement &rpx, OpRef const &input,
...
API_EXPORT QuickShape shape_after_spaceToBatch(Replacement &rpx, OpRef const &input,
                                               OpRef const &block_size);
API_EXPORT QuickShape shape_after_spaceToBatch_w_pad(Replacement &rpx, OpRef const &input,
                                                     OpRef const &block_size, OpRef const &pads);

// :::EXTERNAL_SHAPEFN::: {  qshape shape_after_depthToSpace(op,op); }

///@brief gives the new shape after depthToSpace transformation
API_EXPORT QuickShape shape_after_depthToSpace(Replacement &rpx, OpRef const &input,
                                               OpRef const &block_size);
...
//  (1) first, we 'parse' the rule, this is done by executing the code in the context of a
//  MatchBuilder
...
//
// during matching:
///  - The matcher engine works by first checking all the Op types and parameter counts (in
///  pre-order
//     traversal), and then going back to bind/check the named operands, all  by following a table.
//     There are no "operand_tag_t" involved in this process, since the indices into the output
//     array are baked into the tables in advance.
//   - in the process, all the OpRef  stored in an array of at most [MATCH_MAX_PATTERN], which is
//     stored in the Match object. The first will contain the 'root' Opref, the next 'n' are the
//     matched subops, and the rest are distinct matched parameter names. 'n' could be 0.
//   - Each instance of MatchOpBase has an array std::vector<pair<operand_tag_t,int>> m_operindex,
//   which supplies
//     the 'operand' names for those matched refs, and maps them to indices in the array (the names
//     are in order).
//   - On a complete match we use Match.set_current_matchop() to install a pointer to the
//   MatchOpBase;
//     subsequently, in 'Constraint' and 'Replace' phases, that object's lookup_opertag() method is
//     used to map operand names to OpRef (it maps the m_operindex to an index into the array).
// TODO: we should probably also have a parallel array in Match of the corresponding OpDef pointers,
// which can be filled in lazily (starting with the ones already obtained during matching, with the
// rest init to NULL). This could reduce repeated lookups in op_def_map during Constraint and
// Replace phases.
//
//
...
class MatchOpBase {
 ...
 public:
  ...
  API_EXPORT char const *get_debug_desc() const {
    return match_debug_desc.get();
  }  // may return nullptr
  API_EXPORT std::map<OpId, operand_tag_parm_t> get_inverse_map(MatchOpState const &m) const;

  API_EXPORT const std::vector<std::pair<operand_tag_t, int>> &get_operindex() const {
    return m_operindex;
  };
...
class Match : public GraphOptContext_Base {
 ...
 public:
  ...
  API_EXPORT inline bool opid_is_new(OpId op) const {
...
class Constraint : public hnnx::Match {
 ...
 public:
  ...
  static ReplFuncBool constraint();
 ...
 * actually return a ReplFunc, which is a std::function that is called to generate the graph
 * element.
 *
 * Likewise, SPLIT_START, SPLIT_SIZE, SPLIT_DIM actually return ReplFuncInt, a std::function which
 * is called to generate the integer result, which changes as the autosplit is iterated.
 */
...
class Replacement : public Constraint {
 ...
 private:
  ...
  API_EXPORT inline OpRef apply_param_adapter(const OpDef &base, ReplFunc const &f) {
    return f(*this, base);
  }
  ...
  API_EXPORT OpRef do_ResizeDim(OpDef const &old, int dim, int size, ReplFunc const &f,
  ...
  // A thin subclass of ReplFunc, which can be constructed from a ReplFunc, but also
  // from an operand tag, or string (via Operand()), or a fixed OpRef (this is to support OUTPUT_OF
  // and similar)
  ...
  ///
  /// The reference shape used to evaluate 'expr' is modified from the default by changing dimension
  /// 'dim' to size'
  ///
  ...
  // immed_modifier does step (b) above; it makes the temp object from the ref result and the
  // current opdef The lambda inside WITH_output_like does steps (a) and (c).
  //
  ...
  API_EXPORT static ReplFunc WrapOp_internal(
      char const *op_name, char const *package, ReplFunc_or_Operand &&in_op,
      bool is_idem);  // True for WrapOp, false for WrapOpAlways

  // implements WrapOp("op", "parmname") specifically
  API_EXPORT static ReplFunc WrapOp_quick(char const *op_name, char const *package,
                                          char const *parm);
 ...
 private:
  ...
  API_HIDDEN inline static ReplFunc WITH_SPLIT_HISTORY(ReplFunc_or_Operand &&ref,
                                                       ReplFunc_general &&f) {
    return ReplFunc::create([=](Replacement &rpx, const OpDef &old) -> OpRef {
  ...
  /// \ingroup OptReplacement
  /// @brief WITH_SPLIT_HISTORY(refexp, dim, expr) - evaluate 'expr' using 'refexp' for the split
  /// history
  //
  ...
  API_HIDDEN inline static ReplFunc WITH_SPLIT_HISTORY(ReplFunc_or_Operand &&ref, int dim,
                                                       ReplFunc_general &&f) {
    return ReplFunc::create([=](Replacement &rpx, const OpDef &old) -> OpRef {
      OpRef const ref_id = ref(rpx, old);
      OpRef new_id = f(rpx, old);
 ...
 private:
  ...
  /// \ingroup OptReplacement
  /// @brief WITH_SAME_OUTPUT(refexp, expr) - evaluate 'expr' using 'refexp' for the reference
  /// output type and size
  API_HIDDEN inline static ReplFunc WITH_SAME_OUTPUT(ReplFunc_or_Operand &&ref,
                                                     ReplFunc_general &&f) {
    return WITH_output_like(std::move(ref), std::move(f), mode_with_type | mode_with_size);
  }

  /// \ingroup OptReplacement
  /// @brief Doesn't change anything in graph, just returns the op and adds it to const_tracker if
  /// enabled Does NOT support tracking scalars Please only use quant dependent constants, and avoid
  /// double tracking the same const
  ...
  /// If the target op changes multiple times in a phase, TRACK_SOURCE_ID needs to be used for
  /// recording each id change. It must be used together with cse_after_if in the same DEF_OPT.
  API_HIDDEN inline static ReplFunc TRACK_SOURCE_ID(ReplFunc_or_Operand &&ref, int nested_level,
                                                    ReplFunc_general &&f) {
    return ReplFunc::create([=](Replacement &rpx, const OpDef &old) -> OpRef {
  ...
  /// \ingroup OptReplacement
  /// @brief gen_ShapeOf(any_oper) - Construct an OpDef_Shape with the shape taken from the given
  /// graph operation.
  API_EXPORT static ReplFunc gen_ShapeOf(ReplFunc_or_Operand &&shape);

  API_EXPORT static inline OpDef immed_modifier_OUTPUT_TYPE(OpDef const &old, DType dtype,
                                                            int32_t zero_offset, float stepsize) {
    OutputDef temp{};
    temp.dtype = dtype;
 ...
 public:
  /// \ingroup OptReplacement
  /// @brief WITH_OUTPUT_TYPE(dtype,zero_offset,stepsize,expr) - evaluate 'expr' but using the
  /// specified output type.
  ///
  /// A temporary reference is created which specifies the given dtype, step, and offset instead of
  /// the default; this is used to evaluate 'expr'. If the dtype is not quantized, use 0 and 1.0f
  /// for zero_offset and stepsize.
  API_EXPORT static ReplFunc WITH_OUTPUT_TYPE(DType dtype, int32_t zero_offset, float stepsize,
                                              ReplFunc_general &&f);

  // adapter to allow  WITH_OUTPUT_TYPE to be called with some mixture of literals and oExp, and
  // have them all converted to function objects, which are forwarded to WITH_OUTPUT_TYPE_func
  template <typename TDT, typename TZO, typename TSS>
  static inline ReplFunc WITH_OUTPUT_TYPE(TDT &&dtype, TZO &&zero_offset, TSS &&stepsize,
                                          ReplFunc_general &&f) {
 ...
 private:
  /// \ingroup OptReplacement
  /// @brief WITH_MULT_OUT(int num_outputs, expr) - evaluate 'expr' with 'DType::Multi' for
  /// 'num_outputs' outputs
  ///
  /// A temporary reference is created with OutputDef configured to make an Multi-Output op with the
  /// given number of outputs. This is used to evaluate 'expr'. num_outputs must be >=2.
  ///
  ...
  /// \ingroup OptReplacement
  /// @brief SHAPEFN_APPLY(function,parms...) - generate a shape object by calling a specified
  /// function.
  ///
  /// The named function is called, with specified parameters. These can be strings (assumed to be
  /// be operand references, and converted to OpRef), or scalar expressions.
  ///
  /// See also: \ref ShapeFnApply
  template <typename F_T, typename... Arg_Ts>
  API_HIDDEN static ReplFunc SHAPEFN_APPLY(F_T f, Arg_Ts... args) {
    return ReplFunc::create([=](Replacement &rpx, const OpDef &old) -> OpRef {
      /* Call f(rpx,args...) */
  ...
  /// \ingroup OptReplacement
  /// @brief AUTOSPLIT_SHAPEFN_APPLY(function, "split_tag", parms...) - generate a shape object by
  /// calling a specified function.
  ///
  /// The named function is called, with specified parameters. These can be strings (assumed to be
  /// be operand references, and converted to OpRef), or scalar expressions. The 'split_tag'
  /// parameter is converted to a reference to a Split_Context
  ///
  /// See also: \ref AutoSplitShapeFnApply
  template <typename F_T, typename... Arg_Ts>
  API_HIDDEN static ReplFunc AUTOSPLIT_SHAPEFN_APPLY(F_T f, hnnx::split_context_tag_t whatsplit,
                                                     Arg_Ts... args) {
    return ReplFunc::create([=](Replacement &rpx, const OpDef &old) -> OpRef {
      /* Call f(rpx,split_context.at(whatsplit),args...) */
      return rpx.immed_SHAPEFN_APPLY(old, f, rpx.lookup_split(whatsplit),
  ...
  /// \ingroup OptReplacement
  /// @brief AUTOSPLIT_SLICE(in, start, size ) -> WITH_SIZE( size, WITH_TYPE( in, Op("Slice_shape",
  /// in, start, size)))
  ///
  /// This generates a "Slice_shape" op applied to the given input 'in', with the given 'start' and
  /// 'size' shapes. The output shape is configured to match 'size', and the output type is always
  /// the same as 'in'
  ///
  ...
  API_EXPORT static ReplFunc TYPICAL_SLICE(ReplFunc_or_Operand &&in,
  ...
  API_EXPORT static ReplFunc CHANGEDIM_SLICE(ReplFunc_or_Operand &&in,
                                             hnnx::split_context_tag_t whatsplit, int newdim);

  // Pretty much all TYPICAL_SLICE and CHANGEDIM_SLICE are just ("string", "string") .. so this
  // wrapper will save some code space at the call sites.
  API_EXPORT static ReplFunc CHANGEDIM_SLICE(char const *in_parm, char const *whatsplit,
                                             int newdim);
 ...
 public:
  ...
  API_EXPORT OpRef do_TYPICAL_SLICE(OpDef const &old, OpRef input_op,
                                    hnnx::split_context_tag_t whatsplit, int newdim,
                                    bool reduce_dim = false);

  API_EXPORT OpRef do_AUTOSPLIT(OpDef const &old, int dim, Split_Context &splitinfo, int chunksize,
                                ReplFunc const &f, bool reduce_dim = false,
                                bool autothread = false);

  API_EXPORT OpRef do_AUTOTHREAD(OpDef const &old, int dim, hnnx::split_context_tag_t varname,
                                 int ntiles, ReplFunc const &f);
  ...
  /// slices along dimension dim, with each slice being of 'size' (or possibly smaller, for the last
  /// one).
  ///
  /// This done by
  ///
  ...
  ///   * Within each iteration, SPLIT_START("tag") and SPLIT_SIZE"tag"), when evaluated within
  ///   <repl_expression>,
  ///    will reflect the extent of the current split in the output, and thus can be used to
  ///    construct the corresponding slices of the input. SLICE_DIM("tag") will always give the
  ///    value supplied to the AUTOSPLIT as 'dim'. Normally, this is all done within functions
  ///    invoked via AUTOSPLIT_SHAPEFN_APPLY.
  ///
  /// Rules with AUTOSPLIT should have a constraint to prevent them from being applied where the
  /// split dimension does not exceed size.
  ///
 ...
 private:
  ...
  API_HIDDEN inline static ReplFunc AUTOSPLIT(oExp::expr<V1, T1> &&dim,
                                              hnnx::split_context_tag_t varname,
                                              oExp::expr<V2, T2> &&chunksize,
                                              ReplFunc_general &&f) {
  ...
  API_HIDDEN inline static ReplFunc AUTOSPLIT(oExp::expr<V1, T1> &&dim,
                                              hnnx::split_context_tag_t varname, int chunksize,
                                              ReplFunc_general &&f) {
    return AUTOSPLIT_func(oExp::wrap_as_function<int>(std::move(dim)), varname,
 ...
 public:
  // Performs AUTOSPLIT in the specified dimension to create at most options.autothread_hvx_ntiles
  // splits that will not be further autothreaded.
  API_EXPORT static ReplFunc AUTOTHREAD_HVX(int dim, hnnx::split_context_tag_t varname,
                                            ReplFunc_general &&f);
  // Same for options.autothread_hmx_ntiles.
  API_EXPORT static ReplFunc AUTOTHREAD_HMX(int dim, hnnx::split_context_tag_t varname,
                                            ReplFunc_general &&f);

  static ReplFunc first_AUTOSPLIT(int const dim, hnnx::split_context_tag_t const varname,
                                  int const chunksize, ReplFunc const &f);
 ...
 private:
  ...
  API_FUNC_HIDDEN inline static ReplFunc AUTOSPLIT_FIRST(int dim, hnnx::split_context_tag_t varname,
                                                         oExp::expr<V2, T2> &&chunksize,
                                                         ReplFunc_general &&f) {
    return Replacement::first_AUTOSPLIT(dim, varname,
                                        oExp::wrap_as_function<int>(std::move(chunksize)), f);
  }

  API_FUNC_HIDDEN inline static ReplFunc AUTOSPLIT_FIRST(int const dim,
                                                         hnnx::split_context_tag_t const varname,
                                                         int const chunksize,
                                                         ReplFunc_general const &&f) {
    return Replacement::first_AUTOSPLIT(dim, varname, chunksize, f);
  }

 private:
  /// AUTOSPLIT and reduce the dim
  API_EXPORT static ReplFunc AUTOSPLIT_REDUCE(int dim, hnnx::split_context_tag_t varname,
                                              ReplFunc_general &&f);

  API_EXPORT static ReplFunc TYPICAL_SLICE_REDUCE(ReplFunc_or_Operand &&in,
                                                  hnnx::split_context_tag_t whatsplit);
  ...
  ///
  /// The operation will iterate for "I" >= lo_index, < hi_index; for each value, the
  /// repl_expression is evaluated, and a new Op is created which
  ///
  ...
  ///
  API_EXPORT static ReplFunc OP_ITER(ReplFunc &&op_base, hnnx::split_context_tag_t varname,
                                     int lo_index, int hi_index, ReplFunc_general &&f);

  // same with ReplFuncInt for the index values, so they can be expressions
  API_EXPORT static ReplFunc OP_ITER_func(ReplFunc &&op_base,
                                          hnnx::split_context_tag_t const &varname,
  ...
  API_HIDDEN inline static ReplFunc OP_ITER(ReplFunc &&op_base, hnnx::split_context_tag_t varname,
                                            TLO &&lo_index, THI &&hi_index, ReplFunc_general &&f) {
    return OP_ITER_func(std::move(op_base), varname,
                        oExp::wrap_as_function<int>(std::forward<TLO>(lo_index)),
                        oExp::wrap_as_function<int>(std::forward<THI>(hi_index)), std::move(f));
  }

  API_EXPORT OpRef do_OP_ITER(OpDef const &old, OpDef const &base_op, Split_Context &splitinfo,
                              int lo_index, int hi_index, ReplFunc const &f);

  /// \ingroup OptReplacement
  /// @brief INHERIT_MEMOS_FROM(refexp, expr) - evaluate 'expr' inheriting any persistent memos from
  /// 'refexp'

  API_EXPORT void do_INHERIT_MEMOS(const OpDef &old, OpDef &newdef);

  API_HIDDEN inline static ReplFunc INHERIT_MEMOS_FROM(ReplFunc_or_Operand &&ref,
                                                       ReplFunc_general &&f) {
    return ReplFunc::create([=](Replacement &rpx, const OpDef &old) -> OpRef {
      OpRef const ref_id = ref(rpx, old);
      OpRef new_id = f(rpx, old);
  ...
  /// The return value is the OpRef of the replacement. If it's the same as the OpRef of 'op', it is
  /// assumed that the rule has no effect in this situation.
  ///
  ...
  ///
  template <typename... Ts>
  API_HIDDEN inline static ReplFunc OpMultiOut(unsigned n_out, unsigned outno, char const *opstr,
                                               Ts &&...ts) {
 ...
 public:
  ...
  static ReplFunc Operand(hnnx::operand_tag_parm_t str) {
    return ReplFunc::create(
        [=](Replacement &rpx, const OpDef &old) -> OpRef { return rpx.get_opref(str); });
  }
  ...
  API_EXPORT static ReplFunc Op_inner(char const *str, char const *package, int n_in,
  ...
  ///
  /// The inputs can be any 'replacement' expressions, or operand tags; the shape and type of the Op
  /// output are inherited from the replaced Op -- or from the innermost modifier, if the Op appears
  /// within a modifier.
  ///

  template <typename... Ts>
  API_HIDDEN static ReplFunc Op(char const *str, Ts... ts) {
...
static auto SELECT( bool sel, TA &&iftrue, TB &&iffalse) -> decltype(sel?iftrue:iffalse) {
  ...
  API_EXPORT static ReplFunc SELECT(bool sel, ReplFunc_general &&iftrue,
                                    ReplFunc_general &&iffalse);
  API_EXPORT static ReplFunc SELECT(bool sel, ReplFunc_or_Operand &&iftrue,
  ...
  template <oExp::Variant V, typename T>
  static ReplFunc SELECT(oExp::expr<V, T> &&condn, ReplFunc_or_Operand &&iftrue,
                         ReplFunc_or_Operand &&iffalse) {
    return SELECT_func(oExp::wrap_as_function<bool>(std::move(condn)), std::move(iftrue),
                       std::move(iffalse));
  }
...
OpRef do_replacement(const OpDef & oldop, hnnx::operand_tag_parm_t str)
{
 ...
 public:
  ...
  API_EXPORT static OpRef gen_node(const hnnx::opname_tag_t str, size_t n_in, OpRef const *inputs,
                                   const OpDef &old, const OutputDef *new_odef = nullptr,
                                   char const *package_name = THIS_PKG_NAME_STR,
                                   const OpDef *model = nullptr);
  static inline OpRef gen_node(const hnnx::opname_tag_t str, std::vector<OpRef> const &inputs,
                               const OpDef &old, const OutputDef *new_odef = nullptr,
                               char const *package_name = THIS_PKG_NAME_STR,
                               const OpDef *model = nullptr) {
  ...
  static inline OpRef gen_node(const hnnx::opname_tag_t str, std::initializer_list<OpRef> inputs,
                               const OpDef &old, const OutputDef *new_odef = nullptr,
                               char const *package_name = THIS_PKG_NAME_STR,
                               const OpDef *model = nullptr) {
  ...
  template <size_t N>
  static inline OpRef gen_node(const hnnx::opname_tag_t str, std::array<OpRef, N> const &inputs,
                               const OpDef &old, const OutputDef *new_odef = nullptr,
                               char const *package_name = THIS_PKG_NAME_STR,
                               const OpDef *model = nullptr) {
  ...
  template <DType DT>
  API_EXPORT OpRef gen_Const_scalar(const OpDef &old,
                                    typename dtype_traits<DT>::element_type constval);

  template <DType DT>
  API_EXPORT OpRef gen_Const_1D_array(const OpDef &old,
                                      typename dtype_traits<DT>::element_type const *vals,
                                      size_t n);

  template <DType DT>
  API_EXPORT OpRef gen_Const_mD_array(const OpDef &old,
                                      typename dtype_traits<DT>::element_type const *vals, size_t n,
                                      size_t m);

  API_EXPORT OpRef gen_Const_int32_common(const OpDef &old, const OutputDef &out_def,
                                          const uint8_t *data, size_t data_len);

  API_EXPORT OpRef gen_Const_float_common(const OpDef &old, const OutputDef &out_def,
                                          const uint8_t *data, size_t data_len);

  template <typename UniqueType>
  API_EXPORT static ReplFunc first_replacement();
  template <typename UniqueType>
  API_EXPORT static ReplFunc replacement();
  // typedef OpRef (Replacement::*replacementfn_type)(const OpDef &oldop);
  typedef ReplFunc (*replacementfn_type)();

  template <typename T>
  static constexpr bool is_Op_type() {
...
API_EXPORT std::pair<NN_INT32_T, bool> getconst_int_impl(GraphPrepare &g, OpDef const &opdef,
                                                         int index);
API_EXPORT std::pair<NN_INT32_T, bool> getconst_int_impl(GraphPrepare &g, OpDef const &opdef,
                                                         int index, int index2);
API_EXPORT std::pair<float, bool> getconst_float_impl(GraphPrepare &g, OpDef const &opdef,
                                                      int index);
API_EXPORT std::pair<float, bool> getconst_float_impl(GraphPrepare &g, OpDef const &opdef,
                                                      int index, int index2);
...
template <typename T>
entire_defopt get_entire_defopt();
...
class GraphOptInfo {
 ...
 protected:
  ...
  virtual void build_matchop() {
    ...
    if (defopt.register_tiling &&
        (get_flags() & (OptimFlags::flags_t(OptimFlags::central_autosplit_flag) |
                        OptimFlags::flags_t(OptimFlags::central_only_autosplit_flag))))
      defopt.register_tiling(this);
  }

 public:
  API_EXPORT GraphOptInfo(int priority, OptimFlags::flags_t flags_in,
  ...
  API_EXPORT static void insert_optimization(std::map<int, GraphOptPass> &opt_passes,
                                             GraphOptInfo *p);
  API_EXPORT static void populate_package_optimization_map(
...
#ifndef PREPARE_DISABLED
  template <typename T>
  static void declare_tiling_rule(GraphOptInfo *);

  // Register a tiling rule (defined in tiler.cc)
  static unsigned declare_tiling_rule(unsigned dim, const char *Var, GraphOptInfo *,
  ...
  void set_priority(int new_priority) const {
    const_cast<GraphOptInfo *>(this)->priority = new_priority;
  }
#endif
};

#ifndef DEF_OPT_COMPILE
#define DEF_AUTOSPLIT_COMMON(PRIORITY, FLAGS, MATCHCODE, CONSTRAINTCODE, dim, var, CHUNKSIZE,   \
                             REPLACE)                                                           \
  template <>                                                                                   \
  hnnx::MatchAst_uptr MatchBuilder::matcher<UNIQUE_TYPE>() {                                    \
    return MATCHCODE;                                                                           \
  }                                                                                             \
  template <>                                                                                   \
  ReplFuncBool Constraint::constraint<UNIQUE_TYPE>() {                                          \
    using namespace oExp_for_cst;                                                               \
    using oExp::INT;                                                                            \
    using oExp::UINT;                                                                           \
    auto result = oExp::wrap_param_to<bool>(AND(                                                \
        CONSTRAINTCODE, OR(OPTION_BOOL("ignore_chunksize"), GT(DIM_OF("*", dim), CHUNKSIZE)))); \
    return oExp::wrap_as_function<bool>(result);                                                \
  }                                                                                             \
  template <>                                                                                   \
  ReplFunc Replacement::replacement<UNIQUE_TYPE>() {                                            \
  ...
  template <>                                                                                   \
  ReplFunc Replacement::first_replacement<UNIQUE_TYPE>() {                                      \
    using namespace oExp_for_repl;                                                              \
    using oExp::INT;                                                                            \
    using oExp::UINT;                                                                           \
    pkg_flag = THIS_PKG_NAME_STR;                                                               \
  ...
  hnnx::OptimFlags::flag_evaluate<UNIQUE_TYPE>() noexcept {                                     \
    return static_cast<uint32_t>(any_rule) | static_cast<uint32_t>(FLAGS);                      \
  }                                                                                             \
  template <>                                                                                   \
  void hnnx::GraphOptInfo::declare_tiling_rule<UNIQUE_TYPE>(GraphOptInfo * info) {              \
    declare_tiling_rule(dim, var, info, __FILE__, __LINE__);                                    \
  }                                                                                             \
  template <>                                                                                   \
  hnnx::entire_defopt hnnx::get_entire_defopt<UNIQUE_TYPE>() {                                  \
    return hnnx::entire_defopt{                                                                 \
        MatchBuilder::matcher<UNIQUE_TYPE>(), Constraint::constraint<UNIQUE_TYPE>(),            \
        Replacement::replacement<UNIQUE_TYPE>(), Replacement::first_replacement<UNIQUE_TYPE>(), \
...
#define DEF_AUTOSPLIT_ORDERED(PRIORITY, MATCHCODE, CONSTRAINTCODE, dim, var, CHUNKSIZE, REPLACE) \
  DEF_AUTOSPLIT_COMMON(PRIORITY, ordered_autosplit_flag, MATCHCODE, CONSTRAINTCODE, dim, var,    \
                       CHUNKSIZE, REPLACE)
...
class GraphOptPass {
  ...
  using rules_map_t = std::conditional_t<std::is_same_v<opname_tag_t, string_tag_t>,
                                         minihash_noerase<opname_tag_t, MatcherState>,
  ...
  static uint64_t hash(hnnx::opname_tag_t opstr) {
    return uint64_t(1) << (find_opname_hash(opstr) & 63);
  }
 ...
 public:
  ...
  class RuleList {
   ...
   public:
    RuleList(MatchOpState &matchop_state, const MatcherState &state)
        : matchop_state(matchop_state), state(state) {}
    API_EXPORT MatchIterator begin() const noexcept {
      return MatchIterator(matchop_state, state, 0);
    }
...
API_EXPORT std::map<std::string, std::vector<std::unique_ptr<GraphOptInfo>> *>
    &get_pkg_opt_tmp_map();
...
API_EXPORT void add_package_opt(std::vector<std::unique_ptr<GraphOptInfo>> &opts, int priority,
                                ...
                                char const *const fname, const int lineno);
...
#define INIT_PACKAGE_OPTIMIZATION_DEF()                                                     \
  API_HIDDEN std::vector<std::unique_ptr<hnnx::GraphOptInfo>>                               \
...
#define REGISTER_EXTERNAL_PACKAGE_OPT(PRIORITY, FLAGS, DEFOPT) \
  APPEND_REG_OPT_ELEM(PRIORITY, FLAGS, DEFOPT, __LINE__)

#define REGISTER_INTERNAL_PACKAGE_OPT(PRIORITY, FLAGS, DEFOPT) \
  APPEND_REG_OPT_ELEM(PRIORITY, FLAGS, DEFOPT, __LINE__)
...
#define DEF_PACKAGE_OPTIMIZATION_WITH_FLAGS(PRIORITY, FLAGS, MATCHCODE, CONSTRAINTCODE,     \
                                            REPLACECODE)                                    \
...
#define DEF_INTERNAL_PACKAGE_OPTIMIZATION_WITH_FLAGS(PRIORITY, FLAGS, MATCHCODE, CONSTRAINTCODE, \
                                                     REPLACECODE)                                \
  DEF_PACKAGE_OPTIMIZATION_COMMON(PRIORITY, FLAGS, MATCHCODE, CONSTRAINTCODE, REPLACECODE)       \
  REGISTER_INTERNAL_PACKAGE_OPT((PRIORITY), hnnx::OptimFlags::flag_evaluate<UNIQUE_TYPE>(),      \
                                &hnnx::get_entire_defopt<UNIQUE_TYPE>);
...
#define DEF_PACKAGE_OPTIMIZATION_COMMON(PRIORITY, FLAGS, MATCHCODE, CONSTRAINTCODE, REPLACECODE) \
  template <>                                                                                    \
  [[gnu::always_inline, gnu::cold]] hnnx::MatchAst_uptr MatchBuilder::matcher<UNIQUE_TYPE>() {   \
    return MATCHCODE;                                                                            \
  }                                                                                              \
  template <>                                                                                    \
  [[gnu::always_inline, gnu::cold]] ReplFuncBool Constraint::constraint<UNIQUE_TYPE>() {         \
    using namespace oExp_for_cst;                                                                \
    using oExp::INT;                                                                             \
    using oExp::UINT;                                                                            \
  ...
  template <>                                                                                    \
  [[gnu::always_inline, gnu::cold]] ReplFunc Replacement::replacement<UNIQUE_TYPE>() {           \
    using namespace oExp_for_repl;                                                               \
    using oExp::INT;                                                                             \
    using oExp::UINT;                                                                            \
    pkg_flag = THIS_PKG_NAME_STR;                                                                \
...
#define DEF_OPTIM(PRIORITY, FLAGS, MATCHCODE, CONSTRAINTCODE, REPLACECODE)                 \
  DEF_INTERNAL_PACKAGE_OPTIMIZATION_WITH_FLAGS(PRIORITY, FLAGS, MATCHCODE, CONSTRAINTCODE, \
                                               REPLACECODE)
...
#define DEF_TENSOR_PROPERTIES(...)                                                         \
  ...
  [[maybe_unused]] static bool CTRICKS_PASTER(opdef_proprety, __LINE__) =                  \
      hnnx::register_tensor_properties(THIS_PKG_NAME_STR,                                  \
                                       TensorInfoBuilder(THIS_PKG_NAME_STR, __VA_ARGS__)); \
  }
#else
...
#define COMPILER_FOR_UPDATEABLE_QUANT_WITH_CHECKS(XXF, FUNC, PARA, PRE, POST)                   \
  template <>                                                                                   \
  constexpr bool has_compile_method<&XXF> = true;                                               \
  template <>                                                                                   \
  struct OpaqueT_FOR<&XXF> {                                                                    \
    using type = PARA;                                                                          \
  };                                                                                            \
  template <>                                                                                   \
  bool hnnx::TypicalOpWithCompiler<&XXF, PARA>::check_constraint_for_recompile(Graph &graph_in) \
      const {                                                                                   \
    return POST(graph_in, this);                                                                \
  }                                                                                             \
  template <>                                                                                   \
  hnnx::Executable::ItemType hnnx::TypicalOpWithCompiler<&XXF, PARA>::compile(Graph &graph_in)  \
      const {                                                                                   \
    static_assert(check_szal());                                                                \
    auto [f, v] = FUNC(graph_in, this);                                                         \
    auto pre_check_res = PRE(graph_in, this);                                                   \
    Recompilable_param *const recomp_ptr =                                                      \
        (pre_check_res) ? (Recompilable_param *)(&this->opaque) : nullptr;                      \
...
template <typename T>
bool default_pre_check_for_recompile(Graph &graph_in, T *const op) {
  return true;
}

// the precheck is useful here for disabling recompile, set the precheck return to false
template <typename T>
bool pre_check_for_non_updateable(Graph &graph_in, T *const op) {
  return false;
}

template <typename T>
bool default_post_check_for_recompile(Graph &graph_in, T *const op) {
