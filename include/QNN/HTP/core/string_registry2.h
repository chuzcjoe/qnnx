...
// The empty string always maps to a specific value, a statically allocated entity which can be used
// for static init of string_key objects.
//
// The object pointed to is a pair <stringview, hashval> -- and the stringview pointer is guaranteed
// to be null-terminated - so the conversion from string_key to char * (or to its hash) is very
// quick.

// NOTE: entries cannot be deleted. the destructor of the string_registry_template<> will free
// everything. Exception: you can also call clear(), which deletes everything except the entry for
// empty string.
...
template <int K = 0x13121>
struct polynomial_string_hash {
...
class string_registry_two {
 ...
 public:
  ...
  // Use NOINLINE to avoid "definition of dllimport static field " and "unresolved external symbol"
  // errors on Windows
  API_EXPORT NOINLINE static string_key map_empty_str() { return &empty_string_node; };
};

template <class HASHFUNC>
typename string_registry_two<HASHFUNC>::mapval_t
    hnnx::string_registry_two<HASHFUNC>::empty_string_node = {{"", 0}, 0};

template <class HASHFUNC>
API_EXPORT hnnx::string_registry_two<HASHFUNC>::string_registry_two()
    : m_bulk_current(nullptr), m_bulk_pos(0) {}

template <class HASHFUNC>
API_EXPORT void hnnx::string_registry_two<HASHFUNC>::clear() {
  // clear the rev maps
  // clear the fwd map
  m_fwd_map.clear();
  // clear all the bulk storage (but leave one, if there is one)
  if (m_bulk.size() > 0) {
    while (m_bulk.size() > 1) m_bulk.pop_back();
    m_bulk_current = &m_bulk.back()[0];
    m_bulk_pos = 0;
  }
}
...
template <class HASHFUNC>
typename string_registry_two<HASHFUNC>::string_key hnnx::string_registry_two<HASHFUNC>::map_str(
    char const *s) {
