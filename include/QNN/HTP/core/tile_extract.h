 ...
 * the value into the lower bits of the 'flags' word (a zero value gives the default for the element
 * size).
 *
 *
 ...
 *  portions of the result will contain 'garbage' data - except for 'broadcast' as below. It is a
 * requirement that at least part of the tile falls within the bounds of the input tensor:
 *
 ...
 *      garbage values may be stored into margin areas when the tile overlaps left or right in width
 * dimension.
 *
 ...
 *          (b) return 'tmp_buffer'.
 *      (2) perform the operation, writing the results to the address returned by
 * write_tile_strategy (3) only if (ptr == tmp_buffer): call tens->write_buffer( flags, tmp_buffer,
 * b,h,w,d ) (with the *exact* params used in the call to write_tile_strategy). Step (1) can be
 * skipped if tile_support_direct() returns false for the output tensor (see below). Note also,
 *      'unshuffled' stores to 16-bit crouton may never be direct-mapped.
 *
 * Important: If you specify a particular height in the flags, do not exceed that when storing the
 * output, if using write_tile_strategy.  For instance, if TH=3 is specified in the flags, and
 * write_tile_strategy returns a direct pointer, the pointer may be to the last 3 rows of a crouton,
 * so storing 4 rows will corrupt some other data.
 *
 * CHECKING SUPPORT
 * ================
 ...
 *
 *  - tile_support():       if this returns false, the tile_read/write methods are not supported and
 * will throw an assert. (if properly deployed, this should only happen where the dtype of the
 * tensor is not supported by tiles)
 *  - tile_support_fast():    returns true if the tile support is at least better than a series of
 * element-by-element virtual calls.
 *  - tile_support_direct():  if true, there is a possibility that a 'direct mapping' to the tile
 * layout can occur, depending on the tile position (i.e. it's a crouton layout). When false, you
 * can skip calling write_tile_strategy() since it will never succeed.
 *
 *  Implementation node: there is actually just one virtual method tile_support_bits() which returns
 * 'unsigned'; the methods above test individual bits of that method's result.
 *
 */
...
// these are designed so that, for tensor types which don't support tile ops, the read_tile and
// write_tile methods can just jump to them.
...
constexpr bool tile_support_test_for_linfo() {
  ...
  if constexpr (Linfo::Rank != 4 ||
    ...
    if constexpr (std::is_same_v<Linfo, R4CroutonLayout> ||
                  std::is_same_v<Linfo, R4Crouton2Layout> ||
...
struct tile_methods {
  ...
  API_EXPORT static inline uint8_t const *tile_read(
      LayoutTensorType const *tensor,  // tensor to read from
      unsigned flags,
      uint8_t *tbuf,                  // caller-supplied buffer
      size_t b, int h, int w, int d)  // coordinates
  {
    // If this fires, it likely means a new tensor layout has been added, for which
    // 'tile_support_test_for_linfo' returns true; you can either:
    //   (a) modify tile_support_test_for_linfo() to return false for that - and no get no tile
    //   support; (b) re-enable generic support (see WITH_GENERIC_TILE_SUPPORT in tile_support.cc) -
    //   and get slow tile support;
...
template <>
struct tile_methods<Ldefs::Flat_8> : public tile_methods_r4flat<Ldefs::Flat_8> {};
template <>
struct tile_methods<Ldefs::Flat_16> : public tile_methods_r4flat<Ldefs::Flat_16> {};
template <>
struct tile_methods<Ldefs::Flat_32> : public tile_methods_r4flat<Ldefs::Flat_32> {};

// specialize for Crouton, padding case
// Methods are defined in tile_extract.cc
template <typename Linfo>
struct tile_methods_r4crouton {
  using TensType = LayoutTensor<Linfo>;
  static constexpr bool tile_support_any = true;
  static constexpr bool tile_support_fast = true;
  API_EXPORT static uint8_t const *tile_read(TensType const *tensor,  // tensor to read from
...
// 8 bit
template <>
struct tile_methods<Ldefs::Crouton_8> : public tile_methods_r4crouton<Ldefs::Crouton_8> {};
// 16 bit (different layout!)
template <>
struct tile_methods<Ldefs::Crouton_16> : public tile_methods_r4crouton<Ldefs::Crouton_16> {};

// 32 bit
template <>
struct tile_methods<Ldefs::Crouton_32> : public tile_methods_r4crouton<Ldefs::Crouton_32> {};

// 8 bit
template <>
struct tile_methods<Ldefs::Crouton4x1_8> : public tile_methods_r4crouton<Ldefs::Crouton4x1_8> {};
...
API_FUNC_EXPORT inline void *Tensor::write_tile_strategy(unsigned flags, void *buffer, size_t b,
                                                         int h, int w, int d) {
...
// These could be moved inside the classes, provided the "tile_read/write" functions are declared
// above that, and any specializations of it are defined before the tensor classes are specialized.
template <typename Linfo>
API_FUNC_EXPORT void const *LayoutTensor<Linfo>::read_tile(unsigned flags, void *buffer, size_t b,
                                                           int h, int w, int d) const {
  return (void const *)hnnx::tileExt_priv::tile_methods<Linfo>::tile_read(
      this, flags, (uint8_t *)buffer, b, h, w, d);
}
template <typename Linfo>
API_FUNC_EXPORT void LayoutTensor<Linfo>::write_tile(unsigned flags, void const *buffer, size_t b,
                                                     int h, int w, int d) {
  hnnx::tileExt_priv::tile_methods<Linfo>::tile_write(this, flags, (uint8_t const *)buffer, b, h, w,
                                                      d);
}
template <typename Linfo>
API_FUNC_EXPORT unsigned LayoutTensor<Linfo>::tile_support_bits() const {
...
template <typename T>
struct layout_def_of {};
template <typename L>
struct layout_def_of<LayoutTensor<L>> {
  using type = L;
};
//
...
template <typename TENST>
class tile_support_test {
...
/////////////////////////////////////////
template <unsigned NVECS>
struct aligned_buffer_base {
...
template <unsigned NVECS>
struct tile_buffer_template : public aligned_buffer_base<NVECS> {
 public:
...
template <unsigned NBUFS>
using tile_buffers = tile_buffers_template<NBUFS, 16>;

template <unsigned NBUFS>
using tile_half_buffers = tile_buffers_template<NBUFS, 8>;

template <unsigned NBUFS>
using tile_double_buffers = tile_buffers_template<NBUFS, 32>;

template <unsigned NBUFS>
using tile_quad_buffers = tile_buffers_template<NBUFS, 64>;
...
//
template <unsigned int RANK = 4>
class TileStoreWindowBase {
 protected:
  void *ptr;
 ...
 public:
  ...
  API_EXPORT std::pair<size_t const *, size_t> get_windims() const noexcept {
    return {winsize, RANK};
  }
  ...
  API_EXPORT TileStoreWindowBase(Tensor &otensor, TensorShape<RANK> const &out_shape,
                                 unsigned elbytes) {
  ...
  API_EXPORT TileStoreWindowBase(Tensor &otensor, std::array<size_t, RANK> out_dims,
                                 unsigned elbytes) {
    ptr = ptrw = otensor.raw_data();
    size_t stride = elbytes;
    for (int i = RANK - 1; i >= 0; --i) {
  ...
  API_EXPORT TileStoreWindowBase(Tensor &otensor, ITType const &itens,
                                 TensorShape<RANK> const &offset,
  ...
  API_EXPORT inline void set_window(TType const &tens, TensorShape<RANK> const &offset) {
...
template <unsigned ELBYTES, unsigned RANK = 4>
class TileStoreWindow : public TileStoreWindowBase<RANK> {
 public:
...
// dtype of the correct element size, everything else will work (and get_raw will work with a
// different return type of the same size). So there is a way to make a more generic one of these
// based on element size only, to support use cases where we want the same code to handle QUint8 and
// QUint8 for instance. The t(b,h,w,d) method will still convert correctly to/from float, since the
// interface object is taken from 'otensor':
//     TileStoreWindowTensorGeneric<ELBYTES>
//

template <DType DT>
class TileStoreWindowTensor : public FakeTensor {
 ...
 public:
  ...
  template <typename ITType>
  API_EXPORT TileStoreWindowTensor(Tensor &otensor, ITType const &itens,
                                   TensorShape<4> const &offset, TensorShape<4> const &out_shape)
      : FakeTensor(nullptr),
        ts_window(otensor, itens, offset, out_shape),
        intfc(otensor.interface()) {}

  template <typename TType>
  API_EXPORT inline void set_window(TType const &tens, TensorShape<4> const &offset) {
    ts_window.template set_window<TType>(tens, offset);
  }
  template <typename... ind_types>
  API_EXPORT inline element_type *get_raw_addr(ind_types... inds) {
  ...
  template <typename... ind_types>
  API_EXPORT inline element_type &get_raw(ind_types... inds) {
    static_assert(Rank == (sizeof...(ind_types)), "# of coords must match Rank");
    const std::array<SIdx, Rank> coords = {{static_cast<SIdx>(inds)...}};
 ...
 protected:
  API_EXPORT virtual void *element_addr(
      size_t rank, SIdx const coords_in[],
 ...
 public:
  ...
  API_EXPORT virtual size_t dim(size_t index) const noexcept override {
    return ts_window.win_dim(index);
  }
  ...
  // AMINE TODO: update TileStoreWindowBase to handle valid dims correctly
  virtual inline DynamicStatus get_dynamic_state() const override {
    return DynamicStatus::ValidData;
  }
  ...
  API_EXPORT virtual void write_tile(unsigned flags, void const *buffer, size_t b, int h, int w,
                                     int d) override final {
...
class TileStoreWindowTensorGeneric {
  static_assert(false && ELBYTES, "not specialized for this value of ELBYTES");
};

template <>
class TileStoreWindowTensorGeneric<1> : public TileStoreWindowTensor<DType::QUInt8> {};
template <>
class TileStoreWindowTensorGeneric<2> : public TileStoreWindowTensor<DType::QUInt16> {};
template <>
class TileStoreWindowTensorGeneric<4> : public TileStoreWindowTensor<DType::Int32> {};
...
template <unsigned ELBYTES>
API_FUNC_EXPORT int raw_copy_by_tiles(Tensor &out, Tensor const &in, unsigned flags = 0);
