...
template <typename STYPE, typename TLayout, typename Pad_t>
struct layout_mem_contig {
  ...
  API_EXPORT inline layout_mem_contig(Shape_t const *shp, layout_mem_contig const &other,
                                      hnnx::Allocator *alloc, Tensor::clone_mode cmode)
  ...
  // this implements set_raw_data_despite_danger(void *buffer) override final { bulk_data =
  ...
  API_EXPORT ALWAYSINLINE void *element_addr(Shape_t const *shp, size_t rank,
                                             SIdx const coords_in[]) const noexcept {
    // assert(rank == Rank);
  ...
  API_EXPORT inline storage_type **block_ptr_addr(Shape_t const *shape,
                                                  std::array<SIdx, Rank> coords) const {
    return &bulk_data;
  }
  // dummy for this
  API_EXPORT inline void realloc_blocktab(hnnx::Allocator *alloc, Shape_t const *old_shape,
...
API_EXPORT inline int compare_indirect_blocks(void **ptr_a, void **ptr_b, size_t nblocks,
                                              size_t blocklen) {
...
struct layout_mem_indirect {
  ...
  API_EXPORT layout_mem_indirect(Shape_t const *shp, Graph &graph_in)
      : blocktab((storage_type **)indirect_layout_mem::make_blocktab(
            layout.num_blocks(shp->max_dims), graph_in)) {}
  // duplicate clone from another
  API_EXPORT layout_mem_indirect(Shape_t const *shp, layout_mem_indirect const &other,
                                 hnnx::Allocator *alloc, Tensor::clone_mode cmode)
      : blocktab() {
  ...
  API_EXPORT int compare_memory(Shape_t const *shp, layout_mem_indirect const &rhs) const {
    ...
    return indirect_layout_mem::compare_indirect_blocks((void **)blocktab, (void **)rhs.blocktab,
                                                        nblocks, blocklen);
  }
  // find content hash of memory.
  //
  API_EXPORT uint32_t find_content_hash(Shape_t const *shp, uint32_t oldhash, bool is_float) const {
    size_t const nblocks = layout.num_blocks(shp->max_dims);
    size_t const blocklen = sizeof(storage_type) * layout.block_total(shp->max_dims);
    return Tensor::content_hash_data_indirect(oldhash, (void **)blocktab, nblocks, blocklen,
                                              is_float);
  }
};
///////////////////////////////////////////
...
class BlockTableAccessor {
 protected:
  ...
  static constexpr unsigned n_blockshape_dims =
      tensor_traits<LayoutTensor<Linfo>>::is_variable_block
          ? Rank - tensor_traits<LayoutTensor<Linfo>>::indirect_ranks
          : 0;
  std::array<size_t, n_blockshape_dims> blockshape_dims;

 public:
  API_EXPORT explicit BlockTableAccessor(LayoutTensor<Linfo> const &tens)
      : blktab(tens.blocktab_ptr()) {
    ...
    std::copy_n(shp.max_dims.begin() + (Rank - n_blockshape_dims), n_blockshape_dims,
                blockshape_dims.begin());
  }
  ...
  API_EXPORT inline size_t blocktab_blocksize() const {
    ...
    std::copy_n(blockshape_dims.begin(), n_blockshape_dims,
                dummy_shape.begin() + (Rank - blockshape_dims));
    return TLayout::block_total(dummy_shape);
  }
  API_EXPORT inline size_t blocktab_blocksize_bytes() const {
    return blocktab_blocksize() * sizeof(storage_type);
  }

  API_EXPORT inline size_t blocktab_dim(int i) const { return blkdims[i]; }
  API_EXPORT inline size_t blocktab_dim_stride(int i) const {
    return (i < Rank - 1) ? blkstrides[i + 1] : 1;
  }

  // block_ptr_address(b,h,w,d) and block_ptr accept element coordinates.

  template <typename... ind_types>
  API_EXPORT inline pointer_type *block_ptr_address(ind_types... inds) const {
  ...
  template <typename... ind_types>
  API_EXPORT inline pointer_type &block_ptr(ind_types... inds) const {
    static_assert(Rank == (sizeof...(ind_types)), "# of coords must match Rank");
    const std::array<SIdx, Rank> coords = {{static_cast<SIdx>(inds)...}};
    return *block_ptr_calc(coords);
  }
  ...
  //
  template <typename... ind_types>
  API_EXPORT inline pointer_type &blocktab(ind_types... inds) const {
    static_assert(Rank == (sizeof...(ind_types)), "# of coords must match Rank");
    const std::array<SIdx, Rank> coords = {{static_cast<SIdx>(inds)...}};
  ...
  // 'same_layout' means the same table shape and the same padding offset. Dims may not be
  // identical.
...
template <typename Linfo>
class LayoutTensor : public RankedTensor<Linfo::Rank> {
 protected:
  ...
  int32_t const *const dummy_interface_ptr =
      nullptr;  // need to immediately follow the interface pointer
  using Shape_t = Shape<Rank>;
 ...
 protected:  // interface, then shape, then mem
  using layout_mem_t =
      std::conditional_t<is_indirect, layout_mem_indirect<storage_type, TLayout, Pad_t>,
 ...
 protected:
  ...
  ALWAYSINLINE inline static Shape<Rank> const *init_shape_p(Graph &graph_in,
                                                             const OutputDef &def) {
    ...
    if constexpr (is_singular) {
      if (std::find_if(shp.dims.begin(), shp.dims.end(), [](size_t d) { return d != 1; }) !=
          shp.dims.end()) {
  ...
  [[gnu::noinline]] API_EXPORT LayoutTensor(const Op *producer_in, const OutputDef &def,
                                            Graph &graph_in,
                                            Interface const *(*ifc_maker)(Graph &,
                                                                          OutputDef const &))
      : BaseRT(producer_in),
        interface_ptr((*ifc_maker)(graph_in, def)),  //
        shape(init_shape_p(graph_in, def)),          //
        dynamic_shape(Dynamic_shape_t::crated_shape(
            graph_in, Dynamic_shape_t(shape->dims, DynamicStatus::ValidData))),
        mem(shape, graph_in) {}
  using interface_deser_func = Interface const *(*)(hnnx::Deserz &, Interface const **);
  [[gnu::noinline]] API_EXPORT LayoutTensor(hnnx::Deserz &dctx,
                                            interface_deser_func const ifc_deser_fp)
      : BaseRT(dctx),
        interface_ptr((*ifc_deser_fp)(dctx, &interface_ptr_ref())),
        shape(Shape_t::deserialize(dctx, &shape)),
        dynamic_shape(
            Dynamic_shape_t::deserialize(dctx, &dynamic_shape, (ShapeInterface_t const *)shape)),
        mem(shape, dctx) {}
  // clone ctor.
  [[gnu::noinline]] API_EXPORT LayoutTensor(const LayoutTensor &old, hnnx::Allocator *allocator,
                                            Tensor::clone_mode cmode)
      : BaseRT(old, allocator, cmode),
        interface_ptr(old.interface_ptr),
        shape(old.shape),
        dynamic_shape(old.dynamic_shape),
 ...
 public:
  ...
  API_EXPORT virtual inline size_t max_dim(size_t index) const noexcept override final {
    return shape->dims[index];
  }
  API_EXPORT const std::array<size_t, Rank> &max_dims() const { return shape->dims; }

  template <typename... T>
  API_EXPORT const std::array<size_t, sizeof...(T)> dims(T... indices) const {
  ...
  template <typename... T>
  API_EXPORT const std::array<size_t, sizeof...(T)> max_dims(T... indices) const {
  ...
  API_EXPORT virtual inline DynamicStatus get_dynamic_state() const override {
    return dynamic_shape->get_state();
  }
  ...
  API_EXPORT inline float interface_scale_recip() const {
    return this->interface().get_scale_recip();
  }
  ...
  // block table access
  API_EXPORT inline storage_type **blocktab_ptr() const {
 ...
 protected:
                                       ...
                                       size_t const *const p_new_pads = nullptr)  // optional pads
  {
    ...
    if (p_new_dims) {
      for (int i = 0; i < Rank; i++) newshape.dims[i] = p_new_dims[i];
    }
    if (p_new_pads) {
      for (int i = 0; i < Rank; i++) newshape.pad[i] = p_new_pads[i];
    }
 ...
 public:
  ...
  // Can only be done early in prepare (i.e. just as Op is created).
  inline void change_shape_pad(std::array<size_t, Rank> const &new_dims,
                               std::array<size_t, Rank> const &new_pad,
  ...
  template <typename... ind_types>
  API_EXPORT inline storage_type *const *block_ptr_address(ind_types... inds) {
    const std::array<SIdx, Rank> coords = {{static_cast<SIdx>(inds)...}};
    return mem.block_ptr_addr(shape, coords);
  }
  template <typename... ind_types>
  API_EXPORT inline storage_type const *block_ptr(ind_types... inds) const {
    return *block_ptr_address(inds...);
  }
  template <typename... ind_types>
  API_EXPORT inline storage_type *block_ptr(ind_types... inds) {
    return *block_ptr_address(inds...);
  }

  API_EXPORT std::conditional_t<is_indirect, BlockTableAccessor<Linfo>, void> blocktable_accessor()
      const {
    if constexpr (is_indirect) {
  ...
  API_EXPORT std::conditional_t<is_indirect, std::array<size_t, Linfo::Rank>, void> tile_strides()
      const {
    if constexpr (is_indirect) {
  ...
  template <typename... ind_types>
  API_EXPORT inline storage_type const *get_raw_addr(ind_types... inds) const {
  ...
  template <typename... ind_types>
  API_EXPORT inline storage_type *get_raw_addr(ind_types... inds) {
    static_assert(is_singular || Rank == (sizeof...(ind_types)), "# of coords must match Rank");
    const std::array<SIdx, Rank> coords = {{static_cast<SIdx>(inds)...}};
    return (storage_type *)element_addr0(Rank, coords.data());
  }
  template <typename... ind_types>
  API_EXPORT inline storage_type const &get_raw(ind_types... inds) const {
    static_assert(is_singular || Rank == (sizeof...(ind_types)), "# of coords must match Rank");
    const std::array<SIdx, Rank> coords = {{static_cast<SIdx>(inds)...}};
    return *(storage_type const *)element_addr0(Rank, coords.data());
  }
  template <typename... ind_types>
  API_EXPORT inline storage_type &get_raw(ind_types... inds) {
    static_assert(is_singular || Rank == (sizeof...(ind_types)), "# of coords must match Rank");
    const std::array<SIdx, Rank> coords = {{static_cast<SIdx>(inds)...}};
    return *(storage_type *)element_addr0(Rank, coords.data());
  }
 ...
 protected:
  ...
  [[gnu::noinline]] API_EXPORT int compare_sametype_layout(LayoutTensor const *rhs) const {
    if (shape->dims != rhs->shape->dims) {
      return std::lexicographical_compare(shape->dims.begin(), shape->dims.end(),
                                          rhs->shape->dims.begin(), rhs->shape->dims.end())
                 ? -1
                 : 1;
    }
    if (is_padded) {
      if (shape->max_dims != rhs->shape->max_dims) {
        return std::lexicographical_compare(shape->max_dims.begin(), shape->max_dims.end(),
                                            rhs->shape->max_dims.begin(),
                                            rhs->shape->max_dims.end())
                   ? -1
                   : 1;
      }
  ...
  [[gnu::noinline]] API_EXPORT void allocate_layout(hnnx::Allocator &allocator, unsigned options,
                                                    MemoryClass mclass) {
    ...
    size_t const align =
        traits::is_indirect ? blocksize : std::min(size_t(256), sizeof(storage_type));
  ...
  [[gnu::noinline]] API_EXPORT void enum_memory_blocks_layout(hnnx::MemBlockEnumerator &en,
                                                              MemoryClass mclass) const {
    // get the pointer to block table; and number of entries in it.
    void **const blocktab = this->mem.get_block_list_ptr();
    size_t const nblocks = this->mem.get_block_list_len(this->shape);
  ...
  [[gnu::noinline]] API_EXPORT uint32_t find_content_hash_layout(uint32_t hash_in,
  ...
  API_EXPORT static const char *code_to_type_name() {
    return TensorTypeStruct<LayoutTensor<Linfo>>::name;
  }
};
...
class ConcreteTensor : public LayoutTensor<typename Tinfo::Lconfig> {
 protected:
  ...
  inline Interface_t const *interface_typed() const {
 ...
 public:
  ...
  //    - 'clone duplicate' of the given tensor. Note that cmode is ignored.
  API_EXPORT ConcreteTensor(const ConcreteTensor &old, hnnx::Allocator *allocator,
                            Tensor::clone_mode cmode)
  ...
  API_EXPORT inline float interface_scale_recip() const {
    return interface_typed()->get_scale_recip();
  }
  API_EXPORT inline int32_t interface_offset() const { return interface_typed()->get_offset(); }

  API_EXPORT inline ALWAYSINLINE const element_type *element_ptr(size_t rank,
                                                                 const SIdx coords[]) const {
  ...
  // block table access
  API_EXPORT inline element_type **blocktab_ptr() const {
    return (element_type **)this->mem.get_block_list_ptr();
  }
  API_EXPORT inline element_type *&blocktab_at(size_t i) {
    return (element_type *&)BaseLayout::blocktab_at(i);
  }
  ...
  template <typename... ind_types>
  API_EXPORT inline element_type *const *block_ptr_address(ind_types... inds) {
    return (element_type *const *)BaseLayout::block_ptr_address(inds...);
  };
  template <typename... ind_types>
  API_EXPORT inline element_type const *block_ptr(ind_types... inds) const {
    return *this->block_ptr_address(inds...);
  }
  template <typename... ind_types>
  API_EXPORT inline element_type *block_ptr(ind_types... inds) {
    return *this->block_ptr_address(inds...);
  }
  ...
  //
  template <typename... ind_types>
  API_EXPORT inline Const_Accessor_t operator()(ind_types... inds) const {
    static_assert(is_singular || Rank == (sizeof...(ind_types)), "# of coords must match Rank");
    const std::array<SIdx, Rank> coords = {static_cast<SIdx>(inds)...};
    return Const_Accessor_t(this->element_addr0(Rank, coords.data()), interface_typed());
  }
  template <typename... ind_types>
  API_EXPORT inline Accessor_t operator()(ind_types... inds) {
    static_assert(is_singular || Rank == (sizeof...(ind_types)), "# of coords must match Rank");
    const std::array<SIdx, Rank> coords = {{static_cast<SIdx>(inds)...}};
    return Accessor_t(this->element_addr0(Rank, coords.data()), interface_typed());
  }
  template <typename... ind_types>
  API_EXPORT inline element_type const &get_raw(ind_types... inds) const {
    static_assert(is_singular || Rank == (sizeof...(ind_types)), "# of coords must match Rank");
    const std::array<SIdx, Rank> coords = {{static_cast<SIdx>(inds)...}};
  ...
  template <typename... ind_types>
  API_EXPORT inline element_type const *get_raw_addr(ind_types... inds) const {
    static_assert(is_singular || Rank == (sizeof...(ind_types)), "# of coords must match Rank");
    const std::array<SIdx, Rank> coords = {{static_cast<SIdx>(inds)...}};
    return (element_type const *)this->element_addr0(Rank, coords.data());
  }
  ...
  // allocation and enumeration.
  API_EXPORT virtual void allocate_func(hnnx::Allocator &allocator,
                                        unsigned options) override final {
 ...
 protected:
  ...
  // requested.
  API_EXPORT virtual ALWAYSINLINE void *element_addr(
      size_t rank, const SIdx coords_in[],
  ...
  API_EXPORT virtual void **clone_util(hnnx::Allocator *allocator, std::unique_ptr<Tensor> *tensp,
    ...
    if (infop) {
      ...
      // we provide a pointer to the shape field in the cloned tensor, if applicable; otherwise in
      // 'this'.
  ...
  API_EXPORT static const char *code_to_type_name() {
    return TensorTypeStruct<ConcreteTensor<Tinfo>>::name;
  }
};
...
using layout_of = typename tensor_traits<TT>::layouttensor_type;
}  // namespace hnnx
