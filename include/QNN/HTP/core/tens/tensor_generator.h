...
API_EXPORT inline std::unique_ptr<T> make_tensor_template(Op const *op, OutputDef const &odef,
                                                          Graph &g) {
...
struct tensor_generator_table_entry {
  ...
  inline constexpr tensor_generator_table_entry(TX const *)
      : rank(tensor_traits<TX>::rank),
        dtype(tensor_traits<TX>::dtype),
        fp(make_tensor_template<T, TX>) {}
};
...
make_tengen_init(std::index_sequence<I...>) {
  return {tensor_generator_table_entry(
      static_cast<typename std::tuple_element_t<I, TTUPLE> *>(nullptr))...};
}

template <typename TensorType>
struct API_EXPORT tensor_generator_lookup {
  template <typename TX>
  using has_TensorType_as_base =
      std::integral_constant<bool, std::is_base_of<TensorType, TX>::value>;
  ...
  static std::unique_ptr<Tensor> make
      [[gnu::noinline]] (const Op *producer_in, const OutputDef &def, Graph &graph_in) {
    // concrete types get a shortcut if the dtype & rank match...
    if constexpr (!std::is_abstract<TensorType>::value) {
      if (def.dtype == tensor_traits<TensorType>::dtype &&
          def.rank == tensor_traits<TensorType>::rank) {
  ...
  static bool is_valid(const OutputDef &def) {
    if constexpr (!std::is_abstract<TensorType>::value) {
      if (def.dtype == tensor_traits<TensorType>::dtype &&
        ...
        debuglog(
            "def.dtype %u, tensor_traits<TensorType>::dtype %u, def.rank %u, "
            "tensor_traits<TensorType>::rank %u",
...
struct is_wide_crouton {
  static constexpr bool value = false;
};
template <typename T, typename = void>
struct is_tall_crouton {
  static constexpr bool value = false;
};
template <typename T>
struct is_wide_crouton<T, std::void_t<decltype(T::layout)>> {
  static constexpr bool value = (T::layout.chunk_total == 8 * 8 * 32) &&
                                (T::layout.ChunkSizes[2] > 1) &&
                                (T::layout.ChunkSizes[1] < T::layout.ChunkSizes[2]);
};
template <typename T>
struct is_tall_crouton<T, std::void_t<decltype(T::layout)>> {
  static constexpr bool value = (T::layout.chunk_total == 8 * 8 * 32) &&
                                (T::layout.ChunkSizes[1] > 1) &&
                                (T::layout.ChunkSizes[1] >= T::layout.ChunkSizes[2]);
};
...
constexpr std::unique_ptr<Tensor> (*tensor_generator)(const Op *producer_in, const OutputDef &def,
                                                      Graph &graph_in) =
    tensor_generator_lookup<TensorType>::make;
template <typename TensorType>
API_FUNC_EXPORT inline bool tensor_generator_valid(const Op *producer_in, const OutputDef &def,
                                                   Graph &graph_in) {
...
API_FUNC_EXPORT std::unique_ptr<Tensor> tensor_generator_scalar(const Op *producer_in,
                                                                const OutputDef &def,
                                                                void const *data, size_t len);

template <int relative_tolerance = 1 /* 1% */,
          int absolute_tolerance = 1 /* in 'FLT_EPSILON' ref <climits> */>
...
template <>
struct memclass_of<Tensor> {
  static constexpr MemoryClass memclass = MemoryClass::Default;
};

template <typename T>
struct memclass_of<Vector<T *>> {
  static constexpr MemoryClass memclass = memclass_of<T>::memclass;
};
template <typename T>
struct memclass_of<const Vector<T *>> {
  static constexpr MemoryClass memclass = memclass_of<T>::memclass;
};

template <MemoryClass C, typename... Ts>
struct has_memclass;

template <MemoryClass C>
struct has_memclass<C, std::tuple<>> {
  static constexpr bool value = false;
};

template <MemoryClass C, typename T, typename... Ts>
struct has_memclass<C, std::tuple<T, Ts...>> {
  static constexpr bool value =
      memclass_of<T>::memclass == C || has_memclass<C, std::tuple<Ts...>>::value;
};
...
struct map_rtfd_type {};
template <typename... T>
struct map_rtfd_type<std::tuple<T...>> {
...
template <bool FORCE>
struct SkipRegTensors {
  using type = std::tuple<>;
};

template <typename TUP, bool FORCE = false>
struct deserialize_tensor_tuple {
  template <typename T>
  using not_core_tensor = not_contains_type<typename SkipRegTensors<FORCE>::type, T>;
