...
///  - constexpr DType dtype            // always present (except Tensor,RankedTensor,LayoutTensor);
///  sometimes UNKNOWN
///  - constexpr unsigned rank          // always present (except Tensor); sometimes 0
///  - typedef element_type;            // always present (except Tensor,RankedTensor,LayoutTensor);
///  void in TensorShape
///  - typedef storage_type             // always present (except Tensor,RankedTensor); void in
///  TensorShape
///
...
///  - constexpr bool is_variable_block; // always <= is_indirect; implies that blocksize depends on
///  at least 1 dim.
///  - constexpr unsigned indirect_ranks;  // number of dims which address blocktable; < rank when
///  'is_variable_block',
///                                        // not relevant in other cases.
///  - constexpr bool is_singular;     // set when SingularLayout is used, Only when !is_chunked,
///  !is_indirect
...
using tensor_traits = typename TENST::traits;

//////////////////////////////////////////////////////////////////////////////////////////

/*
 * Now that we have Interfaces and Accessors, which we will use to give a consistent interface to
 * Tensors, let's work on the actual Tensors
 */
...
class Tensor {
 public:
  ...
  // Use with 'dims' to query dimension sizes by name e.g. auto [h, d] = tensor.dims(Tensor::HEIGHT,
  // Tensor::DEPTH)
  ...
  API_EXPORT virtual size_t dim(
      size_t index) const noexcept = 0;  // What's the length of some dimension?
  API_EXPORT virtual std::pair<size_t const *, size_t> get_dims()
      const noexcept = 0;  // return rank, and address of dims[0..n-1]

  API_EXPORT virtual size_t max_dim(size_t index) const noexcept = 0;
  API_EXPORT virtual std::pair<size_t const *, size_t> get_max_dims()
      const noexcept = 0;  // return rank, and address of dims[0..n-1]
  // this is the first virtual method defined externally.
  API_EXPORT virtual uint32_t find_content_hash()
      const noexcept;  // find 'content hash' of the data.
 ...
 protected:
  ...
  dims_extractor(std::pair<size_t const *, size_t> const dims_r, T... indices) {
    auto const read_dim = [dims_r](unsigned i) -> size_t {
      return (i < dims_r.second) ? dims_r.first[i] : 1;
    };
 ...
 public:
  ...
  ALWAYSINLINE inline std::array<size_t, 4> dims()
      const {  // make compatible with typical concrete tensor.
  ...
  API_EXPORT inline GenericAccessorRO operator()(ind_types... inds) const {
  ...
  template <typename... ind_types>
  API_EXPORT inline GenericAccessor operator()(ind_types... inds) {
    const std::array<SIdx, sizeof...(ind_types)> indarr = {static_cast<SIdx>(inds)...};
    hnnx::InterfaceRef intfc = hnnx::InterfaceRef::make_null();
    void *const ptr = element_addr(sizeof...(ind_types), indarr.data(), &intfc);
   ...
   *
   *  For Shape and Scalar tensors: Bits 31:28 contain tensMODE_shape, or tensMODE_scalar; others
   *bits ar 0. Classes which cannot be serialized return 0 in the upper 4 bits.
   */
 ...
 protected:
  ...
  static inline constexpr uint32_t formatcode_for_interface() {
  ...
  static inline constexpr uint32_t formatcode_for_general() {
  ...
  static inline constexpr uint32_t formatcode_for_shape() {
    static_assert(RANK <= tformat_rank_mask);
    return (hnnx::SerOpsInterface::tensMODE_shape << tformat_tmode_shift) |
           (RANK << tformat_rank_shift);
  }
  template <typename IFC>
  static inline constexpr uint32_t formatcode_for_scalar() {
    return (hnnx::SerOpsInterface::tensMODE_scalar << tformat_tmode_shift) |
           formatcode_for_interface<IFC>();
  }
 ...
 public:
  ...
  API_EXPORT virtual bool set_dims(
      const Tensor &prototype) = 0;  // Set the shape of the tensor same as another.
  ...
  API_EXPORT void const *raw_data_const() const noexcept {
  ...
  API_EXPORT inline hnnx::blockid_set_t get_memory_blocks(MemoryClass mc) const {
    return get_memory_blocks(int(mc));
  }
  ...
  API_EXPORT inline void enum_memory_blocks_withfunc(ENFUNC &&ef) const {
  ...
  template <typename REPLFUNC>
  API_EXPORT inline void replace_memory_blocks_withfunc(REPLFUNC &&rf) {
    hnnx::MemBlockReplBlockWrapper<std::remove_reference_t<REPLFUNC>> enumer(
        std::forward<REPLFUNC>(rf));
    this->enum_memory_blocks(enumer);
  }
  // this is passed a map<void*,void*> or any similar type with find() and end(),
  // and uses it to edit the blocks in the tensor.
  template <typename MAPTYPE>
  API_EXPORT inline void replace_memory_blocks_withmap(MAPTYPE const &map) {
  ...
  API_EXPORT std::unique_ptr<Tensor> persistent_clone(hnnx::Allocator *allocator,
                                                      bool zoneb = false) const;
  ...
  API_EXPORT std::unique_ptr<Tensor> reallocate_clone(hnnx::Allocator *allocator,
  ...
  static constexpr unsigned tile_8bit =
      1;  // set when the data is 8 bit and the tensor supports tile access
  static constexpr unsigned tile_16bit =
      2;  // set when the data is 16 bit and the tensor supports tile access
  static constexpr unsigned tile_32bit =
      4;  // set when the data is 32 bit and the tensor supports tile access
  static constexpr unsigned tile_any =
      (1 + 2 + 4);  // one of these bits is set if there is any support
  static constexpr unsigned tile_fast =
      16;  // set only when one of the above is set, and support is HVX accelerated.
  static constexpr unsigned tile_direct =
      32;  // set only when when 'fast' is set, and a direct mapping is possible
  ...
  API_EXPORT inline bool tile_support_fast() const {
    return (tile_support_bits() & tile_fast) != 0;
  }
  API_EXPORT inline bool tile_support_direct() const {
    return (tile_support_bits() & tile_direct) != 0;
  }
  // this is currently a wrapper on tile_write, which inserts the 'write_strategy' flag, and
  // suppresses broadcast and copy flags. It may change to a separate virtual func. (this is defined
  // as an inline, in tile_extract.h).
  ...
  API_EXPORT static uint32_t content_hash_data_indirect(uint32_t inhash, void **blocks,
                                                        unsigned nblocks, size_t blockbytes,
  ...
  API_EXPORT inline void get_tensor_blockinfo(tensor_blockinfo *infop) const {
    clone_util(nullptr, nullptr, infop);
  }
  ...
  inline static void deserialize_blocktable(hnnx::Deserz &dctx, T **&blockptr,
                                            unsigned const nblocks) {
    deserialize_blocktable_generic(dctx, (void ***)&blockptr, nblocks);
  }
  API_EXPORT static void deserialize_blocktable_generic(hnnx::Deserz &dctx, void ***blockp_loc,
                                                        unsigned nblocks);
...
class FakeTensor : public Tensor {
 ...
 public:
  ...
  API_EXPORT virtual std::pair<size_t const *, size_t> get_dims()
      const noexcept override;                                              //->{null,0}
  ...
  API_EXPORT virtual DTypeScaleOff get_dtype_intfc()
      const noexcept override;  // { return DTypeScaleOff(None); }
};
/**
 * @class RankedTensor
 *
 * @brief Almost as abstract as Tensor, but we template the Rank.
 * This allows us to have compile-time checking of the operator(), as well as a public Rank that is
 * static constexpr so it doesn't take a lot of space or performance... The other benefit here is
 * that we can specify RankedTensor<4> to have a fairly generic tensor, but enforce the number of
 * dimensions of the tensor.
 */

template <unsigned TRank>
class RankedTensor : public Tensor {
 public:
  struct traits {
...
template <unsigned TRank>
class TensorShape : public RankedTensor<TRank> {
 ...
 public:
  ...
  API_EXPORT virtual size_t max_dim(size_t index) const noexcept override final {
    return dim(index);
  }
  ...
  API_EXPORT virtual std::pair<size_t const *, size_t> get_max_dims() const noexcept override {
    return get_dims();
  }
  ...
  explicit TensorShape(hnnx::Deserz &dctx)
      : Parent(dctx), shape(dctx.deserialize_uint32_array_sizet<Rank>()) {}
  ...
  API_EXPORT virtual void **clone_util(hnnx::Allocator *allocator, std::unique_ptr<Tensor> *tensp,
                                       Tensor::tensor_blockinfo *infop) const override {
    if (tensp)
      *tensp = std::make_unique<TensorShape>(*this, allocator, Tensor::clone_mode::duplicate);
 ...
 protected:
  ...
  API_EXPORT static const char *code_to_type_name() {
    return TensorTypeStruct<TensorShape<TRank>>::name;
  }
};
...
template <DType DT>
class TensorSclrDT : public Tensor {
 protected:
  using T = typename dtype_traits<DT>::element_type;
  using Interface_t =
      std::conditional_t<dtype_traits<DT>::is_quant, ScaleOffsetInterface<T>, PlainInterface<T>>;
 ...
 public:
  ...
  API_EXPORT virtual size_t rank() const noexcept override {
    return 0;
  }  // What's the rank of this tensor?
  ...
  API_EXPORT virtual void allocate_func(hnnx::Allocator &allocator,
                                        unsigned options) override final {}
  API_EXPORT TensorSclrDT(const Op *producer_in, T value_in)
      : Tensor(producer_in), value(value_in) {
  ...
  template <typename... ind_types>
  API_EXPORT inline const Const_Accessor_t operator()(ind_types... inds) const {
 ...
 protected:
  ...
  API_EXPORT virtual uint32_t find_content_hash() const noexcept override {
    ...
    return mulu32_modular(h, 0x104301) ^
           content_hash_data(&this->value, sizeof(T), dtype_traits<DT>::is_float);
  }
  API_EXPORT static const char *code_to_type_name() {
    return TensorTypeStruct<TensorSclrDT<DT>>::name;
  }
};
// Tensor Scalar depending on type (assuming PlainInterface)

template <typename T>
using TensorScalar = TensorSclrDT<dtype_of_type<PlainInterface<T>>()>;
