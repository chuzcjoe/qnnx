...
class opdef_accessor {
 public:
  ...
  template <typename OEXPR_TYPE>
  static inline OpDef const &get_opdef_oexpr(ECtx &e, OEXPR_TYPE const &oexp);
  ...
  API_EXPORT static Split_Context const &lookup_split(ECtx &e,
                                                      hnnx::split_context_tag_t const &tag);
  ...
  template <typename T>
  API_EXPORT static T get_option(ECtx &e, hnnx::opname_tag_parm_t name);
...
struct opexpr_types {};
template <OpVnt V, typename T>
struct opexpr_types<opexpr<V, T>> {
...
/////////////////////////////////////////
template <>
class opexpr<OpVnt::parm, void> : public opdef_accessor {
...
template <typename OEXPR_TYPE>
inline OpDef const &opdef_accessor::get_opdef_oexpr(ECtx &e, OEXPR_TYPE const &oexp) {
...
template <typename OPA, typename EXPRB>
class opexpr<OpVnt::input_of, std::tuple<OPA, EXPRB>> : public opdef_accessor {
  const OPA m_op;
  const EXPRB m_idx;
...
auto INPUT_OF(TOP &&opa, TEXPR &&exprb) {
...
template <typename OPA, typename EXPRB>
class opexpr<OpVnt::output_of, std::tuple<OPA, EXPRB>> : public opdef_accessor {
  const OPA m_op;
  const EXPRB m_idx;
...
template <typename TOP, typename TEXPR>
auto OUTPUT_OF(TOP &&opa, TEXPR &&exprb) {
  auto wa = wrap_opexpr(std::forward<TOP>(opa));
  auto wb = wrap_param_to<int>(std::forward<TEXPR>(exprb));
...
template <typename T>
class expr<Variant::config, T> : public opdef_accessor {
...
template <>
class expr<Variant::config, bool> : public opdef_accessor {
  const hnnx::operand_tag_t m_option_name;
...
struct property_extract_rank : private opdef_accessor {
  ...
  inline otype eval(ECtx &e, hnnx::operand_tag_parm_t ptag) const {
    return get_outputdef(e, ptag).rank;
  }
...
struct property_extract_dtype : private opdef_accessor {
  typedef DType otype;
  inline otype eval(ECtx &e, hnnx::operand_tag_parm_t ptag) const {
    return get_outputdef(e, ptag).dtype;
  }
...
struct property_extract_stepsize : private opdef_accessor {
  typedef float otype;
  inline otype eval(ECtx &e, hnnx::operand_tag_parm_t ptag) const {
    return get_outputdef(e, ptag).stepsize;
  }
...
struct property_extract_n_inputs : private opdef_accessor {
  typedef size_t otype;
  inline otype eval(ECtx &e, hnnx::operand_tag_parm_t ptag) const {
    return get_opdef(e, ptag).n_inputs();
  }
...
struct dim_extractor : public property_extract_dim {
  ITYPE m_iexpr;
...
template <>
struct dim_extractor<int> : public property_extract_dim {
  int m_iexpr;
  inline size_t eval(ECtx &e, hnnx::operand_tag_parm_t ptag) const {
    int const i = m_iexpr;
    return property_extract_dim::eval(e, ptag, i);
...
template <typename OPEX, typename PROPEXT>
class expr<Variant::property, std::tuple<OPEX, PROPEXT>> {
  const OPEX m_opexpr;
  PROPEXT m_propex;
 ...
 public:
  ...
  template <typename T>
  constexpr expr(OPEX const &opexp, T &&xparm)
      : m_opexpr(opexp), m_propex(std::forward<T>(xparm)) {}
...
inline constexpr auto RANK_OF(TOP &&op) {
  auto op_wrapped = wrap_opexpr(std::forward<TOP>(op));
  return expr<Variant::property, std::tuple<decltype(op_wrapped), property_extract_rank>>(
      op_wrapped);
}

//! DTYPE_OF("operand") - extract dtype of output
template <typename TOP>
inline constexpr auto DTYPE_OF(TOP &&op) {
  auto op_wrapped = wrap_opexpr(std::forward<TOP>(op));
  return expr<Variant::property, std::tuple<decltype(op_wrapped), property_extract_dtype>>(
      op_wrapped);
}

//! ELEMENTSIZE_OF("operand") - extract dtype of output, return element size.
template <typename TOP>
inline constexpr auto ELEMENTSIZE_OF(TOP &&op) {
  auto op_wrapped = wrap_opexpr(std::forward<TOP>(op));
  return expr<Variant::property, std::tuple<decltype(op_wrapped), property_extract_elementsize>>(
      op_wrapped);
}

//! ZERO_OFFSET_OF("operand") - extract zero_offset of output quantization
template <typename TOP>
inline constexpr auto ZERO_OFFSET_OF(TOP &&op) {
  auto op_wrapped = wrap_opexpr(std::forward<TOP>(op));
  return expr<Variant::property, std::tuple<decltype(op_wrapped), property_extract_zero_offset>>(
      op_wrapped);
}

//! STEPSIZE_OF("operand") - extract step size of output quantization
template <typename TOP>
inline constexpr auto STEPSIZE_OF(TOP &&op) {
  auto op_wrapped = wrap_opexpr(std::forward<TOP>(op));
  return expr<Variant::property, std::tuple<decltype(op_wrapped), property_extract_stepsize>>(
      op_wrapped);
}

//! INPUTS_OF("operand") - get the number of inputs of an operand
template <typename TOP>
inline constexpr auto INPUTS_OF(TOP &&op) {
  auto op_wrapped = wrap_opexpr(std::forward<TOP>(op));
  return expr<Variant::property, std::tuple<decltype(op_wrapped), property_extract_n_inputs>>(
      op_wrapped);
}
...
template <typename TOP>
inline constexpr auto OUTPUTS_OF(TOP &&op) {
  auto op_wrapped = wrap_opexpr(std::forward<TOP>(op));
  return expr<Variant::property, std::tuple<decltype(op_wrapped), property_extract_n_outputs>>(
      op_wrapped);
}
//! DIM_OF("operand", idx) - get the size of an output in dimension 'idx'

template <typename TOP, typename T>
inline constexpr auto DIM_OF(TOP &&op, T &&index) {
  auto op_wrapped = wrap_opexpr(std::forward<TOP>(op));
  return expr<Variant::property,
              std::tuple<decltype(op_wrapped), dim_extractor<std::remove_reference_t<T>>>>(
...
template <typename TOP>
inline constexpr auto DATA_SIZE(TOP &&op) {
  auto op_wrapped = wrap_opexpr(std::forward<TOP>(op));
...
//
template <typename TOPA, typename TOPB>
inline auto SAME_OP(TOPA &&opa, TOPB &&opb) {
  auto opa_wrapped = wrap_opexpr(std::forward<TOPA>(opa));
  auto opb_wrapped = wrap_opexpr(std::forward<TOPB>(opb));

  return expr<Variant::opcompare,
              std::tuple<decltype(opa_wrapped), decltype(opb_wrapped), op_compare_same_op>>(
...
//
template <typename TOPA, typename TOPB>
inline auto SAME_ENCODING(TOPA &&opa, TOPB &&opb) {
  auto opa_wrapped = wrap_opexpr(std::forward<TOPA>(opa));
  auto opb_wrapped = wrap_opexpr(std::forward<TOPB>(opb));

  return expr<Variant::opcompare,
              std::tuple<decltype(opa_wrapped), decltype(opb_wrapped), op_compare_same_encoding>>(
      opa_wrapped, opb_wrapped);
}
...
//
template <typename TOPA, typename TOPB>
inline auto SAME_SHAPE(TOPA &&opa, TOPB &&opb) {
  auto opa_wrapped = wrap_opexpr(std::forward<TOPA>(opa));
  auto opb_wrapped = wrap_opexpr(std::forward<TOPB>(opb));

  return expr<Variant::opcompare,
              std::tuple<decltype(opa_wrapped), decltype(opb_wrapped), op_compare_same_shape>>(
      opa_wrapped, opb_wrapped);
}
...
struct expr<Variant::slicedim, T> : private opdef_accessor {
  ...
  expr(hnnx::split_context_tag_t const &tag, int const Split_Context::*which)
      : m_tag(tag), m_which(which) {}
};
...
inline constexpr auto make_getconst_expr(OPEXPR &&opexp, IEXPR &&iexpr) {
  return expr<Variant::getconst,
              tuple<GETCONST, std::remove_reference_t<OPEXPR>, std::remove_reference_t<IEXPR>>>(
...
template <typename OPEXPR, typename IEXPR>
constexpr auto CONSTVAL_INT(OPEXPR &&opexp, IEXPR &&idx) {
...
template <typename OPEXPR, typename IEXPR>
constexpr auto CONSTVAL_INT_VALID(OPEXPR &&opexp, IEXPR &&idx) {
  auto op_wrapped = wrap_opexpr(std::forward<OPEXPR>(opexp));
  auto wi = wrap_param(std::forward<IEXPR>(idx));
...
constexpr auto CONSTVAL_INT_VALID(OPEXPR &&opexp, IEXPR &&idx, IEXPR2 &&idx2) {
  ...
  auto wi = wrap_param(std::forward<IEXPR>(idx));
  auto wi2 = wrap_param(std::forward<IEXPR2>(idx2));
  return make_getconst_expr<getconst_int2_valid>(std::move(op_wrapped), std::move(wi),
                                                 std::move(wi2));
}

//! CONSTVAL_FLOAT("operand",idx) - extract float value from const at given index
template <typename OPEXPR, typename IEXPR>
constexpr auto CONSTVAL_FLOAT(OPEXPR &&opexp, IEXPR &&idx) {
  auto op_wrapped = wrap_opexpr(std::forward<OPEXPR>(opexp));
  auto wi = wrap_param(std::forward<IEXPR>(idx));
...
template <typename OPEXPR, typename IEXPR>
constexpr auto CONSTVAL_FLOAT_VALID(OPEXPR &&opexp, IEXPR &&idx) {
  auto op_wrapped = wrap_opexpr(std::forward<OPEXPR>(opexp));
  auto wi = wrap_param(std::forward<IEXPR>(idx));
...
template <typename CONDEXP, typename OPA, typename OPB>
class opexpr<OpVnt::select, tuple<CONDEXP, OPA, OPB>> {
  CONDEXP m_cond;
  OPA m_opa;
  OPB m_opb;
 ...
 public:
  ...
  constexpr opexpr(CONDEXP const &sel, OPA const &opa, OPB const &opb)
      : m_cond(sel), m_opa(opa), m_opb(opb) {}
};
template <typename SEL, typename A, typename B>
inline constexpr auto make_opselect(SEL s, A a, B b) {
...
inline constexpr auto SELECT(SEL &&s, A &&a, B &&b) {
  ...
  if constexpr (Replacement::is_Op_type<A>() || Replacement::is_Op_type<B>()) {
    static_assert(Replacement::is_Op_type<A>() && Replacement::is_Op_type<B>(),
                  "bad SELECT parameters");
...
template <typename MODE, typename CND>
inline auto make_message_expr(char const *str, CND &&cond) {
  return expr<Variant::message, tuple<MODE, std::remove_reference_t<CND>>>(str,
                                                                           std::forward<CND>(cond));
}
#endif

#ifndef WITH_OPT_DEBUG
// dummy functions (which are constexpr); just generate the bool
inline constexpr auto MESSAGE(char const *str) { return expr<Variant::value, bool>(true); }
template <typename COND>
inline constexpr auto MESSAGE_IF(COND a, char const *str) {
  return wrap_param_to<bool>(a);
}
template <typename COND>
inline constexpr auto MESSAGE_IFNOT(COND a, char const *str) {
  return wrap_param_to<bool>(a);
}
template <typename VALUE>
inline auto MESSAGE_VALUE(VALUE v, char const *str) {
...
inline auto MESSAGE_VALUE(VALUE v, char const *str) {
  ...
  return expr<Variant::message_value, tuple<msg_message_value, std::remove_reference_t<VALUE>>>(
      str, forward<VALUE>(wv));
}
template <typename COND>
inline auto MESSAGE_IF(COND a, char const *str) {
  auto wa = wrap_param_to<bool>(a);
  return make_message_expr<msg_message_if>(str, wa);
}
template <typename COND>
inline auto MESSAGE_IFNOT(COND a, char const *str) {
  auto wa = wrap_param_to<bool>(a);
...
inline auto apply_cst_function(ECtx &e, FUNC f, OpRef tgt, ARGPACK args,
...
struct expr<Variant::external, tuple<FUNC, INPARMS>> : private opdef_accessor {
  ...
  using otype = decltype(apply_cst_function<FUNC, INPARMS>(
      fake_ectx(), m_function, OpRef(), m_other_operands, std::make_index_sequence<NPARMS>()));
  // TODO: we should ensure here that 'otype' is one of the allowed types (and maybe
  // apply_cse_function should do minor coercions, e.g. from short to int).
...
template <typename OPEX>
struct expr<Variant::producer_for, OPEX> : private opdef_accessor {
...
template <typename OPEX>
auto PRODUCER_FOR(OPEX &&producer, char const *consumer_opname) {
...
template <typename OPEX>
struct expr<Variant::eq_opstr, OPEX> : private opdef_accessor {
  std::string m_opname;
  const OPEX m_opexpr;
  inline bool eval(ECtx &e) const {
...
template <typename OPEX>
auto IS_OP(OPEX &&op, char const *opname) {
...
/// @brief ITER_INPUT_OF( <some_op>, "split") - extract the input from <someop> selected by
/// ITER_VAR(split)
///
template <typename OPER>
inline auto ITER_INPUT_OF(OPER &&oper, hnnx::split_context_tag_t whatsplit) {
