...
static inline bool log_condition(const int prio) { return (prio <= GetLogPriorityLevel()); };
...
constexpr static bool log_condition(const int prio) { return false; };
...
PUSH_VISIBILITY(default)
API_FUNC_EXPORT API_C_FUNC void API_FUNC_NAME(SetLogCallback)(DspLogCallbackFunc cbFn,
                                                              int logPriority);
...
#define MAKE_LOG_FMT_WITH_PREFIX(FMT, ...) \
  FILE_BASENAME ":" STRINGIZE(__LINE__) ":" FMT "\n", ##__VA_ARGS__
...
inline void logmsgraw(const int prio, char const *fmt, Types... args) {
...
#define _errlog_(FMT, ...)                                                          \
  HV3_LOG(NN_LOG_ERRORLVL, MAKE_LOG_FMT_WITH_PREFIX(":ERROR:" FMT, ##__VA_ARGS__)), \
...
#define _statlog_(statname, statvalue, dummy) \
  logmsgraw(NN_LOG_STATLVL,                   \
            MAKE_LOG_FMT_WITH_PREFIX("STAT: %s=%lld", statname, (long long)statvalue))
#define _i_statlog_(statname, statvalue, dummy) \
  logmsgraw(NN_LOG_STATLVL_INTERNAL,            \
            MAKE_LOG_FMT_WITH_PREFIX("STAT: %s=%lld", statname, (long long)statvalue))
...
template <class T>
constexpr const char *format_type_check = "";
...
#define errlogf(...) \
  logmsg_fmt(NN_LOG_ERRORLVL, "", MAKE_LOG_FMT_WITH_PREFIX(":ERROR:" FMT, ##__VA_ARGS__))
#define warnlogf(...) \
  logmsg_fmt(NN_LOG_WARNLVL, "", MAKE_LOG_FMT_WITH_PREFIX(":WARNING:" FMT, ##__VA_ARGS__))
#define infologf(FMT, ...) logmsg_fmt(NN_LOG_INFOLVL, MAKE_LOG_FMT_WITH_PREFIX(FMT, ##__VA_ARGS__))
#define verboselogf(FMT, ...) \
  logmsg_fmt(NN_LOG_VERBOSELVL, MAKE_LOG_FMT_WITH_PREFIX(FMT, ##__VA_ARGS__))
#define debuglogf(FMT, ...) \
  logmsg_fmt(NN_LOG_DEBUGLVL, "", MAKE_LOG_FMT_WITH_PREFIX(FMT, ##__VA_ARGS__))
#endif  // 0
