...
struct OptimFlags {
  ...
  enum f : flags_t {
    ...
    fold_relu_flag = flagbit<7>::val,   // if overall flag for folding relu is enabled all rules
                                        // containing this flag will be triggered
    cp_after_always = flagbit<8>::val,  // always do CP after a pass containing this rule
    hmx_short_conv_flag =
        flagbit<9>::val,  // if overall flag for using hmx to do short conv is enabled all rules
                          // containing this flag will be triggered
    relaxed_precision_flag = flagbit<10>::val,  // if overall flag for relaxed precision is enabled
                                                // all rules containing this flag will be triggered
...
AND(
  ...
  AND(
    ...
    NOT( AND( EQ(DIM_HEIGHT("Stride"),4), EQ(DIM_WIDTH("Stride"),4), EQ( DIM_FILTHEIGHT("Weights"),
3), EQ( DIM_FILTWIDTH("Weights"), 3)))
  )
),
...
*/
  ...
  // we clear it whenever we decide to do CSE, and we set it in any other case when any rule has
  // been executed. So, if it's clear when we are called, and the success_flags don't have it,
  // nothing has changed since the previous CSE, and we can ignore all the other CSE conditions.
  ...
  class OptFlagState {
   ...
   public:
    ...
    API_EXPORT inline void update(
        flags_t previous_pass_flags,  // GraphOptPass.flags from the previous pass
        ...
        flags_t success_flags)        // 'or' of all rules which succeeded in previous pass.
    {
      ...
      flags_t next_trigs =
          trigs | (success_flags & (any_rule | cse_set_triggerA | cse_set_triggerB));
      bool const do_cse =
          ((next_trigs & any_rule) != 0 &&
           ((success_flags & cse_after_if) != 0 || (previous_pass_flags & cse_after_always) != 0 ||
            ((next_trigs & cse_set_triggerA) != 0 &&
             (next_pass_flags & cse_before_if_triggerA) != 0) ||
            ((next_trigs & cse_set_triggerB) != 0 &&
             (next_pass_flags & cse_before_if_triggerB) != 0)));
      bool const do_cp =
          ((next_trigs & any_rule) != 0 &&
  ...
  //  inline constexpr OptimFlags::flag_eval<SOMETYPE>() {  return any_rule | (flagname1|
  //  flagname2); }
  // .. and then call that to get the value.
  //

  template <typename U>
  static constexpr flags_t flag_evaluate() noexcept {
