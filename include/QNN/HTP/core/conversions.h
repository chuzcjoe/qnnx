...
template <typename TI>
struct intrange_within_float<float, TI> {
  static_assert(std::numeric_limits<TI>::is_integer);
...
template <typename TI>
struct intrange_within_float<double, TI> {
  static_assert(std::numeric_limits<TI>::is_integer);
...
struct satcast_helper {
  static_assert(std::numeric_limits<TOUT>::is_specialized &&
                std::numeric_limits<TIN>::is_specialized);
 ...
 */
template <typename TOUT, typename TIN>
inline constexpr TOUT saturate_cast(TIN val) {
...
inline TOUT saturate_round(float val) {
...
struct proper_cmp_helper {
  ...
  static const bool CMP_AS_INT =
      (sizeof(TA) < sizeof(int) || (sizeof(TA) == sizeof(int) && ASIGNED)) &&
...
template <typename TA, typename TB>
inline bool constexpr proper_eq(TA a, TB b) {
 ...
 */
template <typename TA, typename TB>
inline bool constexpr proper_ne(TA a, TB b) {
  return !hnnx::prpercmp::proper_cmp_helper<TA, TB>::eq(a, b);
}
/**
 * @brief 'proper' compare of any two integer types, respecting signedness and actual numeric value
 ...
 */
template <typename TA, typename TB>
inline bool constexpr proper_lt(TA a, TB b) {
 ...
 */
template <typename TA, typename TB>
inline bool constexpr proper_ge(TA a, TB b) {
  return !hnnx::prpercmp::proper_cmp_helper<TA, TB>::lt(a, b);
}
/**
 * @brief 'proper' compare of any two integer types, respecting signedness and actual numeric value
 ...
 */
template <typename TA, typename TB>
inline bool constexpr proper_gt(TA a, TB b) {
 ...
 */
template <typename TA, typename TB>
inline bool constexpr proper_le(TA a, TB b) {
  return !hnnx::prpercmp::proper_cmp_helper<TB, TA>::lt(b, a);
}
/**
...
inline bool constexpr proper_inrange(TA x, TB lo, TC limit) {
 ...
 */
template <typename TA, typename TB, typename TC>
inline bool constexpr proper_inrange_closed(TA x, TB lo, TC hi) {
...
inline unsigned constexpr addu32_modular(unsigned a, unsigned b) { return a + b; }
/**
 ...
 */
ATTR_NO_SANITIZE("unsigned-integer-overflow")
inline unsigned constexpr subu32_modular(unsigned a, unsigned b) { return a - b; }
/**
 * @brief multiply unsigned*unsigned->unsigned, escaping 'unsigned overflow' checks
 */
ATTR_NO_SANITIZE("unsigned-integer-overflow")
inline unsigned constexpr mulu32_modular(unsigned a, unsigned b) { return a * b; }
/**
 ...
 */
ATTR_NO_SANITIZE("unsigned-integer-overflow")
inline uint64_t constexpr addu64_modular(uint64_t a, uint64_t b) { return a + b; }

/**
 * @brief subtract u64-u64->u64, escaping 'unsigned overflow' checks
 */
ATTR_NO_SANITIZE("unsigned-integer-overflow")
inline uint64_t constexpr subu64_modular(uint64_t a, uint64_t b) { return a - b; }
/**
 * @brief mul u64*u64->u64, escaping 'unsigned overflow' checks
 */
ATTR_NO_SANITIZE("unsigned-integer-overflow")
inline uint64_t constexpr mulu64_modular(uint64_t a, uint64_t b) { return a * b; }
...
template <typename TOUT, typename TIN>
inline constexpr TOUT image_convert(TIN x) {
...
inline constexpr size_t round_up(size_t a, TD b) {
...
template <typename TD>
inline constexpr size_t round_up(int a, TD b) {
  static_assert(std::is_integral_v<TD>, "round_up can only apply to integer types");
  int const bi = b;
