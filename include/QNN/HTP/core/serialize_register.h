...
template <>
struct dealloc_func_for_op<int> {
  static void func(Graph *graph_in, void *ptr) {}
};
...
inline void register_framework_op(char const *opname) {
  using alloc_func = alloc_func_for_op<OPTYPE>;
  register_op_info(typeid(OPTYPE), hnnx::cost_function_t(StandardCosts::FAST), 0, nullptr, false,
                   opname);
  op_deserializer_fn const fn(alloc_func::alloc_func, get_dealloc_func_for_op<OPTYPE>(),
                              alloc_func::op_size_align);
