...
namespace hnnx {
inline HVX_Vector q6op_V_lo_W(HVX_VectorPair const &w) { return w.v[0]; }
inline HVX_Vector q6op_V_hi_W(HVX_VectorPair const &w) { return w.v[1]; }
...
HVX_Vector do_Vw_vaddORsub_VwVwQ_carry(HVX_Vector const &vu, HVX_Vector const &vv,
                                       HVX_VectorPred *qcarry) {
...
#pragma pack(pop)
inline HVX_Vector vmemu(void const *addr) { return ((unaligned_vector_wrapper const *)addr)->v; }
inline unaligned_vector_wrapper &vmemu(void *addr) { return *(unaligned_vector_wrapper *)addr; }
...
// The bytes are extracted from value, starting at position 'pos0' (and wrapping around, if pos0+n >
// 128). Only the 7 lsbs of pos0 are used.
...
inline HVX_Vector &vmemu(void *addr) { return *(HVX_Vector *)addr; }

inline HVX_Vector vmemu(void const *v) { return *(HVX_Vector const *)(v); }
inline HVX_Vector q6op_V_vldu_A(void const *addr) { return *(HVX_Vector const *)addr; }
inline void q6op_vstu_AV(void *addr, HVX_Vector v) { *(HVX_Vector *)addr = v; }
...
inline void hvx_store_vec_x2_unaligned_inline(void *addr, HVX_Vector v0, HVX_Vector v1,
                                              int bytes) noexcept {
  check_hvx();
...
void hvx_store_vec_x4_unaligned(void *addr, HVX_Vector v0, HVX_Vector v1, HVX_Vector v2,
                                HVX_Vector v3, int bytes) noexcept;

inline void hvx_store_vec_x4_unaligned_inline(void *addr, HVX_Vector v0, HVX_Vector v1,
                                              HVX_Vector v2, HVX_Vector v3, int bytes) noexcept {
  check_hvx();
...
inline HVX_Vector uint64_to_qfloat(HVX_Vector ll_hi, HVX_Vector ll_lo) {
  ...
  HVX_Vector maskZero =
      Q6_V_vand_QV(Q6_Q_vcmp_eq_VwVw(ll_lo, vzero), Q6_Q_vcmp_eq_VwVw(ll_hi, vzero));
...
inline HVX_Vector uint64_to_float(HVX_Vector ll_hi, HVX_Vector ll_lo) {
  ...
  exponent = Q6_Vw_vasl_VwVw(
      exponent, Q6_V_vsplat_R(23));  // align the exponent part of the floating-point representation

  HVX_Vector result = Q6_Vw_vadd_VwVw(exponent, mantissa);
  // handling float conversion for 0 manually
  const HVX_Vector maskZero =
      Q6_V_vand_QV(Q6_Q_vcmp_eq_VwVw(ll_lo, vzero), Q6_Q_vcmp_eq_VwVw(ll_hi, vzero));
...
template <bool RND>
static inline HVX_Vector convert_sf_to_s32_core(HVX_Vector vals) {
  if constexpr (RND) {
...
template <bool RND>
static inline HVX_Vector convert_hf_to_s16_core(HVX_Vector vals) {
#if HEX_ARCH >= 73
  if constexpr (RND) {
