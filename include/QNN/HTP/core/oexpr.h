...
class OptFunction<R(Args...)> {
 public:
  ...
  template <typename L>
  API_EXPORT static R LambdaWrapper(void *t, Args... args) {
    L *const obj = (L *)t;
  ...
  template <typename L>
  API_EXPORT static typename std::enable_if<!std::is_lvalue_reference_v<L>, thisType>::type create(
      L &&lambda) {
...
enum class Variant : int {
  ...
  property,   // <property, tuple<OPEXP, prop_extractor_class>>  - extracts a property or dim of
              // OpDef
...
//
template <typename T>
using sFunction = OptFunction<T(ECtx &)>;

template <Variant V, typename ARG>
class expr {};
...
//
template <Variant V, typename T>
inline auto fake_eval(expr<V, T> const &a) {
...
template <>
struct wrapper_helper<unsigned> {
  static constexpr auto wrap(unsigned x) { return expr<Variant::value, size_t>(size_t(x)); }
};
template <>
struct wrapper_helper<unsigned long> {
  static constexpr auto wrap(unsigned long x) { return expr<Variant::value, size_t>(size_t(x)); }
};
template <>
struct wrapper_helper<unsigned long long> {
  static constexpr auto wrap(unsigned long long x) {
    return expr<Variant::value, size_t>(size_t(x));
  }
};

// a variant is unchanged when wrapped.
template <Variant V, typename T>
struct wrapper_helper<expr<V, T>> {
  static constexpr auto wrap(expr<V, T> const &x) { return x; }
};

template <typename T>
inline constexpr auto wrap_param(T &&p) {
...
inline constexpr auto wrap_param_to(T &&p) {
...
// a unary operator
//
template <typename A, typename OPER>
class expr<Variant::unop, tuple<A, OPER>> {
  const A m_a;
  const OPER m_op;
...
template <typename A, typename OPER>
inline constexpr auto make_unop(A a, OPER op) {
  return expr<Variant::unop, tuple<A, OPER>>(a, op);
}
// specialize for case where input is value (constant folding)
template <typename TA, typename OPER>
inline constexpr auto make_unop(expr<Variant::value, TA> a, OPER op) {
  return wrap_param(op(a.getval()));
}

// a binary operator
//
template <typename A, typename B, typename OPER>
class expr<Variant::binop, tuple<A, B, OPER>> {
  const A m_a;
  const B m_b;
  const OPER m_op;
...
template <typename A, typename B, typename OPER>
inline constexpr auto make_binop(A a, B b, OPER op) {
...
class expr<Variant::binop1, tuple<A, TB, OPER>> {
  const A m_a;   // an oExp
...
class expr<Variant::lg_or, OPERANDS> {
 ...
 public:
  ...
  constexpr otype eval(ECtx &e) const {
    return reduce_logop<Variant::lg_or, 0, OPERANDS>(e, m_operands);
  }
};
template <typename OPERANDS>  // operands is tuple<expr<> ...>
class expr<Variant::lg_and, OPERANDS> {
 ...
 public:
  ...
  inline constexpr expr(OPERANDS const &op) : m_operands(op) {}
  constexpr otype eval(ECtx &e) const {
    return reduce_logop<Variant::lg_and, 0, OPERANDS>(e, m_operands);
  }
};
template <typename OPERANDS>  // operands is tuple<expr<> ...>
class expr<Variant::lg_xor, OPERANDS> {
 ...
 public:
  ...
  inline constexpr expr(OPERANDS const &op) : m_operands(op) {}
  constexpr otype eval(ECtx &e) const {
    return reduce_logop<Variant::lg_xor, 0, OPERANDS>(e, m_operands);
  }
};
template <Variant WHICH, typename OPERANDS>
inline constexpr auto make_logop(OPERANDS &&opers) {
  return expr<WHICH, std::remove_reference_t<OPERANDS>>(std::forward<OPERANDS>(opers));
}
// select
template <typename SEL, typename A, typename B>
class expr<Variant::select, tuple<SEL, A, B>> {
  const SEL m_sel;
  const A m_a;
  const B m_b;
...
template <typename SEL, typename A, typename B>
inline constexpr auto make_select(SEL s, A a, B b) {
...
#define OEXP_ARITH(FNAME, CLST)                                                                    \
  ...
  inline constexpr auto FNAME(A &&pa, B &&pb) {                                                    \
...
#define OEXP_UNARYMATH(FNAME, CLSNAME)         \
  template <typename A>                        \
  inline constexpr auto FNAME(A &&pa) {        \
    auto wa = wrap_param(std::forward<A>(pa)); \
    using atype = decltype(fake_eval(wa));     \
...
template <typename TTO, typename SOMEEXP>
inline constexpr auto make_cast_oper(SOMEEXP &&exp) {
...
static inline auto constexpr INT(TA &&a) {
...
static inline auto constexpr INT(expr<Variant::value, TI> const &val) {
  return expr<Variant::value, int>(int(val.getval()));
}

//! UINT(x) - convert to unsigned (i.e. size_t)
template <typename TA>
static inline auto constexpr UINT(TA &&a) {
  auto wa = wrap_param(std::forward<TA>(a));
...
template <typename TI>
static inline auto constexpr UINT(expr<Variant::value, TI> const &val) {
  return expr<Variant::value, size_t>(size_t(val.getval()));
}

//! DTYPE(x) - convert to DType
template <typename TA>
static inline auto constexpr DTYPE(TA &&a) {
  auto wa = wrap_param(std::forward<TA>(a));
...
template <typename TI>
static inline auto constexpr DTYPE(expr<Variant::value, TI> const &val) {
  return expr<Variant::value, DType>(DType(val.getval()));
}

//! FLOAT(x) - convert to float
template <typename TA>
static inline auto constexpr FLOAT(TA &&a) {
  auto wa = wrap_param(std::forward<TA>(a));
...
template <typename TI>
static inline auto constexpr FLOAT(expr<Variant::value, TI> const &val) {
...
inline constexpr auto OR(TA &&a, TB &&b, Ts &&...ts) {
  auto parms = std::make_tuple(wrap_param_to<bool>(std::forward<TA>(a)),
                               wrap_param_to<bool>(std::forward<TB>(b)),
...
template <typename TA, typename TB, typename... Ts>
inline constexpr auto AND(TA &&a, TB &&b, Ts &&...ts) {
  auto parms = std::make_tuple(wrap_param_to<bool>(std::forward<TA>(a)),
                               wrap_param_to<bool>(std::forward<TB>(b)),
                               wrap_param_to<bool>(std::forward<Ts>(ts))...);
...
//! AND(a,b, ...) - logical AND; evaluation stops after first 'false' operand
template <typename TA>
inline constexpr auto AND(TA &&a) {
  return AND(std::forward<TA>(a), true);
}

//! XOR(a,b, ...) - logical XOR
template <typename TA, typename TB, typename... Ts>
inline constexpr auto XOR(TA &&a, TB &&b, Ts &&...ts) {
  auto parms = std::make_tuple(wrap_param_to<bool>(std::forward<TA>(a)),
                               wrap_param_to<bool>(std::forward<TB>(b)),
                               wrap_param_to<bool>(std::forward<Ts>(ts))...);
...
template <typename SEL, typename A, typename B>
constexpr auto SELECT(SEL &&s, A &&a, B &&b);
...
sFunction<SCALART> make_literal_sfunction(SCALART val) {
...
inline constexpr bool sfunc_bool_false(ECtx &) { return false; }
inline constexpr bool sfunc_bool_true(ECtx &) { return true; }

template <>
inline sFunction<bool> make_literal_sfunction<bool>(bool val) {
  return sFunction<bool>(sFunction<bool>::FunctionWrapper,
                         (void *)(val ? sfunc_bool_true : sfunc_bool_false));
}
...
sFunction<T> wrap_as_function(EX &&a) {
