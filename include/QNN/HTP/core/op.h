...
/*
 * FIXME: instead of deserialize function, we should have a constructor with arguments (const char
 * **bufp, size_t *)
 */
...
class Op : public hnnx::Executable {
  ...
  //! Interface to the external world is 32 bits for an Op ID (0 and above 0xF000_0000 are reserved
  //! for internal use). However, as we break ops down we want to have some semblance of the
  //! original op IDs while still maintaining unique IDs. So we make internal OpIDs 64 bits. Half of
  //! them can be the external OpID, and we can use a counter or something to uniquify in the other
  //! bits. We can accumulate performance information and such to still represent OpIDs on the
  //! interface.
 public:
  Op(){};
  ...
  API_EXPORT bool is_const() const
  ...
  virtual const char *get_docs() const {
    return hnnx::docs_for<Op>();
  }  // LCOV_EXCL_LINE [SAFTYSWCCB-1542]
  ...
  API_EXPORT inline void enumerate_input_blocks(hnnx::MemBlockEnumerator &en) const {
    enumerate_blocks(en, true);
  }
  API_EXPORT inline void enumerate_output_blocks(hnnx::MemBlockEnumerator &en) const {
  ...
  API_EXPORT inline void enumerate_blocks_withfunc(ENFUNC &&ef, bool is_input) const {
    hnnx::MemBlockEnumWrapper<std::remove_reference_t<ENFUNC>> enumer(std::forward<ENFUNC>(ef));
    this->enumerate_blocks(enumer, is_input);
  }
  template <typename ENFUNC>
  API_EXPORT inline void enumerate_input_blocks_withfunc(ENFUNC &&ef) const {
    enumerate_blocks_withfunc(std::forward<ENFUNC>(ef), true);
  }
  template <typename ENFUNC>
  API_EXPORT inline void enumerate_output_blocks_withfunc(ENFUNC &&ef) const {
  ...
  //
  // Caveats:
  //  - ALWAYS CHECK FOR NULL RETURN VALUE. There is no errlog if the clone fails, just a null
  //  return.
  //  - Not all Op can be cloned in this way; it applies only to Ops which can be created from
  //  OpDef.generate().
  //    So, no things like SpillOp or ValidateOp.
  //  - The Op's 'constructor hooks' are only called if 'opclone_realloc' mode is specified (or
  //  selected via opclone_auto)
  //  - 'prepare' is called with tcm_available = true; it is assumed that if new Op needs that, the
  //  original
  //    op needed it too.
  //  - You can pass an alternate Op type ('clone X but as Y'... ); use extreme caution, will only
  //  work if the
  ...
  /// succeeds (and returns true) if val is not empty, 'which' is in range and the Op doesn't
  /// already have that
 ...
 protected:
  ...
  API_EXPORT void enumerate_op_input_blocks(hnnx::MemBlockEnumerator &en,
                                            Tensor const *const *inputs_p, unsigned n) const;
  API_EXPORT void enumerate_op_output_blocks(hnnx::MemBlockEnumerator &en,
                                             hnnx::uptr_Tensor const *outputs_p, unsigned n) const;
  template <typename VIN, typename VOUT>
  [[gnu::always_inline]] inline void enumerate_op_blocks(hnnx::MemBlockEnumerator &en,
                                                         VIN const &vinputs, VOUT const &voutputs,
                                                         bool is_input) const {
    if (is_input) {
...
constexpr hnnx::cost_function_t get_costf() {
...
class ConstWrapperOp : public Op {
 ...
 public:
  ...
  API_EXPORT ConstWrapperOp(Graph &graph_in, OpId my_id_in, const OutputDef &def,
                            void const *data_in);
  ...
  // LCOV_EXCL_STOP
  API_EXPORT virtual std::pair<size_t, size_t> num_inputs_outputs() const override {
    return {0, 1};
  }
...
// so that internal Ops (e.g. PreloadOp) can be based on this and not need to define any they don't
// need
//
class MetaOpBase : public Op {
 public:
  ...
  API_EXPORT virtual GraphStatus prepare(
      hnnx::OpIoPtrs const &,
      bool tcm_available) override;  //{ return GraphStatus::Success;}
  API_EXPORT virtual GraphStatus allocate(
      Graph &graph_in) override;  // { return GraphStatus::Success;}

  API_EXPORT virtual bool is_valid() const noexcept override;  // {return false;}
  API_EXPORT virtual std::pair<size_t, size_t> num_inputs_outputs()
      const override;  //{ return {0,0};}
  API_EXPORT virtual Tensor const *get_input_output(
      size_t which,
      bool is_input) const override;                                          // {return nullptr;}
  API_EXPORT virtual void serialize(hnnx::SerOpsInterface &) const override;  // {}
  API_EXPORT virtual uptr_Op clone_meta(Graph &graph_in,
                                        OpId new_opid) const;  // {return uptr_Op(nullptr);}
};
...
class SpecialPrepOpBase : public MetaOpBase {
 public:
  SpecialPrepOpBase(){};
  SpecialPrepOpBase(Graph &graph_in, unsigned long long int my_id_in)
      : MetaOpBase(graph_in, my_id_in) {}
  ...
  API_EXPORT virtual bool get_opdef_name(OpId opid,
                                         opname_tag_t &result) const;  // {return false} in op.cc
  ...
  // make a CostBasedFeatureDesc. If 'false' is returned, it should be obtained 'in the usual
  // manner'.
  API_EXPORT virtual bool get_costbased_feature(OpId opid, CostBasedFeatureDesc &result) const {
    return false;
  }
};
// LCOV_EXCL_STOP
...
// All methods must be const, and return GraphStatus; the 'default' methods do nothing and return
// GraphNotApplicable. So, we can allow two or more hooks to be attached to an Op; when calling a
// method, we will call it on the first one, and if it returns NotApplicable, we will try the next
