...
constexpr inline unsigned htp_header_get_MAGIC(void const *const p) { return *(unsigned const *)p; }

inline bool htp_is_barrel(void const *const p) {
  return htp_header_get_MAGIC(p) == Hdr_MAGIC_MULTI;
}

inline bool htp_is_pickle(void const *const p) { return htp_header_get_MAGIC(p) == Hdr_MAGIC; }
...
//
inline int htp_header_locate_field(const void *hdr, const size_t buflen, const unsigned tag,
