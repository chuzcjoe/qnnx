...
class IterableTensor {
 ...
 public:
  ...
  API_EXPORT inline auto operator()(size_t b, size_t h, size_t w, size_t d) {
    return access(b, h, w, d);
  }
  ...
  API_EXPORT inline const_iterator begin(std::array<size_t, 4> start) const {
    return const_iterator(*this, start);
  }
  ...
  API_EXPORT inline const_iterator end(std::array<size_t, 4> end) const {
    return const_iterator(*this, end);
  }
...
class TensorIter {
 ...
 public:
  API_EXPORT inline TensorIter(IterableTensor<T> &it, std::array<size_t, 4> loc)
      : myITensor(it), location(loc) {}
  ...
  API_EXPORT inline operator float() const {
    return myITensor(location[0], location[1], location[2], location[3]);
  }
...
class TensorCIter {
 ...
 public:
  API_EXPORT inline TensorCIter(const IterableTensor<T> &it, std::array<size_t, 4> loc)
      : myITensor(it), location(loc) {}
