...
API_EXPORT OpFactory make_op_custom_internal(const std::string_view op_name_in,
                                             const std::string_view type_tag,
                                             op_reg_parms const &opreg_parms,
                                             bool is_external = false,
                                             const std::string_view file_name = "");

API_EXPORT OpFactory make_op_custom(const std::string_view op_name_in,
                                    std::string_view const type_tag,
                                    op_reg_parms const &opreg_parmsm,
...
class GetParms {
 public:
  ...
  constexpr static typename item_return::type get() {
    return op_reg_parms::parms_for<Derived, FlagCounter<Derived, I>::get()>();
  }

  template <auto FP, int I>
  constexpr static typename item_return::type get() {
...
#define MDT(W, LINE)                                                        \
  namespace fold {                                                          \
  template <>                                                               \
  struct ModifiedDerivedType<&W, LINE> : public ModifiedDerivedTypeParent { \
...
#define TYPE_SUFFIX(OP, NMVRT, ARGS)                                                               \
  (hnnx::ConcatStr<hnnx::ConstexprStrLen(OP),                                                      \
                   hnnx::ConstexprStrLen(NMVRT) + hnnx::ConstexprStrLen(ARGS)>(                    \
      OP, (hnnx::ConcatStr<hnnx::ConstexprStrLen(NMVRT), hnnx::ConstexprStrLen(ARGS)>(NMVRT, ARGS) \
               .data())))
...
#define DEF_NATIVE_OP_NMVRT(F, W, OP, NMVRT, LINE)  \
  MDT(F, LINE)                                      \
  APPEND_REG_OP_ELEM(&W, THIS_PKG_NAME_STR "::" OP, \
                     TYPE_SUFFIX(OP, NMVRT, hnnx::ArgsTuples2<&F>::inputTypeNames), LINE)

#define DEF_NATIVE_OP_NMVRT_NO_TCM_FOLDING(F, W, OP, NMVRT)                                       \
  APPEND_REG_OP_ELEM_NO_TCM_FOLDING(W, THIS_PKG_NAME_STR "::" OP,                                 \
                                    TYPE_SUFFIX(OP, NMVRT, hnnx::ArgsTuples2<F>::inputTypeNames), \
                                    false)
...
#define DEF_NATIVE_OP_NMVRT_NO_TCM_FOLDING(F, W, OP, NMVRT) \
  __reg_op__(F, OP)<<<__FILE__, __LINE__>>>
#endif
