...
class NoPadding {
 public:
  ...
  inline constexpr std::array<Idx, Rank> pad_coords(
      const std::array<Idx, Rank> &coords, const std::array<PadT, Rank> &left_padding) const {
    std::array<Idx, Rank> ret{coords};
    return ret;
  }
  template <typename PadT>
  inline constexpr std::array<Idx, Rank> pad_coords(
      const std::array<SIdx, Rank> &coords, const std::array<PadT, Rank> &left_padding) const {
...
class Padding {
 public:
  ...
  inline const std::array<Idx, Rank> pad_coords(const std::array<Idx, Rank> &coords,
    ...
    std::array<Idx, Rank> ret{};
    std::transform(left_padding.begin(), left_padding.end(), coords.begin(), ret.begin(),
                   std::plus<Idx>());
    return ret;
  }
  template <typename PadT>
