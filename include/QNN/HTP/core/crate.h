...
///
// Each record containing an object has a non-null 'dtor' field; if the object is trivially
// destructible, this will be  (dtor_funcp)1, and the object is not on the linked-list.
//
// note:
...
//  the erase calls are ignored since the other objects are going to get dtor'd anyhow (if they have
//  not been already).
// Important: if object A's constructor places B into the crate, then B will very likely get
// destroyed
//  first when the crate is cleared. Thus, A's destructor can't look at B (it can erase B, which is
...
//         - if there is no current chunk, or if the current chunk doesn't have room for
//         'size_needed' bytes,
//           a new chunk is added which does.
//         - enable_raw_mode(size_needed) returns a chunk handle.
//
// Internally, raw_mode causes add_record_slot() to do the same thing, but it only moves
// alloc_count, it does not assign a slot index, and 'idx' is -1 in the returned struct. All callers
// of add_record_slot() *must* check for raw mode (can be done by checking idx < 0), and then avoid
...
template <typename T>
using clear_t = decltype(std::declval<T &>().clear(std::declval<Graph &>()));

template <typename T>
constexpr bool has_clear = is_detected_v<clear_t, T>;
...
class Crate {
  ...
  struct index_rec {
    ...
    dtor_funcp dtor;  ///< pointer to dtor function (null if empty record; (DTOR_TRIVIAL if the
                      ///< object is trivial dtor)
  };
  ...
  struct chunkhdr;
  API_EXPORT static chunkhdr *hdr_of(uptr_chunk_t &p) {
 ...
 public:
  class ChunkHandle {
   ...
   public:
    ...
    friend inline bool operator==(ChunkHandle const &a, ChunkHandle const &b) {
      return a.chunkp == b.chunkp;
    }
  ...
  ChunkHandle first_chunk_handle() const {
    return ChunkHandle(m_chunks.empty() ? nullptr
                                        : hdr_of(const_cast<Crate &>(*this).m_chunks.front()));
  }
  ...
  API_HIDDEN T *emplace(Args &&...args) {
  ...
  API_EXPORT void *emplace_explicit(Deserz &dctx, deserialize_op_func init_func,
                                    deserialize_dtor_func dtor_func, size_align_code_t size_al);

  //! Allocate 'n' of type T in the crate.
  /// Will initially be garbage; T must be trivially destructable (unless waived)
  template <typename T, bool DTOR_OK = false>
...
/*
 * EJP: This seems silly, but I don't know how to get visibility into Graph into a templated Tensor
 * because of include hell.
 */
