...
class Deserz : public DeSerError {
 ...
 public:
  ...
  template <typename T>
  void deserialize_tensor_ref(T const *&where);
  template <typename T>
 ...
 protected:
  ...
  API_EXPORT std::pair<void const *, void const **> deserialize_shared_obj_func(
      void const **ptrloc);
 ...
 public:
   ...
   */
  template <size_t N>
  std::array<size_t, N> deserialize_uint32_array_sizet() {
  ...
  inline bool fixup_encode_for_blocktable(uint32_t const idx, uint32_t const table_offs,
                                          void **const ptrloc) {
...
class Deserializer : public Deserz {
 ...
 public:
  ...
  template <typename T>
  inline void deserialize_tensor_ref(T const *&where) {
  ...
  inline void need_tensor_fixup(object_identity_type oid, Tensor const **where) {
  ...
  constexpr bool is_shared_dynamic_tensor_shape_format() const {
    return shared_dynamic_tensor_shape;
  }
  void set_shared_dynamic_tensor_shape_format(const bool v = true) {
    shared_dynamic_tensor_shape = v;
  }
  ...
  // valid when the entire pickle, in const_extent format, is loaded as a single, persistent dma
  // buffer
  inline unsigned char *get_weight_pointer() {
    return ((unsigned char *)bufstart) + (4 * pickle_len_words);
  };
  POP_WARNING()
  ...
  inline tensor_deserialize_fn_list_t &get_tensor_deserialize_fn_list() {
    return tensor_deserialize_fn_list;
  }

  // Next 4 methods are used to support 'deserialize_by_segments'.
  // 'get_forward_span' returns a 'deser_segment_span' (pair of pointers) for a region of
  // deserialized data from 'ref + start' up to 'ref + end', where start and end (0 <= start < end)
  // are byte offsets relative to some position 'ref' in the deserialized data, and 'ref' is the
  // value which bytes_consumed() returned at that reference point. All should be multiples of 4.
 ...
 protected:
  ...
  ///
  typedef std::unique_ptr<const uint32_t[], conditional_default_deleter<const uint32_t[]>>
      unique_readonly_blob_t;
 ...
 public:
  ...
  // given a destination char pointer, prefilled with \null, fills it in with the name of the
  // const_extent caller must provide destination of sufficient length
  std::string name_from_weight_data(hexagon_nn_wide_address_const_t weight_data,
  ...
  void store_named_weight_bufs(const hexagon_nn_wide_address_const_t *const buffers,
                               const uint64_t *const lengths, const unsigned num_buffers);
  ...
  API_EXPORT bool extract_const_extent_data(uint64_t offs_bytes, size_t len, void *dstp,
                                            DMA_Manager *dma = nullptr);
  // same, using an external const_extent
  bool extract_const_extent_data(uint64_t offs_bytes, size_t len, void *dstp,
                                 hexagon_nn_wide_address_const_t weight_data,
                                 const uint64_t weight_length);
...
inline DeserSegDescs const &Deserz::get_segments() const { return full_deser->segments; }

// unaligned read of 64-bits (two 32-bit aligned reads)
template <>
inline uint64_t Deserz::simple_deserialize<uint64_t>() {
...
inline uint64_t Deserz::deserialize_uint64() { return simple_deserialize<uint64_t>(); }

template <>
inline uint64_t Deserz::deserialize_type<uint64_t>() {
  return deserialize_uint64();
}
template <>
inline float Deserz::deserialize_type<float>() {
...
template <>
inline unsigned long Deserz::deserialize_type<unsigned long>() {
  return deserialize_uint32();
}
template <>
inline long Deserz::deserialize_type<long>() {
  return deserialize_int32();
}
#endif
template <>
inline unsigned Deserz::deserialize_type<unsigned>() {
  return deserialize_uint32();
}
template <>
inline int Deserz::deserialize_type<int>() {
  return deserialize_int32();
}
template <>
inline int16_t Deserz::deserialize_type<int16_t>() {
  return deserialize_int16();
}
template <>
inline uint16_t Deserz::deserialize_type<uint16_t>() {
  return deserialize_uint16();
}
template <>
inline int8_t Deserz::deserialize_type<int8_t>() {
  return deserialize_int8();
}
template <>
inline uint8_t Deserz::deserialize_type<uint8_t>() {
  return deserialize_uint8();
}

// assert( dctx.deserialize_uint32() == SOME_CONST );
// is not safe, since if you turn off asserts, it will no longer read the 4 bytes. This is to allow
// that to work
...
template <typename T>
struct deserialize_tensor_using_constructor;

// this is fully defined in serialize_register.h
template <typename T>
struct alloc_func_for_op;
template <typename T>
struct dealloc_func_for_op;
