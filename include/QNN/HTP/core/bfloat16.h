...
inline constexpr BFloat16::BFloat16(float f) : d(0) {
  ...
  // Preserve NaN values
  // The only potential NaN values that can be lost are the ones that have an exp=0xFF and a non 0
  // bit in the 16 lsb
...
inline constexpr uint16_t BFloat16::sign_bit() const { return d & 0x8000u; }

inline constexpr uint16_t BFloat16::exp_bits() const { return d & 0x7F80u; }

inline constexpr uint16_t BFloat16::frac_bits() const { return d & 0x7Fu; }

inline constexpr bool BFloat16::is_zero() const { return (exp_bits() | frac_bits()) == 0x0000; }

inline constexpr bool BFloat16::is_neg() const { return sign_bit(); }

inline constexpr BFloat16 BFloat16::zero(bool neg) {
  return BFloat16::from_raw((neg) ? 0x8000u : 0x0);
}

inline constexpr BFloat16 BFloat16::qnan() { return BFloat16::from_raw(0x7FA0u); }
...
inline constexpr bool BFloat16::is_finite() const { return is_norm() || is_subnorm(); }
...
inline constexpr uint16_t BFloat16::make(int sign, int exp, uint32_t frac) {
  ...
  if (exp + exp_inc < exp_min()) {
    ...
    return (static_cast<uint16_t>(sign) << 15u) |
           (static_cast<uint16_t>(frac) & 0x007Fu);  // 0 exp bits
  }
  ...
  if (exp_inc > 0) {  // exp_inc < 0 not expected for float32 to bfloat16 casting
    ...
    // assert(clzr == 24);
    // clzr can only be 24 here because this make function is only called in the instantiation of a
    // BFloat16 from a float. In the current code path, there is a rounding of the fractional bits
    // before it is passed into this make function. As a result, there can be at most 8 significant
    // bits in frac variable passed to the make function, which means that clzr can only be 24
    // However, if this function is called in other places where there are different limits on the
    // number of significant bits in the input frac then clzr may be 23 or 24 and that will need to
...
PUSH_VISIBILITY(default)
template <>
class API_EXPORT std::numeric_limits<BFloat16> {
 public:
...
constexpr BFloat16 std::numeric_limits<BFloat16>::quiet_NaN() noexcept { return BFloat16::qnan(); }
