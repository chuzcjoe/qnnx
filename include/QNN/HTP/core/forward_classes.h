...
template <typename T>
ALWAYSINLINE void DeleterWithDisable<T>::operator()(T const *p) const {
...
class uptr_DWD : public std::unique_ptr<T, DeleterWithDisable<T>> {
 ...
 public:
  ...
  ALWAYSINLINE uptr_DWD(T *ptr, bool del_disabled)
      : base_t(ptr, DeleterWithDisable<T>(del_disabled)) {}
