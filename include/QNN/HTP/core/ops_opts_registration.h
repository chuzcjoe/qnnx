...
class reg_op_node {
  ...
  std::string_view const get_subview(std::string_view const strtab,
                                     std::string_view::size_type const start) const {
 ...
 public:
  ...
  void core_process(std::string_view const op_name_strtab,
                    std::string_view const type_tag_strtab) const {
  ...
  void pkg_process(std::string_view const op_name_strtab,
                   std::string_view const type_tag_strtab) const {
    std::string_view const op_name = get_subview(op_name_strtab, op_name_offset);
    std::string_view const type_tag = get_subview(type_tag_strtab, type_tag_offset);
...
class reg_optim_node {
 ...
 public:
  ...
  constexpr reg_optim_node(uint16_t const p, OptimFlags::flags_t const fl,
                           hnnx::get_entire_defopt_t d, uint16_t const l) noexcept
  ...
  void core_process(std::string_view const fname) const {
    hnnx::add_package_opt(current_package_opts_storage_vec_func(), priority, flags, defopt,
                          fname.data(), line);
  }
...
// LCOV_EXCL_START [SAFTYSWCCB-1736] constexprs resolved during compile time
template <std::string_view::size_type S>
struct sv_size_wrapper {
  std::string_view v;
};
...
class built_array {
 ...
 public:
   ...
   */
  constexpr built_array<T, N + 1> append(T newElem) const {
    return built_array<T, N + 1>(*this, newElem);
  }
  ...
  constexpr built_array<T, N + I> append(sv_size_wrapper<I> newElem) const {
...
class built_array<T, 0> {
 public:
   ...
   */
  template <std::string_view::size_type I>
  constexpr built_array<T, I> append(sv_size_wrapper<I> newElem) const {
...
/** @brief op_name_strtab_t empty struct to help specialize arr_container for the op_name string
 * table */
struct op_name_strtab_t {};
/** @brief type_tag_strtab empty struct to help specialize arr_container for the type_tag string
 * table */
struct type_tag_strtab_t {};

template <typename>
constexpr bool is_strtab() {
  return false;
}
template <>
constexpr bool is_strtab<op_name_strtab_t>() {
  return true;
}
template <>
constexpr bool is_strtab<type_tag_strtab_t>() {
  return true;
}
...
/** @brief arr_container */
template <typename T, bool S = is_strtab<T>()>
struct arr_container {
  /** @brief chain link to the built_array contained in this structure */
  template <typename UNIQ_TY, uint32_t I>
  static constexpr built_array<T, I> chain = {};
};

/** @brief arr_container<T, true> */
template <typename T>
struct arr_container<T, true> {
  /** @brief chain link to the built_array contained in this structure */
...
class reg_op_table {
 ...
 public:
  ...
  constexpr reg_op_table(reg_op_node const *const p, uint32_t const n,
                         std::string_view::value_type const *const o,
                         std::string_view::size_type const o_size,
                         std::string_view::value_type const *const t,
                         std::string_view::size_type const t_size,
                         std::string_view::value_type const *const f) noexcept
      : entries(p),
        num_entries(n),
        op_name_strtab{o, o_size},
        type_tag_strtab{t, t_size},
        file_name{op_file_name(f)} {}
...
template <uint32_t I>
using ba_str = built_array<std::string::value_type, I>;

/** @brief ba_op a built_array of reg_op_nodes */
template <uint32_t I>
using ba_op = built_array<reg_op_node, I>;

/** @brief ba_opt a built_array of reg_optim_nodes */
template <uint32_t I>
using ba_opt = built_array<reg_optim_node, I>;

/** @brief ba_op_table a built_array of reg_op_table_wrappers */
template <uint32_t I>
using ba_op_table = built_array<reg_op_table_wrapper, I>;

/** @brief ba_opt_table a built_array of reg_opt_table_wrappers */
template <uint32_t I>
using ba_opt_table = built_array<reg_opt_table_wrapper, I>;
 ...
 */
// LCOV_EXCL_START [SAFTYSWCCB-1736] constexprs resolved during compile time
template <typename UNIQ_TY, int32_t I>
class NodeCounter {
  ...
  /** @brief inc_op_name_strtab_size @return 0, or some string size constant if the string table
   * needs to grow */
  constexpr static uint64_t inc_op_name_strtab_size() noexcept { return 0; }
  /** @brief inc_type_tag_strtab_size @return 0, or some string size constant if the string table
   * needs to grow */
 ...
 public:
  ...
  constexpr static int32_t reg_op_count() noexcept {
    return inc_op() + NodeCounter<UNIQ_TY, I - 1>::reg_op_count();
  }
  ...
  /** @brief op_name_strtab_size @return The string table size for the ops that have been registered
   * so far */
  ...
  /** @brief type_tag_strtab_size @return The string table size for the ops that have been
   * registered so far */
 ...
 */
template <typename U, uint32_t I>
struct StrtabUpdate {
  /** @brief is_new_op_name whether the Op name to be appended is already present in the
   * op_name_strtab */
  ...
  /** @brief is_new_type_tag whether the type suffix to be appended is already present in the
   * type_tag_strtab */
...
constexpr std::string_view make_string_view(
    std::array<std::string::value_type, N> const &arr) noexcept {
