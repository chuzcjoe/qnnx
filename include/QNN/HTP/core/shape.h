...
shape_reduce_map::iterator shape_find_in_map(shape_reduce_map &map, void const *shp, unsigned hash,
                                             unsigned shplen);

void shape_serialize(Serializer &sctx, unsigned rank, size_t const *dims, size_t const *max_dims,
                     uint8_t const *pad);
...
// This will just do Shape<rank>::canonical_shape( graph, *ref_shape, new_flags), and return the
// result cast back to ShapeFlags const *.
ShapeFlags const *copy_shape_with_flags(Graph &gr, ShapeFlags const *ref_shape, unsigned rank,
                                        ShapeFlag newflags);
...
struct ShapeInterface : public hnnx::ShapeFlags {
...
template <size_t Rank>
struct Shape : public ShapeInterface<Rank> {
  ...
  explicit Shape(const size_t *dims_in)
      : ShapeInterface<Rank>(hnnx::ptr_to_stdarray<Rank, size_t>(dims_in), false),
        max_dims(hnnx::ptr_to_stdarray<Rank, size_t>(dims_in)),
        pad(){};
...
template <size_t Rank>
struct DynamicShape : public ShapeInterface<Rank> {
  using ShapeInterface<Rank>::dims;
 ...
 public:
  explicit DynamicShape(const size_t *dims_in, DynamicStatus state_in)
      : ShapeInterface<Rank>(hnnx::ptr_to_stdarray<Rank, size_t>(dims_in), true),
        dynamic_state(state_in){};
  ...
  API_EXPORT static ShapeInterface<Rank> const *deserialize(hnnx::Deserz &dctx,
                                                            const ShapeInterface<Rank> **ptrloc,
...
static const DynamicShape<1> null_dynamic_shape =
    DynamicShape<1>(std::array<size_t, 1>({0}), DynamicStatus::ValidData);
