...
#define IS_SCALAR(X) \
  AND(EQ(DIM_BATCHES(X), 1), EQ(DIM_HEIGHT(X), 1), EQ(DIM_WIDTH(X), 1), EQ(DIM_DEPTH(X), 1))

#define IS_NOT_SCALAR(X) \
  OR(NE(DIM_BATCHES(X), 1), NE(DIM_HEIGHT(X), 1), NE(DIM_WIDTH(X), 1), NE(DIM_DEPTH(X), 1))

#define IS_SHAPE_1x1x1xd(X) \
  AND(EQ(DIM_BATCHES(X), 1), EQ(DIM_HEIGHT(X), 1), EQ(DIM_WIDTH(X), 1), NE(RANK_OF(X), 5))

#define IS_SHAPE_1x1x1x1xc(X)                                                              \
  AND(EQ(RANK_OF(X), 5), EQ(DIM_BATCHES(X), 1), EQ(DIM_HEIGHT(X), 1), EQ(DIM_WIDTH(X), 1), \
      EQ(DIM_DEPTH(X), 1))

#define IS_1HD_H1D(A, B)                                              \
  AND(EQ(DIM_HEIGHT(A), 1), NE(DIM_WIDTH(A), 1), EQ(DIM_WIDTH(B), 1), \
      EQ(DIM_WIDTH(A), DIM_HEIGHT(B)), EQ(DIM_DEPTH(A), DIM_DEPTH(B)))
...
#define WEIGHT_STORAGE(WEIGHT, SPLIT)                                        \
  MUL(ELEMENTSIZE_OF(WEIGHT), DIM_FILTHEIGHT(WEIGHT), DIM_FILTWIDTH(WEIGHT), \
      ROUNDUP(DIM_FILTDEPTH(WEIGHT), 32), MIN(SPLIT, DIM_NFILTS(WEIGHT)))

#define ACT_STORAGE_EST(WEIGHT, ACT, SPLIT)                                                   \
  ADD(MUL(ROUNDUP(ADD(7, DIM_FILTHEIGHT(WEIGHT)), 8),                                         \
          ROUNDUP(MUL(DIM_WIDTH(ACT), ELEMENTSIZE_OF(ACT)), 8), ROUNDUP(DIM_DEPTH(ACT), 32)), \
...
#define GOOD_WEIGHTS(WEIGHT, ACT, SPLIT)                                      \
  LT(ADD(WEIGHT_STORAGE(WEIGHT, SPLIT), ACT_STORAGE_EST(WEIGHT, ACT, SPLIT)), \
     SELECT(EQ(ELEMENTSIZE_OF(WEIGHT), 1), TCM_MAXTENSOR_HALF_SIZE,           \
...
#define DO_BIG_WIDTH_SPLIT                                                       \
  AND(GT(DIM_WIDTH("*"), BIG_WIDTH_SIZE), CAN_FINE_SPLIT, GE(DIM_DEPTH("*"), 4), \
      GT(DIM_HEIGHT("*"), 1))
...
#define SHOULD_AUTOTHREAD                \
  AND(NOT(OPTION_BOOL("central_tiler")), \
      GT(DATA_SIZE("*"), MUL(OPTION_INT("autothread_size_kb"), 1024)))
// When autothreading on width, beware of rounding based on element size
#define SHOULD_AUTOTHREAD1(ACT1) \
  AND(SHOULD_AUTOTHREAD, EQ(ELEMENTSIZE_OF("*"), ELEMENTSIZE_OF(ACT1)))
#define SHOULD_AUTOTHREAD2(ACT1, ACT2) \
  AND(SHOULD_AUTOTHREAD1(ACT1), EQ(ELEMENTSIZE_OF("*"), ELEMENTSIZE_OF(ACT2)))
...
#define SHOULD_AUTOTHREAD_UNARY          \
  AND(NOT(OPTION_BOOL("central_tiler")), \
      GT(MUL(3, DATA_SIZE("*")), MUL(OPTION_INT("autothread_size_kb"), 2 * 1024)))
// When autothreading on width, beware of rounding based on element size
#define SHOULD_AUTOTHREAD_UNARY1(ACT) \
  AND(SHOULD_AUTOTHREAD_UNARY, EQ(ELEMENTSIZE_OF("*"), ELEMENTSIZE_OF(ACT)))
...
#define SMART_CHANNEL_SIZE(WEIGHT_STR, ACT_STR)                                       \
  SELECT(GOOD_WEIGHTS(WEIGHT_STR, ACT_STR, CHANNEL_SPLIT_SIZE), CHANNEL_SPLIT_SIZE,   \
         SELECT(GOOD_WEIGHTS(WEIGHT_STR, ACT_STR, DIV(CHANNEL_SPLIT_SIZE, 2)),        \
                DIV(CHANNEL_SPLIT_SIZE, 2),                                           \
                SELECT(GOOD_WEIGHTS(WEIGHT_STR, ACT_STR, DIV(CHANNEL_SPLIT_SIZE, 4)), \
                       DIV(CHANNEL_SPLIT_SIZE, 4), 32)))

// For HMX DWC
#define DWC_ACT_STORAGE_EST(WEIGHT, ACT, SPLIT)                         \
  ADD(MUL(ROUNDUP(ADD(7, DIM_FILTHEIGHT(WEIGHT)), 8),                   \
          ROUNDUP(MUL(DIM_WIDTH(ACT), ELEMENTSIZE_OF(ACT)), 8), SPLIT), \
      MUL(8, ROUNDUP(MUL(DIM_WIDTH("*"), ELEMENTSIZE_OF("*")), 8), SPLIT))

#define DWC_GOOD_WEIGHTS(WEIGHT, ACT, SPLIT)                                      \
  LT(ADD(WEIGHT_STORAGE(WEIGHT, SPLIT), DWC_ACT_STORAGE_EST(WEIGHT, ACT, SPLIT)), \
     DIV(TCM_MAXTENSOR_SIZE, 2))
...
#define DWC_SMART_CHANNEL_SIZE(WEIGHT_STR, ACT_STR)                                       \
  SELECT(DWC_GOOD_WEIGHTS(WEIGHT_STR, ACT_STR, CHANNEL_SPLIT_SIZE), CHANNEL_SPLIT_SIZE,   \
         SELECT(DWC_GOOD_WEIGHTS(WEIGHT_STR, ACT_STR, DIV(CHANNEL_SPLIT_SIZE, 2)),        \
                DIV(CHANNEL_SPLIT_SIZE, 2),                                               \
...
#define MAX_CHANNEL_SIZE(ACT)                                                     \
  ROUNDUP(MAX(1, DIV(TCM_MAXTENSOR_HALF_SIZE,                                     \
                     MUL(ROUNDUP(DIM_HEIGHT(ACT), 8),                             \
                         ROUNDUP(MUL(DIM_WIDTH(ACT), ELEMENTSIZE_OF(ACT)), 8)))), \
          32)
...
/// @brief SAME_DTYPE_QUANT("A", "B") -> true if the operands have the same dtype, stepsize and zero
/// offset
...
#define ESTIMATE_TENSOR_SIZE(T)                            \
  MUL(DIM_BATCHES(T), ROUNDUP(DIM_HEIGHT(T), TILE_HEIGHT), \
      ROUNDUP(MUL(DIM_WIDTH(T), ELEMENTSIZE_OF(T)), 8), ROUNDUP(DIM_DEPTH(T), 32))

#define ESTIMATE_SIZE(ACT, WEIGHTS, OUT)                                                      \
  ADD(MUL(DIM_BATCHES(ACT),                                                                   \
          ROUNDUP(ADD(DIM_HEIGHT(ACT), SELECT(EQ(DIM_FILTHEIGHT(WEIGHTS), 1), 0, 8)),         \
                  TILE_HEIGHT),                                                               \
          ROUNDUP(ADD(MUL(DIM_WIDTH(ACT), ELEMENTSIZE_OF(ACT)),                               \
...
#define ESTIMATE_SIZE_QUANT(ACT, WEIGHTS, OUT)                                        \
  ADD(MUL(DIM_BATCHES(ACT),                                                           \
          ...
          ROUNDUP(ADD(MUL(DIM_WIDTH(ACT), ELEMENTSIZE_OF(ACT)),                       \
                  ...
                  8),                                                                 \
          ROUNDUP(DIM_DEPTH(ACT), 32)),                                               \
      ROUNDUP(MUL(WEIGHT_STORAGE(WEIGHTS, DIM_NFILTS(WEIGHTS)), 2), 2048),            \
      ESTIMATE_TENSOR_SIZE(OUT), ROUNDUP(MUL(8, ROUNDUP(DIM_DEPTH(OUT), 32)), 2048))
...
#define ESTIMATE_MIN_SIZE(ACT, WEIGHTS, STRIDE, DIL_H, DIL_W)                             \
  ADD(MUL(ROUNDUP(ADD(GET_IN_SIZE(8, DIM_FILTHEIGHT(WEIGHTS), DIM_HEIGHT(STRIDE), DIL_H), \
                  ...
                  8),                                                                     \
          ROUNDUP(ADD(MUL(GET_IN_SIZE(MIN_WIDTH_OF(ACT), DIM_FILTWIDTH(WEIGHTS),          \
                                      DIM_WIDTH(STRIDE), DIL_W),                          \
                          ELEMENTSIZE_OF(ACT)),                                           \
                      SELECT(EQ(DIM_FILTWIDTH(WEIGHTS), 1), 0, 8)),                       \
                  8),                                                                     \
          ROUNDUP(DIM_DEPTH(ACT), 32)),                                                   \
...
#define ESTIMATE_SIZE_ALIGNED_SLICE(ACT, START, OUT)             \
  ADD(SELECT(EQ(DIM_WIDTH(START), 0), ESTIMATE_TENSOR_SIZE(ACT), \
             MUL(ESTIMATE_TENSOR_SIZE(ACT), 2)),                 \
  ...
  MAX(ROUNDUP(DIV(SUBS(DIV(MUL(TCMSIZE, 7), 8),                                                    \
                       ADD(ROUNDUP(WEIGHT_STORAGE(WEIGHT_STR, DIM_NFILTS(WEIGHT_STR)), 2048),      \
                           MUL(ROUNDUP(DIM_HEIGHT(ACT_STR), 8), ROUNDUP(DIM_DEPTH(ACT_STR), 32),   \
                               SUB(DIM_FILTWIDTH(WEIGHT_STR), 1), DILATION,                        \
                               ELEMENTSIZE_OF(ACT_STR)))),                                         \
...
#define EVEN_TILE_UNDER_CUSTOM(CURRENT, TARGET, ROUNDER) \
  ROUNDUP(DIV(CURRENT, ADD(DIV(CURRENT, TARGET), 1)), ROUNDER)

// Stays adaptive until CURRENT = 2*TARGET, then just tiles to size TARGET consistently
// Balances the need to avoid the horrible case where you tile 260 into 256, with the benefits of
// consistent tiling (that is, maybe avoiding lots of concat - retile operations in a large network
...
// Gotta be small enough to fit into VTCM, and then another factor of 4 so that we exploit
// parallelism. Using half of VTCM instead of the full VTCM, as a precaution  / to let other ops run
// in parallel Gotta use DIM_CHANNEL(ACT_STR) since there's no channel-tiling the activation unless
// it's DWC, and stride 2 isn't. Has to be rounded up to 16, because it must be a multiple of 8 in
// the end, and space2depth divides width by 2 Default is 256; smaller tiles usually get better
// performance, and size 256 doesn't incur much overhead. Also, for the activation, multiplying
// height by 2 (becase that's how it'll get tiled if "*" is tiled to TILE_HEIGHT) and multiplying
// the whole thing by 2 (because whatever we tile "*" to, we tile ACT to 2x, due to stride).
//
// Sometimes, we need to tile all the way down to 8; hence, we can specify whether to round to 16 or
// to 8
#define SMART_EARLY_WIDTH_S2(ACT_STR, WEIGHT_STR, OUT_STR, TCMSIZE, ROUNDER)                      \
  EVEN_TILE_UNDER_CUTOFF2_CUSTOM(                                                                 \
      DIM_WIDTH(OUT_STR),                                                                         \
      MIN(256,                                                                                    \
          ROUNDUP(MAX(16, DIV(SUBS(DIV(MUL(TCMSIZE, 7), 16),                                      \
                                   ROUNDUP(WEIGHT_STORAGE(WEIGHT_STR, MIN(DIM_NFILTS(WEIGHT_STR), \
                                                                          CHANNEL_SPLIT_SIZE)),   \
                                           2048)),                                                \
...
#define MAX_GOOD_WIDTH_CHANSHUF(ACT_STR, OUT_STR, TCMSIZE)                                 \
  MAX(MIN_WIDTH,                                                                           \
      ROUNDUP(DIV(DIV(MUL(TCMSIZE, 6), 8),                                                 \
                  MUL(4, ADD(MUL(ELEMENTSIZE_OF(ACT_STR), ROUNDUP(DIM_HEIGHT(ACT_STR), 8), \
                                 ROUNDUP(DIM_DEPTH(ACT_STR), 32)),                         \
...
#define MAX_GOOD_WIDTH_QUANTIZE(IN, OUT, TCMSIZE)                                   \
  MAX(MIN_WIDTH_OF(OUT),                                                            \
      DIV(TCMSIZE, MUL(4, ADD(MUL(ELEMENTSIZE_OF(IN), ROUNDUP(DIM_HEIGHT(IN), 8),   \
                                  ROUNDUP(DIM_DEPTH(IN), 32)),                      \
                              MUL(ELEMENTSIZE_OF(OUT), ROUNDUP(DIM_HEIGHT(OUT), 8), \
                                  ROUNDUP(DIM_DEPTH(IN), 32))))))

#define MAX_GOOD_WIDTH_ELEMWISE(FIRST_IN_STR, SECOND_IN_STR, OUT_STR, TCMSIZE)                     \
  MAX(MIN_WIDTH,                                                                                   \
      ROUNDUP(                                                                                     \
          DIV(DIV(MUL(TCMSIZE, 6), 8),                                                             \
...
#define FP16_CONST_CASTSLICE(X, Y, Z)                                                          \
  LET(X, Op(FROM_DEFAULT_PACKAGE("SlicePad_shape_inplace"),                                    \
            LET(Y, Op(FROM_DEFAULT_PACKAGE("Cast_fp32_to_fp16_plain"), Z)), "Before", "Start", \
            "Out", "Zero"))
...
#define REARRANGE_FROM_W1(A) \
  WITH_SIZE(SHAPE_FROM_W1(A), WITH_TYPE(WITH_SAME_ID("*", A), Op("space_rearrange", A)))
...
#define SHAPE_FROM_H1(A)                                                  \
  SELECT(IS_QUINT8(A),                                                    \
         ...
         gen_Shape(DIM_BATCHES(A), MIN(8, DIV(ADD(DIM_WIDTH(A), 3), 4)),  \
                   MUL(DIV(ADD(DIM_WIDTH(A), 31), 32), 4), DIM_DEPTH(A)))
...
// QNN's --debug is used purely for accuracy debugging, so the performance impact from the
// duplication of the conv op isnt a concern
#define ACT_FUSION_MULTI_OUT_CHECK(OP)                                                 \
  OR(EXTERNAL_CONSTRAINT(has_only_one_consumer, OP), OPTION_BOOL("force_conv_fusion"), \
     AND(PRODUCER_FOR(OP, "*Output"), EXTERNAL_CONSTRAINT(has_n_consumers, OP, 2),     \
         PRODUCER_FOR("*", "*Output")))
...
#define WIDTH_TO_HEIGHTX_CONSTRAINT(OPSTR)                                              \
  OR(AND(GT(DIM_WIDTH(OPSTR), TILE_HEIGHT), EQ(REM(DIM_WIDTH(OPSTR), TILE_HEIGHT), 0)), \
     AND(IS_QUINT16(OPSTR), LT(DIM_WIDTH(OPSTR), 32), GT(DIM_WIDTH(OPSTR), 4),          \
         EQ(REM(DIM_WIDTH(OPSTR), 4), 0)))
  ...
  SELECT(EQ(REM(DIM_WIDTH(OPSTR), TILE_HEIGHT), 0),                                                \
         ...
         SELECT(EQ(REM(DIM_WIDTH(OPSTR), 4), 0),                                                   \
                gen_Shape(DIM_BATCHES(OPSTR), MUL(DIM_HEIGHT(OPSTR), 4), DIV(DIM_WIDTH(OPSTR), 4), \
                          DIM_DEPTH(OPSTR)),                                                       \
                gen_Shape(DIM_BATCHES(OPSTR), DIM_HEIGHT(OPSTR), DIM_WIDTH(OPSTR),                 \
                          DIM_DEPTH(OPSTR))))
...
#define HEIGHT84_SHAPE(OPSTR)                                                                  \
  SELECT(EQ(REM(DIM_WIDTH(OPSTR), TILE_HEIGHT), 0),                                            \
         SELECT(AND(LE(DIM_WIDTH(OPSTR), 16), IS_QUINT16(OPSTR),                               \
                    NOT(OPTION_BOOL("dynamic_graph_input"))),                                  \
                gen_Shape(DIM_BATCHES(OPSTR), DIV(DIM_WIDTH(OPSTR), 4), 4, DIM_DEPTH(OPSTR)),  \
                gen_Shape(DIM_BATCHES(OPSTR), TILE_HEIGHT, DIV(DIM_WIDTH(OPSTR), TILE_HEIGHT), \
                          DIM_DEPTH(OPSTR))),                                                  \
...
#define HEIGHT_TO_WIDTH_CONSTRAINT(OPSTR)                                      \
  AND(GE(DIM_HEIGHT(OPSTR), 8192), EQ(REM(DIM_HEIGHT(OPSTR), TILE_HEIGHT), 0), \
      EQ(DIM_WIDTH(OPSTR), 1), EQ(REM(DIM_DEPTH(OPSTR), 32), 0))
