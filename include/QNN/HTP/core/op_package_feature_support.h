 ...
 *   if there are parameter orders and/or per-channel ops listed using macros below, op package
...
typedef std::unordered_map<std::string, std::vector<ParamConfig_t>>
    ParamMap_t;  // pkg::op name -> vector<param>

API_EXPORT API_C_FUNC std::string API_FUNC_NAME(combine_pkg_op_name)(const char *package_name,
                                                                     const char *op_name);
 ...
 */
API_EXPORT std::vector<ParamConfig_t> *add_to_param_map(ParamMap_t &pmap,
                                                        std::string_view package_op_name);

// adds a new op parameter entry into a vector of parameter configs
API_EXPORT void add_package_individual_param_config(std::vector<ParamConfig_t> *pvec,
                                                    const char *param_name, bool mandatory,
                                                    void *default_val);
...
// adds a variable number of ParamConfig_t constructed from [param_name, mandatory, default_val] to
// a ParamConfig_t vector
template <typename... T>
API_EXPORT void add_package_param_configs_base(std::vector<ParamConfig_t> *pvec,
                                               const char *param_name, bool mandatory,
...
template <typename... T>
API_EXPORT void add_package_param_configs(ParamMap_t &pmap, std::string_view package_op_name,
                                          const char *param_name, bool mandatory, void *default_val,
                                          T &&...args) {
...
template <typename... T>
API_EXPORT void add_package_axis_params(std::set<std::string> &aset, const char *param_name,
                                        T &&...args) {
...
// adds a variable number of per-channel scaled package_name::op_name into a set of
// package_name::op_name
template <typename... T>
API_EXPORT void add_package_per_channel_ops(std::set<std::string> &oset, const char *op_name,
                                            T &&...args) {
...
#define DECLARE_PACKAGE_PARAM_ORDER_DEF() \
  API_HIDDEN hnnx::ParamMap_t &current_package_param_order_storage_map_func();
...
#define DEF_PACKAGE_PARAM_ORDER(OP, PARAM1, MANDATORY1, DEFAULT1, ...)                           \
  [[maybe_unused]] static bool CTRICKS_PASTER(_PKG_PARAM_ORDER_REG_, __LINE__) =                 \
      (hnnx::add_package_param_configs(current_package_param_order_storage_map_func(),           \
                                       hnnx::combine_pkg_op_name(THIS_PKG_NAME_STR, OP), PARAM1, \
                                       MANDATORY1, DEFAULT1, ##__VA_ARGS__),                     \
       true);
...
#define REGISTER_PACKAGE_PARAM_ORDERS()                                                      \
  if (getPkgParamTmpMap().find(std::string(THIS_PKG_NAME_STR)) == getPkgParamTmpMap().end()) \
    getPkgParamTmpMap()[std::string(THIS_PKG_NAME_STR)] =                                    \
        &current_package_param_order_storage_map_func();
...
#define REGISTER_PACKAGE_AXIS_PARAMS()                                \
  if (getPkgParamAxesTmpMap().find(std::string(THIS_PKG_NAME_STR)) == \
      getPkgParamAxesTmpMap().end())                                  \
...
#define LIST_PACKAGE_PER_CHANNEL_QUANTIZED_OPS(...)                                     \
  [[maybe_unused]] static bool CTRICKS_PASTER(_PKG_PER_CHANNEL_OPS_, __LINE__) =        \
      (hnnx::add_package_per_channel_ops(currentPackagePerChannelQuantizedOpsSetFunc(), \
                                         ##__VA_ARGS__),                                \
       true);

#define REGISTER_PACKAGE_PER_CHANNEL_QUANTIZED_OPS()                      \
  if (getPkgPerChannelOpsTmpMap().find(std::string(THIS_PKG_NAME_STR)) == \
      getPkgPerChannelOpsTmpMap().end())                                  \
    getPkgPerChannelOpsTmpMap()[std::string(THIS_PKG_NAME_STR)] =         \
        &currentPackagePerChannelQuantizedOpsSetFunc();
