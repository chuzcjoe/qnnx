...
class PackageOpStorageBase {
 public:
  ...
  API_EXPORT PackageOpStorageBase(const std::string_view op_name_in,
                                  const std::string_view type_tag_in, const bool is_external_in,
                                  const op_reg_parms opreg_params_in);
  API_EXPORT OpFactory make_op_wrapper() const;
};

// The map to store op package ops
API_EXPORT std::map<std::string, std::vector<std::unique_ptr<PackageOpStorageBase>> *>
    &get_pkg_op_tmp_map();
...
#define INIT_PACKAGE_OP_DEF()                                                              \
  API_HIDDEN std::vector<std::unique_ptr<hnnx::PackageOpStorageBase>>                      \
...
#define PKG_TYPE_SUFFIX(OP, ARGS) \
  (hnnx::ConcatStr<hnnx::ConstexprStrLen(OP), hnnx::ConstexprStrLen(ARGS)>(OP, ARGS))
...
inline float call_cost_func(package_cost_function_t func, const Op *op) { return (func)(op); }
inline float call_cost_func(std::string_view, const Op *op) { return 0.0; }
// LCOV_EXCL_STOP
namespace hnnx {
template <auto F>
void add_package_op_ext(std::vector<std::unique_ptr<PackageOpStorageBase>> &ops,
                        const std::string_view op_name_in, const char *type_tag,
