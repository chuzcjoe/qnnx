...
template <unsigned X>
static constexpr unsigned log2_ceil() {
  if constexpr (X <= 16) {
...
inline constexpr unsigned encode_opaquet_size() {
  ...
  if constexpr (length == 0) {  // if 0, we don't care about the alignment
    ...
    static_assert(align >= 1 && align <= max_opaquet_align && (align & (align - 1)) == 0,
                  "bad alignment value");
...
class SerOpsInterface {
 ...
 protected:
  ...
  virtual void op_serialize_func(Op const *op, unsigned n_in, Tensor const *const *in_tens,
                                 unsigned n_out, uptr_Tensor const *out_tens, unsigned mode) = 0;
 ...
 public:
  ...
  virtual void
  after_non_runlist() = 0;  // call after serializing 'non_runlist', before 'combined runlist'
  ...
  // to be called from TypicalOpWithCompiler<F, OpaqueT>::serialize, with OpaqueT explicitly
  // specified
  ...
  inline void op_typical_with_extra(Op const *op, std::array<const Tensor *, N_IN> const &inputs,
                                    std::array<uptr_Tensor, N_OUT> const &outputs) {
    op_serialize_func(op, N_IN, inputs.data(), N_OUT, outputs.data(),
                      bake::encode_opaquet_size<OpaqueT>());
  }
  ...
  inline void op_variadic(Op const *op, V_IN const &inputs, V_OUT const &outputs) {
    op_serialize_func(op, inputs.size(), inputs.data(), outputs.size(), outputs.data(),
                      opMODE_variadic);
  }
  ...
  inline void op_for_tensor(Op const *op, uptr_Tensor const &out_tens) {
    op_for_tensor_func(op, 1, &out_tens);
  }
  ...
  // The serialize method calls methods of the returned handle (which call protected .spcl_XX
  // virtual methods) and then .spcl_done is called when the handle is deleted. So you can do the
  // whole thing in one line, e.g.
