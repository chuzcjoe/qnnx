...
class OpInfo {
 ...
 public:
  ...
  OpInfo(cost_function_t cost_in, Flags_word flags_in, OpFactory op_factory_in, bool is_external_in,
      ...
      : cost(cost_in),
        flags(flags_in),
        is_external_flag(is_external_in),
        op_factory(op_factory_in),
  ...
  API_EXPORT cost_function_t const &get_cost() const {
    return cost;
...
API_FUNC_EXPORT void register_op_info(const std::type_info &type, cost_function_t cost,
                                      Flags_word flags, OpFactory op_factory, bool is_external,
                                      const std::string_view type_tag);

template <typename T>
API_FUNC_EXPORT inline void register_op_info(cost_function_t cost, Flags_word flags,
                                             OpFactory op_factory, bool is_external,
                                             const std::string_view type_tag) {
