...
// This class is used, on both encoder and decoder, to contain a 'const extent descriptor' in its
// raw form, (just an array of uint32) and provide higher-level access to the contents.
...
class ConstExtentDesc {
 ...
 public:
  ...
  static uint8_t constexpr HEADER_FLAG_IS_REPLACEABLE =
      (1 << 3);  ///< Contents are replaceable weights
  ...
  // Note: if 'adjust_offset' is true, the 'offset' field from the containing extent will be added
  // to offset, so that the offset is from the start of the descriptor, instead of the start of the
  // containing extent.
  ...
  // optional name of the const_extent this descriptor corresponds to. Used for matching in
  // weight_sharing.
  ...
  enum offset_adjust_t {
    ...
    OFFSET_ADJUST_FALSE = OFFSET_ADJUST_DESC_REL,  ///< Alias to descriptor-relative address
                                                   ///< (default) - i.e. dont adjust
    OFFSET_ADJUST_EXTENT_REL = 1,                  ///< Adjust offset relative to extent
    OFFSET_ADJUST_TRUE =
        OFFSET_ADJUST_EXTENT_REL,  ///< Alias to extent-relative offset - i.e. adjust
  ...
  ///
  mempool_entry mempool_info(unsigned idx,
                             offset_adjust_t adjust_offset = OFFSET_ADJUST_FALSE) const;
  ...
  class mempool_iterator {
   public:
    ...
    ///
    explicit mempool_iterator(ConstExtentDesc const &cedesc, uint32_t index)
        : _cedesc(cedesc), _index(index) {}
