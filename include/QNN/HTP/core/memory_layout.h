...
static inline constexpr std::array<T, Rank> make_stdarray_helper(const T val,
...
static inline constexpr std::array<T, Rank> make_stdarray(const T val) {
 ...
 */
template <size_t... Stuff>
struct IChunkedMemoryLayout;

/*
 * The smallest Chunk is just 1 element
 */
template <size_t RankVal, size_t IndirRanks>
struct IChunkedMemoryLayout<RankVal, IndirRanks> {
  ...
  static inline constexpr std::array<size_t, Rank> pad_dims(
      const std::array<size_t, Rank> dims_in) {
    return dims_in;
  }
};
...
struct IChunkedMemoryLayout<RankVal, IndirRanks, Dim, ChunkSize, Rest...> {
  ...
  static constexpr unsigned inner_dim =
      ((ChunkSize == 0) && Smaller::is_valid_chunk) ? Dim : Smaller::inner_dim;
  static constexpr std::array<size_t, Rank> embiggen_chunksize(
      const std::array<size_t, Rank> smaller_chunksize) {
  ...
  static constexpr size_t chunk_total =
      ChunkSize ? Smaller::chunk_total * ChunkSize : Smaller::chunk_total;
  /* Where in the chunk is this element? */
  /*
   *  FIXME sooner than later: recommendation to return std::pair or similar of chunk_index and
   * chunk_offset Can keep compatibility easily enough with a single wrapper.
   */
  static inline constexpr size_t chunk_offset(const std::array<size_t, Rank> &padded_coords,
                                              const std::array<size_t, Rank> &dims_total,  //
                                              size_t block_off = 0) {
...
template <size_t RankVal>
struct FlatMemoryLayout {
  static constexpr size_t Rank = RankVal;
  static constexpr size_t indirect_ranks =
      RankVal;  // to be consistent; only applies when chunk_total > 1.
  static constexpr std::array<size_t, Rank> ChunkSizes = hnnx::make_stdarray<size_t, Rank>(1);
  static constexpr size_t chunk_total = 1;
  ...
  static inline constexpr std::array<size_t, Rank> pad(const std::array<size_t, Rank> dims_in) {
    return dims_in;
  }
...
class R4WideCroutonLayout
    : public ChunkedMemoryLayout<4, 0, 0, 1, 0, 2, 0, 3, 0, 1, 2, 2, 32, 3, 32> {};

// Croutons for HMX, YYYXDDDDDXX chunks
class R4Crouton4x1Layout
    : public ChunkedMemoryLayout<4, 0, 0, 1, 0, 2, 0, 3, 0, 1, 8, 2, 2, 3, 32, 2, 4> {};

// Croutons for HMX, YYXXDDDDDYX chunks
class R4Crouton2x2Layout
    : public ChunkedMemoryLayout<4, 0, 0, 1, 0, 2, 0, 3, 0, 1, 4, 2, 4, 3, 32, 1, 2, 2, 2> {};

// Croutons for HMX, YYXXDDDDDYX chunks (wide aspect ratio)
class R4WideCrouton2x2Layout
    : public ChunkedMemoryLayout<4, 0, 0, 1, 0, 2, 0, 3, 0, 2, 16, 3, 32, 1, 2, 2, 2> {};

// Croutons2 for HMX, 8x4x32 chunks where the data is 16b
class R4Crouton2Layout
    : public ChunkedMemoryLayout<4, 0, 0, 1, 0, 2, 0, 3, 0, 1, 8, 2, 2, 3, 32, 2, 2> {};

// Note, we want the lists of numbers to be paired properly, some of the commas won't have spaces
// after.
// clang-format off
...
class R4WideCrouton4Layout
    : public ChunkedMemoryLayout<4, 0, 0, 1, 0, 2, 0, 3, 0, 1, 2, 2, 8, 3, 32> {};

class R4Weights8x4Layout
    : public ChunkedMemoryLayout<4, 0, 0, 1, 0, 3, 0, 2, 0, 0, 8, 1, 4, 2, 16, 3, 32, 2, 2> {};

class R5CroutonLayout
    : public ChunkedMemoryLayout<5, 0, 0, 1, 0, 2, 0, 3, 0, 4, 0, 2, 8, 3, 8, 4, 32> {};
...
// typedef ChunkedMemoryLayout<4, 0,0, 1,0, 2,0, 3,0, 1,8, 2,8, 3,32> R4CroutonLayout;		// Croutons
// for HMX, 8x8x32 chunks
