 ...
 * it will need an output (which is pre-allocated, so it's not a return value, but a
 * pointer/reference to something that gets filled in...), and let's give it a hook to indicate
 * failure.  Since the results are not returned, we can have a simple integer return value to
 * indicate failure.
 *
 ...
 * * The number and types of the inputs:  1  input tensor of type "const GenericTensor" (inputs are
 * const)
 ...
 *
 *
 * This function prototype is within the abilities of even the most C++ - averse assembly
 * programmer. All we need to do is create all the C++ information from this function definition.
 *
 ...
 * keeping them in appropriately-typed tuples, and then applying the tuple to the templated
 * execution function F.
 *
 * On x86, this yields the generation of an execute() function that loads up the pointers out of the
 * arrays and calls the execution function, directly (not through a pointer) and immediately.  This
...
/*
 * FIXME: tensor serialization/deserialization should update its own ID information, rather than put
 * it in op...
 */
...
class TypicalOpUtil : public Op {
 protected:
  ...
  void do_deserialize(Deserz &dctx, size_t n_in, Tensor const **inputs, size_t n_out,
                      uptr_Tensor *outputs);

  GraphStatus do_allocate(Graph &graph_in, size_t n_out, uptr_Tensor *outputs);

  // used in TypicalOpIO::prepare
  GraphStatus assign_input_pointers(OpIoPtrs const &op_io_ptrs, size_t n_inputs,
                                    Tensor const **inputs);
  ...
  GraphStatus output_scratch_create(Graph &gr, size_t num_scratch_outputs,
                                    uptr_Tensor *scratch_outputs,
                                    tensor_generate_fp const *out_gen_functions,
...
template <unsigned N_OUT, unsigned N_IN>
struct TypIoIo {
...
template <unsigned N_OUT>
struct TypIoIo<N_OUT, 0> {
  std::array<uptr_Tensor, N_OUT> outputs_arr;
...
template <unsigned N_OUT, unsigned N_IN>
class TypicalOpIoBase : public TypicalOpUtil {
 ...
 protected:
  ...
  // in TypicalOp::execute method, we static-assert this, and check that the subclass is the same
  // size as this base.
 ...
 public:
  ...
  // * It requires an architecture or feature not available (Hexagon architecture version or HVX
  // present, etc)
  // ** But, this we could exclude from even entering the ops that are registered, right?
  // * The inputs are invalid types
  // ** For example, if we mandate a tensor with true elements of float type, we can't accept one
  // with quint8 types.
  ...
  virtual void serialize(SerOpsInterface &sctx) const override {
    sctx.op_typical(this, io.inputs(), io.outputs());
  }
};
...
class TypicalOpIO
    : public TypicalOpIoBase<ArgsTuples<Ftype>::n_outputs, ArgsTuples<Ftype>::n_inputs> {
 protected:
 ...
 public:
  ...
  explicit TypicalOpIO(OpIoPtrs const &ioptrs)
      : TypicalOpIoBase<n_outputs, n_inputs>(ioptrs) {}  // see note below...
  ...
  static bool valid_construction(OpIoPtrs const &op_io_ptrs, const OpId my_id_in) {
    ...
    // debuglog("numbers OK");
    if (!are_input_tensors_compatible<n_inputs, input_tuple_type>(graph_in,
                                                                  op_io_ptrs.in_tensors.data()))
      return false;
    if (build_options_pub::DebugRegistry) debuglog("inputs OK");
    if (!op_io_ptrs.is_clone_mode()) {
      if (!are_output_defs_valid<n_nonscratch_outputs, output_tuple_defs>(
              op_io_ptrs.out_defs.data(), graph_in))
        return false;
    }
    // debuglog("outputs OK");
    return true;
  ...
  static tensor_generate_fp const *output_generator_array() {
    return tensor_gen_array_ptr<true_output_tuple_type>();
  }
  // Return a pointer to a const array of [n_scratch_outputs] of dt_rank_pair, each of which is the
  // dt,rank for one of the scratch output tensors; it will be nullptr if there are none.
  static inline dt_rank_pair const *scratch_output_dt_rank_array() {
    if constexpr (n_scratch_outputs == 0) {
      ...
      return tensor_dt_rank_array_for_scratch<n_nonscratch_outputs,
                                              true_output_tuple_type>::table_p();
    }
  }
    ...
    if constexpr (build_options_pub::WITH_PREPARE) {
      ...
      if (result == GraphStatus::Success) {
        if constexpr (n_scratch_outputs == 0) {
          ...
          result = this->output_allocate_with_scratch(
              op_io_ptrs, n_outputs, n_scratch_outputs, this->io.outputs().data(),
              output_generator_array(), scratch_output_dt_rank_array());
        }
      }
 ...
 protected:
  ...
  static constexpr size_t parm_n_total =
      n_outputs + n_inputs + (has_slice_parm ? 1 : 0) + (has_graph_parm ? 1 : 0);
  ...
  // This is only used for outputs & inputs & scratch outputs; not for 'extras' (slice_spec, Graph
  // const &)
  template <size_t I>
  inline auto &get_exec_parm() const noexcept {
    static_assert(I < parm_n_inout);
    if constexpr (I < n_nonscratch_outputs ||
                  I >= n_nonscratch_outputs + n_inputs) {  // output (incl. scratch)
  ...
  template <size_t... I>
  inline GraphStatus call_with_parms(Ftype f, Graph *gp, op_slice_spec ss,
...
template <auto F>
class TypicalOp
    : public TypicalOpIO<std::remove_pointer_t<decltype(F)>>  //, public Cost<TypicalOp<F>>
{
 ...
 public:
  ...
  // Definitions for inputs.  These aren't needed after we prepare the graph, so perhaps they could
  // be moved out
  // std::array<InputDef,n_inputs> input_defs;
  // Definitions for outputs.  These aren't needed after we prepare the graph, so perhaps they could
  // be moved out
  ...
  // This should turn into efficient code that pulls out the inputs to their correct place as
  // arguments then sibling calls the templated function directly. Performance is very important for
  // execute()
  virtual GraphStatus execute(Graph *const g,
                              op_slice_spec const slice_spec_in) const noexcept override {
    ...
    return this->call_with_parms(F, g, slice_spec,
                                 std::make_index_sequence<TypicalOpIO<Ftype>::parm_n_inout>{});
  }
...
template <auto F, typename OpaqueT>
class TypicalOpWithCompiler : public TypicalOp<F> {
  ...
  static inline constexpr bool check_szal() {
    ...
    return bake::check_size_align<TypicalOpWithCompiler<F, OpaqueT>>::template check<
        std::get<0>(claimed_size), std::get<1>(claimed_size)>();
  }
 ...
 public:
  ...
  virtual Flags_word get_flag_word() const override {
    return hnnx::flags_for<TypicalOpWithCompiler>;
  }
#else
...
template <auto F>
constexpr bool has_compile_method = false;

template <auto F>
struct API_HIDDEN OpaqueT_FOR {
  using type = char;
};

template <auto F>
using FType_of = typename std::remove_pointer<decltype(F)>::type;

template <auto F>
struct API_HIDDEN DerivedType {
  using FType = FType_of<F>;
  using type = std::conditional_t<has_compile_method<F>,
                                  hnnx::TypicalOpWithCompiler<F, typename OpaqueT_FOR<F>::type>,
                                  hnnx::TypicalOp<F>>;
};
...
API_HIDDEN inline constexpr unsigned get_op_num_inputs() {
  return OpT::n_inputs;
}
template <typename OpT>
API_HIDDEN inline constexpr unsigned get_op_num_outputs() {
  return OpT::n_outputs;
}

template <typename OpT, unsigned IDX>
using op_input_t =
    std::remove_pointer_t<typename std::tuple_element_t<IDX, typename OpT::input_tuple_type>>;
template <typename OpT, unsigned IDX>
using op_output_t =
    std::remove_pointer_t<typename std::tuple_element_t<IDX, typename OpT::output_tuple_type>>;
...
template <unsigned IDX, typename OpT>
API_HIDDEN inline auto const &get_op_input(OpT &op) {
  using in_ttype = op_input_t<OpT, IDX>;
  in_ttype const &in = *static_cast<in_ttype const *>(op.io.inputs()[IDX]);
  return in;
}
template <unsigned IDX, typename OpT>
API_HIDDEN inline auto &get_op_output(OpT &op) {
  using out_ttype = op_output_t<OpT, IDX>;
  out_ttype &out = *static_cast<out_ttype *>(op.io.outputs()[IDX].get());
  return out;
}

template <unsigned O_IDX, unsigned I_IDX, typename OpT>
void ctor_hook_flat_inplace(Graph &, OpT &op) {
...
template <unsigned O_IDX, unsigned I_IDX, typename OpT>
void ctor_hook_crouton_inplace(Graph &, OpT &op) {
  auto &out = get_op_output<O_IDX>(op);
  auto const &in = get_op_input<I_IDX>(op);
