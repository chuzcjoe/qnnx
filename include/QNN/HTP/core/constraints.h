...
//! IS_SIGNED_SYMM("operand") -> bool (true if operand has 'Signed Quantized' output with offset ==
//! 0)
...
//! IS_FULLY_CONNECT_WEIGHT("operand") -> bool (true if operand is QUInt8 or (QInt8 and
...
//! DIM_FILTHEIGHT("operand") -> unsigned (extract 'filter height' dimension from filter weights,
//! #0)
...
#define IS_EMPTY(X) \
  AND(IS_EMPTY_DIM(X, 0), IS_EMPTY_DIM(X, 1), IS_EMPTY_DIM(X, 2), IS_EMPTY_DIM(X, 3))
...
#define IS_FP16_BF16_4(W, X, Y, Z)                                           \
  OR(AND(EQ(DTYPE_OF(W), DType::Float16), EQ(DTYPE_OF(X), DType::Float16),   \
         EQ(DTYPE_OF(Y), DType::Float16), EQ(DTYPE_OF(Z), DType::Float16)),  \
     AND(EQ(DTYPE_OF(W), DType::BFloat16), EQ(DTYPE_OF(X), DType::BFloat16), \
         EQ(DTYPE_OF(Y), DType::BFloat16), EQ(DTYPE_OF(Z), DType::BFloat16)))

#define IS_FP16_BF16_N(_1, _2, _3, _4, NAME, ...) NAME
#define IS_FP16_BF16(...)                                                                     \
  IS_FP16_BF16_N(__VA_ARGS__, IS_FP16_BF16_4, IS_FP16_BF16_3, IS_FP16_BF16_2, IS_FP16_BF16_1) \
  (__VA_ARGS__)
