...
#define TILEDATA(adrtab, next_tab_col, next_tab_row, h, w, d)                                    \
  {                                                                                              \
    (uint8_t **)(adrtab), static_cast<uint32_t>(next_tab_col),                                   \
        static_cast<uint32_t>(next_tab_row), static_cast<uint32_t>(w), static_cast<uint32_t>(h), \
...
inline int32_t max_i32(int32_t a, int32_t b) { return Q6_R_max_RR(a, b); }
inline int32_t min_i32(int32_t a, int32_t b) { return Q6_R_min_RR(a, b); }
inline uint32_t max_u32(uint32_t a, uint32_t b) { return Q6_R_maxu_RR(a, b); }
inline uint32_t min_u32(uint32_t a, uint32_t b) { return Q6_R_minu_RR(a, b); }
#else
inline int32_t max_i32(int32_t a, int32_t b) { return (a < b) ? b : a; }
inline int32_t min_i32(int32_t a, int32_t b) { return (a < b) ? a : b; }
inline uint32_t max_u32(uint32_t a, uint32_t b) { return (a < b) ? b : a; }
 ...
 */
template <uint32_t MBITS>
inline constexpr std::pair<int32_t, uint32_t> get_scalefactor(float x) {
  union {
    float f;
    uint32_t u32;
  } const uu = {x};
 ...
 *
 */
[[maybe_unused]] inline ALWAYSINLINE constexpr uint32_t get_scaling_params(float x, int max_sl,
                                                                           int max_sr) {
  auto [e, m] = get_scalefactor<15>(x);
  // Set a sl or sr amount to perform a multiply of 2^exponent by mantissa.
  int sl = (e > 0) ? e : 0;
  int sr = (e > 0) ? 0 : -e;
  // The max_sl allows the addition of extra left shifts when working with small numbers having
  // negative exponents. For every extra left shift, there is an offsetting right shift added so
  // that the net right shift amount required from the exponent stays the same. The max_sr parameter
  // provides a ceiling to the required offsetting right shifts, preventing the total right shift
  // requirement from being large enough to erase data through shifting.
  if (sl == 0 && sr > 0) {
