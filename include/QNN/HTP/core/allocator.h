 ...
 * is all this pimpl stuff needed, or could we just inherit Allocator and have a
 * unique_ptr<Allocator> in our graph?
 */
...
class Allocator {
 public:
  ...
  // Either allocates enough, or dips into a buffer (and changes the buffer pointer and size
  // parameter accordingly). al is an alignment parameter; it must be a power of 2 or the code below
  // won't work.
  ...
  API_EXPORT virtual void allocate_n(void **arrp, size_t n, size_t block_size, size_t alignment,
                                     MemoryClass memclass, unsigned options, DType dtype);
  ...
  // Currently this is used when deserializing, since we can't free things immediately when in
  // Crate.
  //
  ...
  API_EXPORT virtual void allocate_persistent_blocks(void **table, size_t nblocks,
                                                     size_t block_size, size_t alignment,
                                                     unsigned options);

  API_EXPORT inline void *allocate(const void *oldval, size_t block_size, size_t alignment,
                                   MemoryClass memclass, unsigned options, DType dtype) {
    PUSH_WARNING()
  ...
  template <typename T>
  API_EXPORT static inline T *round_up_align(T *p, size_t align) {
...
class FakeAllocator : public Allocator {
 public:
  API_EXPORT FakeAllocator(Allocator::Mode mode_in, Graph &graph_in)
      : Allocator(mode_in, graph_in){};
...
class MemPoolRunTimeAccessor {
 ...
 public:
  API_EXPORT MemPoolRunTimeAccessor(hexagon_nn_wide_address_const_t spill_area_in,
                                    fa::PoolDesc const *const pt, unsigned const pt_size)
  ...
  API_EXPORT constexpr unsigned num_pools() const {
    return max_pool_id;
