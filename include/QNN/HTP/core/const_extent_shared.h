...
constexpr unsigned CONST_EXTENT_MEMPOOL_RECORD_SIZE_BYTES =
    CONST_EXTENT_MEMPOOL_RECORD_SIZE_WORDS * 4u;

// This function is used by deserializer to help it extract the extent-desc table (as a
// vector<uint32_t>) from some arbitrary point down the pickle. Parameter is a pointer to the first
// 4 words; the return value is
