...
class MatchAstSubnode {
 ...
 protected:
  ...
  MatchAst_uptr
      m_sub;  ///<  points to a MatchAstNode if the input is a contained Op; empty otherwise
  operand_tag_t m_optag;  ///<   m_optag contains the operand tag if the input is not a contained op
                          ///<   ; "" otherwise
 public:
...
class MatchAstNode {
 ...
 protected:
  ...
  API_EXPORT std::unique_ptr<char[]> make_debug_desc(
      std::map<operand_tag_t, int> const &opertag_to_idx_map) const;

 public:
  API_EXPORT MatchAstNode(char const *, char const *, node_variant, int n_subnodes,
                          MatchAstSubnode *subnodes);
};
...
class MatchBuilder {
 protected:
  ...
  template <typename... S>
  using are_strings = std::conjunction<std::is_same<const char *, S>...>;

  template <typename... S>
  using have_matchast_uptr = std::disjunction<std::is_same<hnnx::MatchAst_uptr, S>...>;

  // just convert each 'input' to a MatchAstSubnode, and build a MatchAstNode from that.
  template <typename... Ts>
  API_EXPORT static
      typename std::enable_if<have_matchast_uptr<Ts...>::value, hnnx::MatchAst_uptr>::type
      Op(char const *opname, Ts &&...ts) {
    std::array<hnnx::MatchAstSubnode, sizeof...(Ts)> subnodes = {
        std::move(std::forward<Ts>(ts))...};
  ...
  /// \ingroup OptMatch
  /// @brief define an Op to match, with at least the specified number of inputs: OpVarIn("opname",
  /// ..inputs..) This will match the same as Op, but will also accept additional (unspecified)
  /// inputs. The number of inputs may be zero: OpVarIn("opname") matches any op "opname".
  ///
  template <typename... Ts>
  API_EXPORT static
      typename std::enable_if<have_matchast_uptr<Ts...>::value, hnnx::MatchAst_uptr>::type
    ...
    std::array<hnnx::MatchAstSubnode, sizeof...(Ts)> subnodes = {
        std::move(std::forward<Ts>(ts))...};
    return std::make_unique<hnnx::MatchAstNode>(
        opname, pkg_flag.c_str(), hnnx::MatchAstNode::is_OpVarIn, sizeof...(Ts), subnodes.data());
  }
  ...
  ///
  API_EXPORT static hnnx::MatchAst_uptr LET(hnnx::operand_tag_parm_t optag,
                                            hnnx::MatchAst_uptr &&subnode) {
  ...
  //
  API_EXPORT static void implement_LET(hnnx::operand_tag_parm_t optag,
                                       hnnx::MatchAst_uptr &subnode);

 public:
  template <typename T>
  API_EXPORT static hnnx::MatchAst_uptr matcher();
