...
inline unsigned constexpr round_up(unsigned x, unsigned m) { return ((x + (m - 1)) / m) * m; }
...
inline constexpr std::pair<unsigned, unsigned> typical_op_tgt_size_align(unsigned n_in,
                                                                         unsigned n_out) {
  // 1 pointer per input, plus tensor_uptr_ptrs per output; but if n_in = n_out == 0, it's 1
  // pointer. (for a 'fill' byte).
...
// 'tensor_op_tgt_size_align is used for crate accounting of ShapeWrapperOp, ConstWrapperOp,
// DummyOp<N> In a proper 'baked graph' we don't need to insert these, just the tensors...
...
//
inline constexpr std::tuple<unsigned, unsigned, unsigned> typical_op_extra_tgt_size_align(
    unsigned n_in, unsigned n_out, unsigned extra_len, unsigned extra_align) {
...
constexpr std::pair<unsigned, unsigned> simplewrap_op_tgt_size_align(unsigned n_in,
                                                                     unsigned n_out) {
...
constexpr std::pair<unsigned, unsigned> shape_tgt_size_align(unsigned rank) {
  ...
  return {round_up(tgt_sizet_bytes * (1 + 1 + 1 + 2 * rank) + rank, tgt_sizet_bytes),
          tgt_sizet_bytes};
}
...
constexpr std::pair<unsigned, unsigned> dynamic_shape_tgt_size_align(const unsigned rank) {
  ...
  return {round_up(tgt_sizet_bytes * rank + (4 * tgt_sizet_bytes), tgt_sizet_bytes),
          tgt_sizet_bytes};
}
...
template <typename T, typename SZAL>
constexpr bool size_align_matches(SZAL sz) {
...
// Just need to include a call to 'do-nothing' bake::check_size_align<T>::template
// check<SIZE,ALIGN>(); The static assert is *disabled* unless compiling on hexagon (or compatible
// host).
//
