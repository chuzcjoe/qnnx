...
#define IMPL_APPEND_REG_OP_ELEM_NO_TCM_FOLDING(I, FP, OP, TAG, IS_EXTERNAL)                        \
  ...
  template <>                                                                                      \
  constexpr int32_t NC<(I)>::inc_op() noexcept {                                                   \
    return 1;                                                                                      \
  }                                                                                                \
                                                                                                   \
  ...
  constexpr bool StrUpd<(I)>::is_new_op_name =                                                     \
      make_string_view(op_name_chain<UniqTy<0>, I - 1>().get_arr())                                \
          .rfind(std::string_view{(OP), sizeof(OP)}) == std::string_view::npos;                    \
                                                                                                   \
  /** @brief Whether the name for this type suffix is already present in the type suffix string    \
   * table */                                                                                      \
  template <>                                                                                      \
  ...
  template <>                                                                                      \
  constexpr uint64_t NC<(I)>::inc_op_name_strtab_size() noexcept {                                 \
    if (StrUpd<(I)>::is_new_op_name) {                                                             \
      ...
      return 0U;                                                                                   \
                                                                                                   ...
                                                                                                   \
  /** @brief Update the size of the type suffix string table for this file. @return 0 or           \
   * TAG.size() */                                                                                 \
  template <>                                                                                      \
  constexpr uint64_t NC<(I)>::inc_type_tag_strtab_size() noexcept {                                \
  ...
  template <>                                                                                      \
  template <>                                                                                      \
  constexpr ba_str<NC<(I)>::op_name_strtab_size()> op_name_strtab_container::chain<                \
      UniqTy<0>, NC<(I)>::reg_op_count(), NC<(I)>::op_name_strtab_size()> =                        \
  ...
  constexpr uint16_t StrUpd<(I)>::op_name_offset =                                                 \
      static_cast<uint16_t>(make_string_view(op_name_chain<UniqTy<0>, I>().get_arr())              \
                                .rfind(std::string_view{(OP), sizeof(OP)}));                       \
                                                                                                   \
  /** @brief Grow the type suffix string table for this file. No-op if it already contains the     \
   * string */                                                                                     \
  template <>                                                                                      \
  template <>                                                                                      \
  constexpr ba_str<NC<(I)>::type_tag_strtab_size()> type_tag_strtab_container::chain<              \
      UniqTy<0>, NC<(I)>::reg_op_count(), NC<(I)>::type_tag_strtab_size()> =                       \
  ...
  template <>                                                                                      \
  template <>                                                                                      \
  constexpr ba_op<NC<(I)>::reg_op_count()>                                                         \
      op_arr_container::chain<UniqTy<0>, NC<(I)>::reg_op_count()> =                                \
...
#define IMPL_APPEND_REG_OP_ELEM(I, FP, OP, TAG, LINE)                                              \
  ...
  constexpr bool StrUpd<(I)>::is_new_op_name =                                                     \
      make_string_view(op_name_chain<UniqTy<0>, I - 1>().get_arr())                                \
          .find(std::string_view{(OP), sizeof(OP)}) == std::string_view::npos;                     \
                                                                                                   \
  /** @brief Whether the name for this type suffix is already present in the type suffix string    \
   * table */                                                                                      \
  template <>                                                                                      \
  ...
  constexpr uint16_t StrUpd<(I)>::op_name_offset =                                                 \
      static_cast<uint16_t>(make_string_view(op_name_chain<UniqTy<0>, I>().get_arr())              \
                                .find(std::string_view{(OP), sizeof(OP)}));                        \
                                                                                                   \
  /** @brief Grow the type suffix string table for this file. No-op if it already contains the     \
   * string */                                                                                     \
  template <>                                                                                      \
  ...
  constexpr ba_op<NC<(I)>::reg_op_count()>                                                         \
      op_arr_container::chain<UniqTy<0>, NC<(I)>::reg_op_count()> =                                \
          chain<UniqTy<0>, NC<(I - 1)>::reg_op_count()>.append(hnnx::reg_op_node{                  \
              hnnx::GetParms::get<fold::ModifiedDerivedType<FP, LINE>::Modified, I>(),             \
              StrUpd<(I)>::op_name_offset, StrUpd<(I)>::type_tag_offset, false});

/** @brief APPEND_REG_OP_ELEM (used by REGISTER_OP, REGISTER_OP_HVX, etc.) */
#define APPEND_REG_OP_ELEM(FP, OP, TAG, LINE) \
  IMPL_APPEND_REG_OP_ELEM(__COUNTER__, FP, OP, TAG, LINE)
...
#define IMPL_APPEND_REG_OPT_ELEM(I, PRIORITY, FLAGS, DEFOPTFN, LINE)                  \
  ...
  template <>                                                                         \
  constexpr int32_t NC<(I)>::inc_opt() noexcept {                                     \
    return 1;                                                                         \
  }                                                                                   \
                                                                                      \
  ...
  template <>                                                                         \
  template <>                                                                         \
  constexpr ba_opt<NC<(I)>::reg_opt_count()>                                          \
      opt_arr_container::chain<UniqTy<0>, NC<(I)>::reg_opt_count()> =                 \
...
#define IMPL_INITIALIZE_TABLES(COUNT)                                        \
  ...
  namespace {                                                                \
  template <int32_t I>                                                       \
  using NC = NodeCounter<UniqTy<0>, I>;                                      \
  template <int32_t I>                                                       \
  using StrUpd = StrtabUpdate<UniqTy<0>, I>;                                 \
  }                                                                          \
  template <>                                                                \
  constexpr int32_t NC<(COUNT)>::reg_op_count() noexcept {                   \
    return 0;                                                                \
  }                                                                          \
  template <>                                                                \
  constexpr int32_t NC<(COUNT)>::reg_opt_count() noexcept {                  \
    return 0;                                                                \
  }                                                                          \
  template <>                                                                \
  constexpr uint64_t NC<(COUNT)>::op_name_strtab_size() noexcept {           \
    return 0U;                                                               \
  }                                                                          \
  template <>                                                                \
  constexpr uint64_t NC<(COUNT)>::type_tag_strtab_size() noexcept {          \
    return 0U;                                                               \
  }                                                                          \
  template <>                                                                \
  template <>                                                                \
  constexpr ba_op<0> op_arr_container::chain<UniqTy<0>, 0> = {};             \
  template <>                                                                \
  template <>                                                                \
  constexpr ba_opt<0> opt_arr_container::chain<UniqTy<0>, 0> = {};           \
  template <>                                                                \
  template <>                                                                \
  constexpr ba_str<0> op_name_strtab_container::chain<UniqTy<0>, 0, 0> = {}; \
  template <>                                                                \
  template <>                                                                \
  constexpr ba_str<0> type_tag_strtab_container::chain<UniqTy<0>, 0, 0> = {};
...
#define IMPL_FINALIZE_TABLES(COUNT, NAME)                                                                                                                            \
  ...
  constexpr auto OPS_REG_TABLE(NAME) =                                                                                                                               \
      op_arr_container::chain<UniqTy<0>, NC<(COUNT)>::reg_op_count()>.get_arr();                                                                                     \
  ...
  constexpr auto OPTS_REG_TABLE(NAME) =                                                                                                                              \
      opt_arr_container::chain<UniqTy<0>, NC<(COUNT)>::reg_opt_count()>.get_arr();                                                                                   \
  }                                                                                                                                                                  \
  namespace hnnx {                                                                                                                                                   \
  /** @brief Exported getter function for the Op registration table, its associated string tables,                                                                   \
   * and their sizes */                                                                                                                                              \
  ...
  extern "C" reg_opt_table const *EXT_OPTS_REG_TABLE(NAME)() {                                                                                                       \
    static constexpr reg_opt_table table{                                                                                                                            \
        OPTS_REG_TABLE(NAME).size() ? &OPTS_REG_TABLE(NAME).front() : nullptr,                                                                                       \
...
#define OPS_TABLE_WEAK_SYMBOL(NAME) \
  DECLARE_CLANG_WEAK_SYMBOL(reg_op_table, EXT_OPS_REG_TABLE(NAME), EMPTY_OPS_TABLE)
#define OPTS_TABLE_WEAK_SYMBOL(NAME) \
...
#define IMPL_BEGIN_OPS_OPTS_LIST(I)                                              \
  ...
  template <>                                                                    \
  template <>                                                                    \
  constexpr ba_op_table<0> op_table_arr_container::chain<UniqTy<0>, (I)> = {};   \
  template <>                                                                    \
  template <>                                                                    \
  constexpr ba_opt_table<0> opt_table_arr_container::chain<UniqTy<0>, (I)> = {}; \
  }
...
#define IMPL_END_OPS_OPTS_LIST(PREFIX, COUNT)                                                     \
  ...
  auto const PREFIX##_op_package_ops_list =                                                       \
      hnnx::op_table_arr_container::chain<UniqTy<0>, (COUNT)>.get_arr();                          \
                                                                                                  \
  /** @brief PREFIX_op_package_opts_list references all of the registered graph optimizations for \
   * the HTP Core. */                                                                             \
  auto const PREFIX##_op_package_opts_list =                                                      \
      hnnx::opt_table_arr_container::chain<UniqTy<0>, (COUNT)>.get_arr();                         \
  }
...
#define END_OPS_OPTS_LIST()                                                                      \
  ...
  /** This is done to avoid init-time regressions when Graph::init_once() is called during graph \
   * creation. */                                                                                \
  /** NOTE: OpPackages register in their init functions rather than at static-init time (See     \
   * op_register_ext.h) */                                                                       \
  [[maybe_unused]] static bool core_REGISTER_OPS_AND_OPTS =                                      \
      (hnnx::core_ops_opts_registration(), true);
 ...
 *  (only the null sentinel node is present). Op source files will then override these weak
 * definitions with strong ones containing the actual list.
 */
