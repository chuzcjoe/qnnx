...
template <>
struct findhash<unsigned long long> {
...
struct findhash<unsigned long>
    : public findhash<std::conditional<(sizeof(unsigned long) > sizeof(unsigned)),
                                       unsigned long long, unsigned>::type> {};
...
inline uint32_t findhash_sizet(size_t val) { return findhash<size_t>()(val); }
//
// define for T*
//
template <typename T>
...
template <>
struct findhash<int> : public findhash<unsigned> {};
template <>
struct findhash<long> : public findhash<unsigned long> {};
template <>
struct findhash<long long> : public findhash<unsigned long long> {};
...
template <typename Key, bool ERASE_OK>
struct hashmap_traits {  // defaults for ERASE_OK=true
  static constexpr bool valid = std::is_trivial<Key>::value;
};

template <typename Key>
struct hashmap_traits<Key, false> {  // defaults for ERASE_OK=false
...
struct simple_raw_entry : public simple_key<Key> {
  ...
  template <class... Args>
  inline void emplace_new(Key k, Args &&...args) {
    XASSERT(!is_null(k) && !this->is_inuse());
    this->first = k;
...
template <typename Key>
struct simple_raw_entry<Key, no_value> : public simple_key<Key> {
  static inline bool is_null(Key k) { return simple_key<Key>::traits::is_null(k); }
  simple_raw_entry() : simple_key<Key>() {}
  inline void destroy() {}
...
class hashmap {
  ...
  typedef typename std::conditional<ERASE_OK, erasable_raw_entry<Key, T>,
                                    simple_raw_entry<Key, T>>::type raw_entry_t;
  ...
  typename std::conditional<ERASE_OK, size_t, stuck_at_0<size_t>>::type
      m_deleted;  // deleted entries (if ERASE_OK)
 ...
 public:
  ...
  static_assert(ERASE_OK || sizeof(raw_entry_t) == sizeof(value_type),
                "failed to make compatible layout");
 ...
 private:
  ...
  class hmap_kiterator {
   ...
   protected:
    hashmap *m_object;
    int m_posn;
    hmap_kiterator(hashmap const *obj, int posn)
        : m_object(const_cast<hashmap *>(obj)), m_posn(posn) {}
   ...
   public:
    ...
    value_type const &operator*() const {
      return *reinterpret_cast<value_type *>(&m_object->m_table[m_posn]);
    }
    value_type const *operator->() const {
      return reinterpret_cast<value_type *>(&m_object->m_table[m_posn]);
    }
    hmap_kiterator &operator++() {  // pre-inc
      m_posn = m_object->find_next_for_iter(m_posn);
      return *this;
    }

    hmap_kiterator operator++(int) {  // post-inc
  ...
  class hmap_iterator : public hmap_kiterator {
   ...
   protected:
    ...
    hmap_iterator coerce_from(typename otherhash::hmap_kiterator const &other) {
   ...
   public:
    ...
    value_type &operator*() const {
      return const_cast<value_type &>(this->hmap_kiterator::operator*());
    }
    value_type *operator->() const {
      return const_cast<value_type *>(this->hmap_kiterator::operator->());
    }
    hmap_iterator &operator++() {  // pre-inc
      return static_cast<hmap_iterator &>(this->hmap_kiterator::operator++());
    }
    hmap_iterator operator++(int) {  // post-inc
 ...
 protected:
  template <typename otherhash>
  static hmap_iterator coerce_iter(typename otherhash::hmap_kiterator const &other) {
    return hmap_iterator((hashmap *)other.m_object, other.m_posn);
  }
 ...
 public:
  ...
  const_iterator cbegin() const {
    return hmap_kiterator{const_cast<hashmap *>(this), find_next_for_iter(m_hashN)};
  }
  ...
  template <class... Args>
  std::pair<iterator, bool> emplace(Key const &k, Args &&...args) {
  ...
  template <class... Args>
  std::pair<iterator, bool> try_emplace(Key const &k, Args &&...args) {
  ...
  T &operator[](Key const &k) {
    static_assert(std::is_constructible<T>::value,
                  "map[] requires a null-constructible mapped_type");
    table_iter_type const titer = find_key_for_ins(k);
    if (!titer->is_inuse()) {
 ...
 protected:
  ...
  template <bool INSERT = true>
  table_iter_type lookup_key_template(Key const &k) {
    size_t const mask = m_hashN - 1;
    uint32_t const hsh = HSH()(k);
    size_t probe_at = hsh & mask;
    size_t const delta = (hsh >> 15) | 1;  // must be odd.
    // printf("       ...probing %llu in %zu at %zu, span=%zu", (unsigned long long)k,
    // mask+1,probe_at,delta&mask);
  ...
  void rehash_table(bool growing) {
    ...
    int const new_log2 =
        growing ? (ceiling_log2(sz | 15) + 2) : (ceiling_log2(sz + 3 + (sz >> 1)) + 1);
...
template <typename Key, bool ERASE_OK,
          typename HSH = findhash<typename value_proxy<Key, true>::type>>
class hashset : private hashmap<typename value_proxy<Key, true>::type, no_value, ERASE_OK, HSH> {
 ...
 protected:
  static inline iterator iter_up(impl_iterator const &it) {
    return hashk::template coerce_iter<hashimpl>(it);
  }
  static inline const_iterator iter_up(impl_kiterator const &it) {
    return hashk::template coerce_iter<hashimpl>(it);
  }
  static inline impl_iterator iter_down(iterator const &it) {
    return hashimpl::template coerce_iter<hashk>(it);
  }
...
using minihash_noerase = minObj::hashmap<K, T, false>;
template <typename K, typename T>
using minihash = minObj::hashmap<K, T, true>;

template <typename K>
using miniset_noerase = minObj::hashset<K, false>;
template <typename K>
using miniset = minObj::hashset<K, true>;
