...
struct Rename {
  ...
  void update(First first, Rest... rest) {
    update1(first);
    update(rest...);
  }
  template <typename... Flags>
  explicit Rename(Flags... flags_) {
    update(flags_...);
  }
};
...
struct Op : public StringList {
  template <typename... Args>
  explicit Op(Args... args) : StringList(args...) {}
};
struct Outputs : public StringList {
  template <typename... Args>
  explicit Outputs(Args... args) : StringList(args...) {}
};
struct Fixed : public StringList {
  template <typename... Args>
  explicit Fixed(Args... args) : StringList(args...) {}
};
struct Crouton : public StringList {
  template <typename... Args>
  explicit Crouton(Args... args) : StringList(args...) {}
};
struct Flat : public StringList {
  template <typename... Args>
  explicit Flat(Args... args) : StringList(args...) {}
};
struct MainMemory : public StringList {
  template <typename... Args>
  explicit MainMemory(Args... args) : StringList(args...) {}
};
struct Tcm : public StringList {
  template <typename... Args>
  explicit Tcm(Args... args) : StringList(args...) {}
};
struct PreferTcm : public StringList {
  template <typename... Args>
  explicit PreferTcm(Args... args) : StringList(args...) {}
};

struct FixedConstant : public StringList {
  template <typename... Args>
  explicit FixedConstant(Args... args) : StringList(args...) {}
};

struct UnalignedOk : public StringList {
  template <typename... Args>
  explicit UnalignedOk(Args... args) : StringList(args...) {}
};
struct WhenFitsIfTCM : public StringList {
  template <typename... Args>
  explicit WhenFitsIfTCM(Args... args) : StringList(args...) {}
};
struct CroutonIf : public StringList {
  template <typename... Args>
  explicit CroutonIf(Args... args) : StringList(args...) {}
};
...
struct TensorInfoBuilder : public hnnx::TensorInfo {
  ...
  inline void update1(Op names_p) {
    ...
    assert((std::all_of(names.begin(), names.end(),
  ...
  inline void update1(const Fixed fixed_p) {
    ...
    const unsigned really_fixed =
        fixed_names & ~(is_flat | is_crouton | is_tcm | prefer_tcm | is_main_memory);
  ...
  template <typename... Args>
  TensorInfoBuilder(const char *package_, Args... args) : package(package_) {
    update(args...);
  }
...
namespace hnnx {
// In tcm_migration.cc
extern API_FUNC_EXPORT bool register_tensor_properties(const char *package,
                                                       DefProperties::TensorInfoBuilder);
}  // namespace hnnx
