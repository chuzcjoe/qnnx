...
// This is called by its typedef 'hnnx::op_slice_spec' everywhere; it has a short name hnnx::OsS
// since it's mangled into all the Op execute method names. The parameter must be the last one
// (unless there is a Graph const &, in which case it's before that).
struct OsS {
 protected:
  // on hexagon:
...
class API_EXPORT Executable {
 public:
  ...
  virtual GraphStatus execute(
      EXECUTE_METHOD_PARMS) const noexcept = 0;  // Needs to be at vtable offset zero!!!
  virtual ItemType compile(
      Graph &graph_in) const;  // Turn this Executable into a function pointer and data pointer.
  virtual bool check_constraint_for_recompile(
      Graph &graph_in) const;  // allows for additional check for running during recompile
  virtual ~Executable() = default;
  static const size_t *vtable(Executable const *);  // helper function: get vtable
  static size_t execute_address(
      Executable const *);  // helper function: get address of execute() function
