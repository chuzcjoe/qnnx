 ...
 *        to QNN Operation.
 */
typedef enum { QNN_OP_MAPPING_TYPE_TENSOR = 0, QNN_OP_MAPPING_TYPE_OP = 1 } Qnn_MappingType_t;
...
typedef enum {
  ...
  /// Qnn_BwAxisScaleOffsetMapped_t. Support
  /// can be checked via
  /// QNN_PROPERTY_TENSOR_SUPPORT_QUANTIZATION_ENCODING_BW_AXIS_SCALE_OFFSET_MAPPED.
  QNN_QUANTIZATION_ENCODING_BW_AXIS_SCALE_OFFSET_MAPPED = 8,
  /// Indicates per-block scale-offset, bw, mapped encoding type. See Qnn_BwBlockMapped_t. Support
  /// can be
  ...
  /// Indicates per-block scale-offset encoding type. See Qnn_BwBlockwiseExpansionMapped_t. Support
  /// can be
  ...
  /// Qnn_BwFloatBlockEncoding_t. Support
...
typedef struct {
  ...
  /// Size of the block scaling storage, must be able to store at least blockScaleBitwidth sized
  /// values.
  Qnn_BlockwiseExpansionBlockScaleStorageType_t blockScaleStorageType;
  union UNNAMED {
    /// A contiguous array of block scalings of size axisSize*numBlocksPerAxis. The array is laid
    /// out such that an element can be accessed via
    /// blocksScale8[axisIter*numBlocksPerAxis+blockIter]. Used when blockStorageSize is
    /// QNN_BLOCKWISE_EXPANSION_BITWIDTH_SCALE_STORAGE_8.
    uint8_t* blocksScale8;
    /// A contiguous array of block scalings of size axisSize*numBlocksPerAxis. The array is laid
    /// out such that an element can be accessed via
    /// blocksScale16[axisIter*numBlocksPerAxis+blockIter]. Used when blockStorageSize is
...
typedef struct {
  ...
  /// The dimension of the vector encoding. e.g 1D,2D,3D... for 1, 2 or 3 weights per index,
  /// respectively
...
/**
 * @brief A struct to express float block quantization parameters. A tensor is divided into blocks
 *of size blockSize, where blockSize is an array of length rank.
 *
...
/**
 * @brief A struct to express bit-width float block quantization parameters. A tensor is divided
 *into blocks of size blockSize, where blockSize is an array of length rank.
 *
 * * @note num of floatScaleOffsets (i.e. num of blocks) must be ==
 * ceil(dimensions[0]/blockSize[0])*ceil(dimensions[1]/blockSize[1]) ...
 * .... *ceil(dimensions[rank-1] / blockSize[rank-1]).
...
typedef struct {
  ...
  union UNNAMED {
    /// Used when quantizationEncoding is QNN_QUANTIZATION_ENCODING_SCALE_OFFSET. Note that this
    /// field is a value.
    Qnn_ScaleOffset_t scaleOffsetEncoding;
    /// Used when quantizationEncoding is QNN_QUANTIZATION_ENCODING_AXIS_SCALE_OFFSET. Note that
    /// this field is a value.
    Qnn_AxisScaleOffset_t axisScaleOffsetEncoding;
    /// Used when quantizationEncoding is QNN_QUANTIZATION_ENCODING_BW_SCALE_OFFSET. Note that this
    /// field is a value.
    Qnn_BwScaleOffset_t bwScaleOffsetEncoding;
    /// Used when quantizationEncoding is QNN_QUANTIZATION_ENCODING_BW_AXIS_SCALE_OFFSET. Note that
    /// this field is a value.
    Qnn_BwAxisScaleOffset_t bwAxisScaleOffsetEncoding;
    /// Used when quantizationEncoding is QNN_QUANTIZATION_ENCODING_BLOCK. Note that this field is a
    /// value.
    Qnn_BlockEncoding_t blockEncoding;
    /// Used when quantizationEncoding is QNN_QUANTIZATION_ENCODING_BLOCKWISE_EXPANSION. Note that
    /// this field is a pointer.
    Qnn_BlockwiseExpansion_t* blockwiseExpansion;
    /// Used when quantizationEncoding is QNN_QUANTIZATION_ENCODING_VECTOR. Note that this field is
    /// a pointer.
    Qnn_VectorEncoding_t* vectorEncoding;
    /// Used when quantizationEncoding is QNN_QUANTIZATION_ENCODING_FLOAT_BLOCK. Note that this
    /// field is a value.
    Qnn_FloatBlockEncoding_t floatBlockEncoding;
    /// Used when quantizationEncoding is QNN_QUANTIZATION_ENCODING_BW_AXIS_SCALE_OFFSET_MAPPED.
    /// Note that this field is a pointer.
    Qnn_BwAxisScaleOffsetMapped_t* bwAxisScaleOffsetMappedEncoding;
    /// Used when quantizationEncoding is QNN_QUANTIZATION_ENCODING_BW_BLOCK_MAPPED. Note that this
    /// field is a pointer.
    Qnn_BwBlockMapped_t* bwBlockMappedEncoding;
    /// Used when quantizationEncoding is QNN_QUANTIZATION_ENCODING_BW_BLOCKWISE_EXPANSION_MAPPED.
    /// Note that this field is a pointer.
    Qnn_BwBlockwiseExpansionMapped_t* bwBlockwiseExpansionMappedEncoding;
    /// Used when quantizationEncoding is QNN_QUANTIZATION_ENCODING_BW_FLOAT_BLOCK. Note that this
    /// field is a value.
...
typedef Qnn_ErrorHandle_t (*Qnn_GetTensorRawDataFn_t)(Qnn_ContextHandle_t context,
                                                      Qnn_GraphHandle_t graph, uint64_t id,
