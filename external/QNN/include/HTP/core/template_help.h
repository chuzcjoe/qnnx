...
template <template <typename> typename Tname>
struct TemplateTypeWrapper {};

template <template <size_t> typename Tname>
struct TemplateIdxWrapper {};

template <typename twrap, size_t val>
struct UnwrapIdxTemplate_struct {};
...
template <typename Twrap, size_t Val>
using UnwrapIdxTemplate = typename UnwrapIdxTemplate_struct<Twrap, Val>::type;

template <typename twrap, typename tapply>
struct UnwrapTypeTemplate_struct {};
 ...
 */
template <template <typename...> typename C, typename T, typename... Rest>
struct TupleCons<T, C<Rest...>> {
  using type = C<T, Rest...>;
};

template <template <typename> class Pred, template <typename> class Wrapper, typename...>
struct TypeFilter;
...
struct TypeFilter<Pred, Wrapper, Head> {
  using type =
      std::conditional_t<Pred<std::remove_reference_t<std::remove_pointer_t<Head>>>::value,
                         std::tuple<Wrapper<std::remove_reference_t<Head>>>,  // FIXME: remove
                                                                              // remove_reference_t
                         std::tuple<>>;
};
/*
...
struct TypeFilter<Pred, Wrapper, Head, Tail...> {
  using type = std::conditional_t<
      Pred<std::remove_reference_t<std::remove_pointer_t<Head>>>::value,
...
struct TupFilter;
// template<template<typename> class Pred, template<typename> class Wrapper, typename...> struct
// TupFilter;
...
template <template <typename> typename Pred, template <typename...> typename C>
struct TupFilter<Pred, C<>> {
  using type = C<>;
};
...
template <template <typename> class Wrap, typename...>
struct TupMap;

template <template <typename> typename Wrap, template <typename...> typename C>
struct TupMap<Wrap, C<>> {
  using type = C<>;
};
...
#else
template <template <typename> class Wrap, typename Tup>
using TupMap_t = typename TupMap<Wrap, Tup>::type;

template <template <typename> class Filt, typename Tup>
using TupFilter_t = typename TupFilter<Filt, Tup>::type;
#endif
...
template <typename T>
struct Unboxed<const Vector<T>> {
  using type = T;
};
template <typename T>
struct Unboxed<Vector<T>> {
  using type = T;
};
...
template <typename T>
using unboxed_t = typename Unboxed<T>::type;

template <class T>
using is_not_const =
    std::integral_constant<bool, !std::is_const<std::remove_pointer_t<unboxed_t<T>>>::value>;
template <class T>
using is_const =
    std::integral_constant<bool, std::is_const<std::remove_pointer_t<unboxed_t<T>>>::value>;
...
template <template <typename...> typename C, typename Default>
struct First_Tuple_Element<C<>, Default> {
  using type = Default;
};

template <typename T, typename Default>
using first_tuple_element = typename First_Tuple_Element<T, Default>::type;
...
constexpr static inline const std::array<T, N> ptr_to_stdarray_helper(const T *carray,
...
constexpr static inline const std::array<T, N> ptr_to_stdarray(const T *carray) {
...
template <typename T>
struct add_uniqueptr {
  using type = typename std::unique_ptr<T>;
};
template <class T>
using add_uniqueptr_t = typename add_uniqueptr<T>::type;
...
template <typename T>
struct OpArgCat {
  // if it's a subclass of GraphHandleBase (without actually being GraphHandleBase) it's
  // 'graph_handle' otherwise invalid.
  static constexpr OpArgCategory value = (std::is_base_of_v<hnnx::GraphHandleBase, T> &&
                                          !std::is_same_v<const hnnx::GraphHandleBase, const T>)
...
//
template <typename T>
struct OpArgCat<Vector<T *> const &> {
  static constexpr OpArgCategory value = !std::is_base_of_v<Tensor, T> ? OpArgCategory::invalid
                                         : std::is_const_v<T>          ? OpArgCategory::vararg_in
                                                                       : OpArgCategory::vararg_out;
};
template <typename T>
struct OpArgCat<Vector<T *>> : public OpArgCat<Vector<T *> const &> {};
...
//  - Each one's category must be >= the previous category, and can only be equal if it's
//  'tensor_out' or 'tensor_in'.
//    The first one's previous is considered to be 'invalid', which is < all valid.
//  EXCEPT:
//  - New category of tensor_out is allowed after any > tensor_out and < slice_spec; this is counted
//  as the first 'scratch output'.
//  - if at least one 'scratch output' has been seen before, the category must be either
//  'tensor_out' (which is counted), or >= slice_spec.
//
// LCOV_EXCL_START [SAFTYSWCCB-1736] constexprs resolved during compile time
// used in locally with  constexpr lvalue
template <OpArgCategory... Args>
inline constexpr int CheckOpFuncArgs() {
  ...
  if constexpr (N > 0) {
    ...
    for (unsigned i = 0; i < N; i++) {
      ...
      } else if (cat == cat_previous && cat != OpArgCategory::tensor_in &&
                 cat != OpArgCategory::tensor_out) {
 ...
 */
template <typename... T>
using Concat = typename Concat_struct<T...>::type;

//////////
// ArgTupFilter_t<CAT, Args...> -> tuple<Args...> with only ops of given cat removed.
// Also, refs are removed.
//
template <typename T1, typename TUP>
struct TupleBuild {};
template <typename T1, typename... Types>
struct TupleBuild<T1, std::tuple<Types...>> {
  using type = std::tuple<T1, Types...>;
};

template <OpArgCategory CAT, typename... Types>
struct ArgTupFilterHelper {};

template <OpArgCategory CAT, typename T1, typename... Types>
struct ArgTupFilterHelper<CAT, T1, Types...> {
 private:
  using tail = typename ArgTupFilterHelper<CAT, Types...>::type;

 public:
  using type =
      std::conditional_t<OpArgCat<T1>::value == CAT,  // is T1 included?
                         typename TupleBuild<std::remove_reference_t<T1>, tail>::type, tail>;
};

// just one...
template <OpArgCategory CAT, typename T1>
struct ArgTupFilterHelper<CAT, T1> {
  using type = std::conditional_t<OpArgCat<T1>::value == CAT,
                                  std::tuple<std::remove_reference_t<T1>>, std::tuple<>>;
};

// empty case...
template <OpArgCategory CAT>
struct ArgTupFilterHelper<CAT> {
  using type = std::tuple<>;
};

template <OpArgCategory CAT, typename... Types>
using ArgTupFilter_t = typename ArgTupFilterHelper<CAT, Types...>::type;
...
struct CheckGraphHandleTuple;
template <>
struct CheckGraphHandleTuple<std::tuple<>> {
  using type = std::tuple<>;
};
template <typename GH>
struct CheckGraphHandleTuple<std::tuple<GH>> {
...
struct ArgsTuples<R(Args...)> {
 ...
 public:
  // If this is > 0, then that many of the 'output' (the last of them) are actually 'scratch'
  // output, they are counted in n_outputs. If not supported in VariadicOp, this must be checked
  // there.
  static constexpr size_t n_scratch_outputs =
      (check_op_func_val <= 0) ? size_t(0) : size_t(check_op_func_val);
  ...
  using input_tuple =
      ArgTupFilter_t<OpArgCategory::tensor_in, Args...>;  // the inputs as real types
  using output_tuple =
      ArgTupFilter_t<OpArgCategory::tensor_out, Args...>;  // the outputs as real types
  using var_input_tuple =
      ArgTupFilter_t<OpArgCategory::vararg_in, Args...>;  // variadic input tuple
  using var_output_tuple =
      ArgTupFilter_t<OpArgCategory::vararg_out, Args...>;  // variadic output tuple
  ...
  static constexpr bool has_graph =
      (std::tuple_size<graph_handle_tup>::value > 0);  // does it have a graph operand?
  static constexpr bool has_slice_spec =
      (std::tuple_size<slice_spec_tup>::value > 0);  // has op_slice_spec?
  ...
  // Note: may or may not add graph_handle_tup to the end, depending on what actual handle type it
  // is (see CheckGraphHandleTuple).
  ...
  // a string in the form of "@t1.t2.t3"... where t1,t2,t3,etc are the typenames of the input
  // arguments
  //  as defined by DEFINE_TYPENAME
  static constexpr auto nameArray = GetTypeNames<tname_args_tuple>(
      std::make_index_sequence<std::tuple_size_v<tname_args_tuple>>{});
  static constexpr const char *inputTypeNames = nameArray.data();
};

template <auto F>
struct ArgsTuples2 : public ArgsTuples<std::remove_pointer_t<decltype(F)>> {};
...
struct contains_type<std::tuple<T, TX...>, T> {
  static const bool value = true;
};
template <typename TA, typename... TX, typename T>
struct contains_type<std::tuple<TA, TX...>, T> {
  static const bool value = contains_type<std::tuple<TX...>, T>::value;
};
template <typename TUPLET, typename T>
struct not_contains_type {
...
struct Concat_struct<C<As...>> {
  using type = C<As...>;
};

template <template <typename...> typename C, typename... As, typename... Bs>
struct Concat_struct<C<As...>, C<Bs...>> {
  using type = C<As..., Bs...>;
};
...
template <typename W, typename X, typename Y, typename Z, typename... More>
struct Concat_struct<W, X, Y, Z, More...> {
  using type = typename Concat_struct<typename Concat_struct<W, X, Y>::type,
                                      typename Concat_struct<Z, More...>::type>::type;
};

#if !defined(NDEBUG)
static_assert(std::is_same_v<Concat<std::tuple<int>, std::tuple<char, float>, std::tuple<>,
                                    std::tuple<bool>, std::tuple<void *, int *>>,
                             std::tuple<int, char, float, bool, void *, int *>>);
static_assert(std::is_same_v<Concat<std::tuple<>, std::tuple<float, char>, std::tuple<int, char *>,
                                    std::tuple<bool>>,
...
struct pair_to_iterators : std::pair<IterT, IterT> {
  pair_to_iterators(std::pair<IterT, IterT> const &&iter_pair_in)
      : std::pair<IterT, IterT>(std::move(iter_pair_in)) {}
  pair_to_iterators(std::pair<IterT, IterT> const &iter_pair_in)
      : std::pair<IterT, IterT>(iter_pair_in) {}
...
bool insert_ordered_no_dups(std::vector<T, Allocator> &vec, T const &value) {
...
inline T const *array_search_ordered(T const *arr, int n, K const &k, EXTRACTOR extr) {
  ...
  if constexpr (LTHR > 0) {
    ...
    while (p < p_end && extr(*p) < k) ++p;
  }
  return p;
}

// same with no EXTRACTOR
template <bool CHECK_EQ, int LIN_THR, typename T>
T const *array_search_ordered(T const *arr, int n, T const &k) {
...
T *array_search_ordered(T *arr, int n, K const &k, EXTRACTOR extr) {
  return const_cast<T *>(array_search_ordered<CHECK_EQ, LIN_THR, EXTRACTOR, T>(
      const_cast<T const *>(arr), n, k, extr));
}
// with T *, no extractor
template <bool CHECK_EQ, int LIN_THR, typename T>
T *array_search_ordered(T *arr, int n, T const &k) {
  return const_cast<T *>(array_search_ordered<CHECK_EQ, LIN_THR>(const_cast<T const *>(arr), n, k,
                                                                 [](T const &x) { return x; }));
}
...
T2 const *sorted_pair_lookup(std::vector<std::pair<T1, T2>> const &v, T1 const &k) {
  int n = v.size();
  auto const *arrp = v.data();
  auto const *posn =
      array_search_ordered<false, 6>(arrp, n, k, [](decltype(*arrp) const &p) { return p.first; });
...
std::tuple<T1, Tx...> const *sorted_tuple_lookup(std::vector<std::tuple<T1, Tx...>> const &v,
                                                 T1 const &k) {
  int n = v.size();
  auto const *arrp = v.data();
  auto const *posn = array_search_ordered<false, 4>(
      arrp, n, k, [](decltype(*arrp) const &tup) { return std::get<0>(tup); });
