...
// NOTE: this file contains defs for concurrent deserialize which are needed on both decode and
// prepare side; mostly just the format of the Aux Data records. Defs needed only on decode side are
// in 'deser_concurrent.h', which #includes this file.
...
// This is string is used both by hexnn and QNN to understand that a particular graph has no
// segments And therefore it should not try to measure it.
...
// An exception is 'crate_seg_len'- this may be less than arr[i+1].crate_offset -
// arr[i].crate_offset
//  due to padding.
...
struct runlist_auxdata_seg_desc {
  uint32_t deser_offset;  // where the input (pickle) data begins - reference point is the start of
                          // 'Runlist' as
  //                     // defined in docs/pickle_format.md, i.e. the location of 'n_ops_total'
  //                     word
...
// NOTE: if that gets backed out later, leave this here but remove it from
// DesConcur_AUXDATA_REC_VERSION
//
