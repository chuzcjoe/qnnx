...
class API_EXPORT cost_function_t {
 ...
 public:
  ...
  inline float operator()(const Graph &graph_in, Op const *op) const {
    return (*funcp)(*this, graph_in, op);
  }
  ...
  template <class T>
  static float cfunc(cost_function_t const &, const Graph &, Op const *);
};
