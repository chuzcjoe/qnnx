...
inline T *DCrate::alloc_array(const size_t n) {
  if (nextp != nullptr) {
...
inline T *DCrate::emplace(Args &&...args) {
  if (nextp != nullptr) {
...
inline void *DCrate::emplace_explicit(Deserz &dctx, deserialize_op_func const init_func,
                                      deserialize_dtor_func const dtor_func,
                                      size_align_code_t const size_al) {
  if (nextp != nullptr) {
...
// are just 'Deserz &'
template <typename T>
inline T *DCrate::emplace0(Deserz &dctx) {
...
//
template <typename T>
inline void hnnx::cratevec<T>::init(hnnx::DCrate *crate_p, size_t n) {
  assert(m_len == 0);
  if (n) {
