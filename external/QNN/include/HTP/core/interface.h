...
template <>
constexpr DType dtype_of_type<ScaleOffsetInterface<uint8_t>>() {
  return DType::QUInt8;
}
template <>
constexpr DType dtype_of_type<ScaleOffsetInterface<int8_t>>() {
  return DType::QInt8;
}
template <>
constexpr DType dtype_of_type<ScaleOffsetInterface<uint16_t>>() {
  return DType::QUInt16;
}
template <>
constexpr DType dtype_of_type<ScaleOffsetInterface<int16_t>>() {
  return DType::QInt16;
}

template <>
constexpr DType dtype_of_type<PlainInterface<Float16>>() {
  return DType::Float16;
}
template <>
constexpr DType dtype_of_type<PlainInterface<BFloat16>>() {
  return DType::BFloat16;
}
template <>
constexpr DType dtype_of_type<PlainInterface<float>>() {
  return DType::Float32;
}
template <>
constexpr DType dtype_of_type<ScaleOffsetInterface<NN_INT32_T>>() {
  return DType::QInt32;
}
template <>
constexpr DType dtype_of_type<PlainInterface<NN_INT32_T>>() {
  return DType::Int32;
}
template <>
constexpr DType dtype_of_type<PlainInterface<NN_INT64_T>>() {
  return DType::Int64;
}
...
class Interface {
 ...
 public:
  ...
  static Interface const *canonical_instance_for();  // inline is below

 protected:
  template <DType DT>
  static intfc_methods const &methods_for();  // inline is below
...
// The ifc_hash' method does not compute the complete hash; it remains to 'xor' with
// unsigned(dtype). In cases where it just returns 0, a null ptr can be used.
//
struct intfc_methods {
  ...
  Interface::ifc_compare_fp
      ifc_compare;  // <- may be null if always returns 0 (e.g. PlainIterface).
};
...
// can define this now.
template <DType DT>
inline hnnx::intfc_methods const &Interface::methods_for() {
...
//
template <typename IFC>
inline Interface const *Interface::canonical_instance_for() {
...
template <>
inline Interface const *Interface::canonical_instance_for<NullInterface>() {
...
class InterfaceRef {
 ...
 public:
  ...
  API_EXPORT InterfaceRef(intfc_methods const &mthods, Interface const *ifc_p)
      : methods_p(&mthods), intfc_p(ifc_p) {}
  ...
  API_EXPORT void write_float(void *ptr, const float in) const noexcept {
    methods_p->write_float(intfc_p, ptr, in);
  }
  API_EXPORT float read_float(const void *ptr) const noexcept {
    return methods_p->read_float(intfc_p, ptr);
  }
...
template <typename IFCT>
Interface::qparms &qparms_for_interface_patch(size_t);
}  // namespace hnnx
 ...
 public:
  ALWAYSINLINE GenericAccessorRO(void const *const data_in, hnnx::InterfaceRef const &interface_in)
      : data(const_cast<void *>(data_in)),
        interface(*interface_in.get_intfc_ptr()),
...
class GenericAccessor : public GenericAccessorRO {
 ...
 public:
  ALWAYSINLINE GenericAccessor(void *const data_in, hnnx::InterfaceRef const &interface_in)
      : GenericAccessorRO(data_in, interface_in),
        write_fp(interface_in.get_methods_ptr()->write_float) {}
...
struct DTypeScaleOff {
  ...
  DTypeScaleOff(DType dt, Interface::qparms const &qpp) noexcept
      : dtype(dt), scale(qpp.scale), offset(qpp.offset) {}
  DTypeScaleOff(DType dt, hnnx::InterfaceRef const &iref) noexcept
      : DTypeScaleOff(dt, *iref.get_qparms()) {}
...
class NullInterface final : public Interface {
 ...
 public:
  ...
  static inline constexpr dtype_info get_dt_info() noexcept {
    return hnnx::dtype_info_v<DType::UNKNOWN>;
  }
 ...
 public:
  ...
  static inline hnnx::InterfaceRef get_refobj() noexcept {
    return hnnx::InterfaceRef(methods_for<DType::UNKNOWN>(),
                              Interface::canonical_instance_for<NullInterface>());
  }
...
template <>
struct hnnx::make_interface<NullInterface> {
 ...
 */
template <typename T>
class PlainInterface final : public Interface {
  friend constexpr hnnx::ifc_method_table_t hnnx::construct_ifc_method_table();
 ...
 private:
  static void write_float(Interface const *self, void *ptr,
                          const float in) noexcept;  // inlined below
 ...
 public:
  ...
  static inline hnnx::InterfaceRef get_refobj() noexcept {
    return hnnx::InterfaceRef(methods_for<dtype>(),
                              Interface::canonical_instance_for<PlainInterface>());
  }
 ...
 private:
  ...
  class AcsrRO {
   ...
   public:
    using element_type = T;
    using AccessorRO = AcsrRO;
    inline AcsrRO(void const *data_in, PlainInterface const *)
        : val(*static_cast<T const *>(data_in)) {}
  ...
  class Acsr {
   ...
   public:
    using element_type = T;
    using AccessorRO = AcsrRO;
    ALWAYSINLINE inline Acsr(void *data_in, PlainInterface const *)
        : data(static_cast<T *>(data_in)) {}
    ALWAYSINLINE inline Acsr(Acsr const &) = default;
    ALWAYSINLINE inline Acsr(Acsr &&) = default;
    ALWAYSINLINE inline ~Acsr() = default;
...
template <>
API_EXPORT inline constexpr float PlainInterface<float>::convert_from_float(const float &in) {
  return in;
}

// PlainInterface<Float16>::convert_from_float: no-op for values in Float16 range, clamp to max
// otherwise.
template <>
API_EXPORT inline constexpr Float16 PlainInterface<Float16>::convert_from_float(const float &in) {
...
template <>
API_EXPORT inline constexpr BFloat16 PlainInterface<BFloat16>::convert_from_float(const float &in) {
  return static_cast<BFloat16>(in);
}

// needs to be defined *after* convert_from_float is specialized
template <typename T>
inline void PlainInterface<T>::write_float(Interface const *, void *ptr, const float in) noexcept {
...
template <typename T>
struct hnnx::make_interface<PlainInterface<T>> {
  API_EXPORT static inline Interface const *from_odef(Graph &, OutputDef const &odef) {
 ...
 * This is good for quantization schemes where you want to quantize an arbitrary, possibly
 * asymmetric range. We compute and cache the reciprocal of the scale for conversion from float A
 * default constructor sets the offset to 0 and the scale to 1.0, which would be suitable for
 * integers. The reciprocal of scale should be computed so we don't have to divide.
 */
...
class SOIfcBase : public Interface {  // base of ScaleOffsetInterface<T>
 protected:
  ...
  template <typename X>
  friend Interface::qparms &hnnx::qparms_for_interface_patch(size_t);
  ...
  SOIfcBase(const OutputDef &def, const dtype_info dt_)
      : SOIfcBase(def.zero_offset, def.stepsize, dt_) {
...
template <typename T>
class ScaleOffsetInterface final : public SOIfcBase {
  friend constexpr hnnx::ifc_method_table_t hnnx::construct_ifc_method_table();

 public:
  API_EXPORT ScaleOffsetInterface(const int offs_, const float scale_)
      : SOIfcBase(offs_, scale_, get_dt_info()) {}
  ...
  API_EXPORT static inline constexpr T saturate(TX in) {
    return saturate_cast<T>(in);
  }
 ...
 private:
  ...
  // LCOV_EXCL_START [SAFTYSWCCB-1736] constexprs resolved during compile time
  static constexpr intfc_methods get_method_table() {
    return {hnnx::dtype_info_v<dtype>, read_float, write_float,
            get_qparms_meth,           ifc_hash,   ifc_compare};
  }
  // LCOV_EXCL_STOP
 ...
 public:
  ...
  inline hnnx::InterfaceRef get_refobj() const noexcept {
    return hnnx::InterfaceRef(methods_for<dtype>(), this);
  }
  inline DTypeScaleOff get_dtype_scaleoff() const noexcept {
    return DTypeScaleOff(dtype, *get_qparms_meth(this));
  }

  API_EXPORT inline int compare(const ScaleOffsetInterface &rhs) const noexcept {
    return SOIfcBase::compare(rhs);
  }
...
template <typename T>
struct hnnx::make_interface<ScaleOffsetInterface<T>> {
  // can only declare these here, since we can't see into Graph at this point.
  // Code is in tensor.cc
  API_EXPORT static ScaleOffsetInterface<T> const *from_exemplar(
      Graph &, ScaleOffsetInterface<T> const &exemplar);
 ...
 protected:
  ...
  API_EXPORT static ScaleOffsetInterface<T> const *to_crate(
      Graph &, ScaleOffsetInterface<T> const &exemplar);
};
