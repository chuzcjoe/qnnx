...
struct runlist_fixup_state {
  ...
  runlist_seg_descriptor *seg_desc =
      nullptr;  // Corresponding 'runlist_seg_descriptor' for reference.
...
//
// This contains 'supplemental' fixup records for a segment;  there is one instance in each
// runlist_seg_descriptor, and a pointer to in the runlist_fixup_state. When the
// 'runlist_fixup_state' is moved in or out of the Deserz, the pointer to this remains. To avoid the
// overhead of vec_push_back, this // has a static array into which values are recorded; when this
// is full (or near full), all the records within are appended to the vector in a single operation.
// At the end of the operation, any remaining records are appended to the vector, but only if the
// vector is not empty (we can read the records out of the fixed array, if they all fit).
//
// The append() is not safe unless 'ensure_room_for' is checked first; you can e.g. do
// ensure_room_for(3) ahead of doing up to 3 append It is best to use a constant as parameter to
// ensure_room_for, i.e. ahead of code which may append *up to* 4 values, use ensure_room_for(4);
// this simplifies the inline expansion of 'ensure_room_for', and makes very little difference to
// performance compared to using the exact value.
//
class fixup_supplemental_recs {
...
class DCrate {
 ...
 public:
  ...
  template <typename FI, typename FD, typename SA>
  void *emplace_explicit(Deserz &dctx, FI, FD, SA);
...
bool fixup_encode_for_blocktable(runlist_fixup_state &seginfo, uint32_t idx, uint32_t table_offs,
                                 void **ptrloc);
