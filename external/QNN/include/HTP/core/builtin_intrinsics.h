...
static inline bool HEX_ADD_OVERFLOW(_T a, _T b, _T *out) {
  *out = a + b;
...
template <typename _T>
static inline bool HEX_MUL_OVERFLOW(_T a, _T b, _T *out) {
  *out = a * b;
  return ((b > 0) &&
          (a > std::numeric_limits<_T>::max() / b || a < std::numeric_limits<_T>::min() / b)) ||
         ((b < 0) &&
          (a > std::numeric_limits<_T>::min() / b || a < std::numeric_limits<_T>::max() / b));
}
...
template <typename _T>
static inline int HEX_COUNT_ONE_BIT(_T x) {
...
static inline int HEX_COUNT_LEADING_ZERO_UL(unsigned long x) {
  return sizeof(x) == 8 ? HEX_COUNT_LEADING_ZERO_ULL(x)
                        : HEX_COUNT_LEADING_ZERO(static_cast<unsigned int>(x));
}
...
static inline int HEX_COUNT_TRAILING_ZERO_UL(unsigned long x) {
  return sizeof(x) == 8 ? HEX_COUNT_TRAILING_ZERO_ULL(x)
                        : HEX_COUNT_TRAILING_ZERO(static_cast<unsigned int>(x));
}
...
template <typename _T>
static inline _T HEX_ATOMIC_FETCH_AND_AND(_T volatile *_p, _T _v) {
  _InterlockedAnd((long *)_p, (long)_v);
  return static_cast<_T>(*_p);
}

template <typename _T>
static inline _T HEX_ATOMIC_FETCH_AND_OR(_T volatile *_p, _T _v) {
...
#define HEX_ATOMIC_BOOL_COMPARE_AND_SWAP(_p, _old, _new) \
  (HEX_ATOMIC_VAL_COMPARE_AND_SWAP(_p, _old, _new) == (_old))
 ...
 */
template <typename T>
struct promote_shift_operand {
  typedef T type;
};

template <>
struct promote_shift_operand<char> {
  using type = int;
};
template <>
struct promote_shift_operand<signed char> {
  using type = int;
};
template <>
struct promote_shift_operand<unsigned char> {
  using type = int;
};
template <>
struct promote_shift_operand<short> {
  using type = int;
};
template <>
struct promote_shift_operand<unsigned short> {
  using type = int;
};

template <typename T>
using promote_shift_operand_t = typename promote_shift_operand<T>::type;
...
template <typename T>
constexpr unsigned get_safe_shift_mask() {
  return unsigned(
      CHAR_BIT * sizeof(promote_shift_operand_t<std::remove_cv_t<std::remove_reference_t<T>>>) - 1);
}
