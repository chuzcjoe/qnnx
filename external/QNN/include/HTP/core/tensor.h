 ...
 * * Abstract tensor as a base class that provides a generic interface always, using runtime
 * polymorphism
 ...
 * The accessor object works like an rvalue or lvalue, but is able to decode (rvalue) or encode
 * (lvalue) the data as appropriate. (At least, that's how I think it should work...)
 *
 * We'd like to use the operator() to allow us to have multidimentional-array-indices-like
 * interface, much in the same way we have in Eigen.  So for a 4D tensor, with indices
 * batchidx,row,col,channel, we should be able to say out_tensor(batchidx,row,col,channel) =
 * in_tensor(batchidx,row,col,channel)
 *
 * Although we might consider a variety of different types for tensor internal values, including
 * int32, I propose to use "float" as the defualt interface type.  It should work well for many
 * integers, and is the appropriate interface for real data whether quantized or not.  A reasonable
 * alternative would be double, but double is quite a bit more expensive on Hexagon.  Of course,
 * other methods of accessing the data could be made for different types.
 *
 ...
 *
 * Tensors are very fundamental to how we are going to work on things, so it's incredibly important
 * to be on our best behavior here.
 *
 */

/*
 * EJP: FIXME:
 * * The helper classes, Accessors and Interfaces and such, should be in a sub-namespace for
 * cleanliness.
