...
struct GraphStatus {
  ...
  enum GraphErrorCode {
    ...
    ErrorFatalCheck = 15,  // preprocess in prepare, e.g. clear the opid_alias_map, check
                           // connectivity, order_nodes
    ErrorBadOpName = 16,
    ErrorFatalOptimize = 17,
    ErrorFatalCSE =
        18,  // steps that combined with CSE e.g. dead_code_removal_and_cse, const_prop_and_cse()
    ...
    ErrorTCMAcquire =
        24,  // we can recover from TCM acquire failures (when tcm was locked by a different client)
    ErrorHMXAcquire = 25,
    ErrorHMXPower = 26,
    ErrorBadPMU = 27,
    ErrorThreadCounts = 28,
    ErrorClobberedPMU = 29,  // Something clobbered our expected PMU event.
    ErrorWeightsCompressedNoAperture =
        30,  // Weights are DLBC compressed, but failed to acquire aperture for it
    ErrorRank = 31,
    ErrorHMXRelease = 32,
    ErrorTCMRelease = 33,
    ErrorWeightsCompressedBadFormat =
        34,  // Weights are DLBC compressed, but compression format is not supported
