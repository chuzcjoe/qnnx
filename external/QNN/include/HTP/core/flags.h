...
enum class Flags : unsigned {
  ...
  CAN_BE_SRC_DESTRUCTIVE,       // Op will work correctly if input[0] and output[0] are at the same
                                // places in TCM
  IS_WEIGHT_FOR_BIT_REARRANGE,  // Indicates Weight data will be used for bit rearrangement
  IS_PREDICATE_MARKER_TRUE,     // Op is a marker node, all the producers of this node have a true
                                // sense for predication.
...
// INPLACE_NOP is a null_exec op that has 1 input and 1 output, with identical DType and
// quantization, shape, and tensor type; which could just be bypassed. Examples are the 'Padzap'
// with no work to do, and all ForceFormat_flat where the input is already flat (implemented by
// format_no_translate_flat).

// IS_COPY is a op that has 1 input and 1 output, with the same shape and tensor layout type; the
// memory class and dtype can be different (dtype must be the same #bytes); but the operation must
// be fulfilled by raw copy of the input block(s) to the output block(s). Mainly this is intended to
// mark const->TCM operations that can be replaced by 'const-fill'.
...
constexpr Flags_word flags_for = flags_for_t<T, S>::value;
...
// LCOV_EXCL_START [SAFTYSWCCB-1542]
template <typename T>
[[maybe_unused]] static constexpr const char *docs_for() {
  return DOCS_UNSET;
}
// LCOV_EXCL_STOP
...
#define FLAGS_FOR_IMPL(T, S, ...)                                                                 \
  ...
  /* LCOV_EXCL_START [SAFTYSWCCB-1736] constexprs resolved during compile time */                 \
  template <>                                                                                     \
  constexpr unsigned FlagCounter<T, S>::increment() {                                             \
    return 1U;                                                                                    \
  }                                                                                               \
  /* LCOV_EXCL_STOP */                                                                            \
  template <>                                                                                     \
  struct hnnx::flags_for_t<T, FlagCounter<T, S>::get()> {                                         \
    constexpr static Flags_word value = hnnx::flagval_generate<__VA_ARGS__>;                      \
    static_assert(FlagCounter<T, S>::get() <= hnnx::FLAG_FOLDING_LIMIT,                           \
                  "Flag folding limit exceeded, this means you tried to register too many flags " \
                  "to the same type.");                                                           \
    static_assert(FlagCounter<T, S>::get() == 1 ||                                                \
                      value == hnnx::flags_for<T, FlagCounter<T, S>::get() - 1>,                  \
                  "Flags mismatch, this happens when different flags have been registered for "   \
                  "the same type. Due to TCM folding, this can also happen when registering "     \
                  "different flags for the TCM and non-TCM op implementations.");                 \
  };

#define FLAGS_FOR(F, ...) FLAGS_FOR_IMPL(F, __COUNTER__, __VA_ARGS__)
#define FLAGS_FOR_DT_NO_TCM_FOLDING(F, ...) \
  FLAGS_FOR_IMPL(DerivedType<&F>::type, __COUNTER__, __VA_ARGS__)
...
#define DOCS_FOR(F, DOCSTRING)                \
  template <>                                 \
  constexpr const char *hnnx::docs_for<F>() { \
    return DOCSTRING;                         \
  }
#else
#define DOCS_FOR(F, DOCSTRING)
