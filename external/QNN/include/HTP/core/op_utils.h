...
template <typename T>
static inline bool is_output_def_valid(const OutputDef &output_def, Graph &graph_in) {
  return tensor_generator_valid<T>(nullptr, output_def, graph_in);
}

template <typename T>
static inline bool is_input_tensor_compatible(Graph &graph_in, Tensor const *tensor,
                                              unsigned position) {
...
static inline bool are_output_defs_valid_helper(std::index_sequence<I...>,
                                                OutputDef const *const *outputs_in,
                                                Graph &graph_in) {
  //  tensor_generator below returns a unique pointer which will be released on return (i.e. when
  //  object goes out of scope)
  // this check preferably should be done with boolean valid_tensor() method instead of creating an
  // actual tensor but for now to limit generation of more template code this approach should
  // suffice.
...
static inline bool are_input_tensors_compatible_helper(std::index_sequence<I...>, Graph &graph_in,
  ...
  return (
      (is_input_tensor_compatible<std::tuple_element_t<I, TupType>>(graph_in, inputs_in[I], I)) &&
      ...);
}

template <size_t N, typename TupType>
static inline bool are_input_tensors_compatible(Graph &graph_in, Tensor const *const *inputs_in) {
  return are_input_tensors_compatible_helper<TupType>(std::make_index_sequence<N>{}, graph_in,
                                                      inputs_in);
}

template <typename T>
std::unique_ptr<Tensor> tensor_output_alloc(const Op *producer_in, const OutputDef &output_def,
                                            Graph &graph_in) {
...
inline constexpr std::array<tensor_generate_fp, N> tensor_gen_array_helper(
...
template <typename TupType>
inline tensor_generate_fp const *tensor_gen_array_ptr() {
...
struct dt_rank_pair_for_tens {
  static_assert(int(sizeof(Tens)) < 0,
                "Can't use Tens as 'scratch' output type, only Concrete Tensor");
};
// Specialized for Concrete<Tensor> only.
template <typename Tinfo>
struct dt_rank_pair_for_tens<ConcreteTensor<Tinfo>> {
 private:
...
template <typename TupType, size_t N, size_t... I>
inline constexpr std::array<dt_rank_pair, N> tensor_dt_rank_array_helper(
    std::index_sequence<I...>) {
...
template <typename TupType>
inline dt_rank_pair const *tensor_dt_rank_array_ptr() {
  if constexpr (std::tuple_size_v<TupType> != 0) {
...
template <unsigned NPREFIX, typename TupOfTens, bool FINAL = (NPREFIX == 0)>
struct tensor_dt_rank_array_for_scratch;

// case with NPREFIX = 0
template <typename... Tts>
struct tensor_dt_rank_array_for_scratch<0, std::tuple<Tts...>, true> {
  static inline dt_rank_pair const *table_p() {
    return tensor_dt_rank_array_ptr<std::tuple<Tts...>>();
  }
};
...
inline uint32_t get_spatial_mask() {
  // NOLINTNEXTLINE(misc-const-correctness): Don't const this variable
  uint32_t spatial_mask = 0x38;  // 0b111000
  if constexpr (std::is_base_of<LayoutWideCrouton_8, Ttype>::value)
    spatial_mask = 0x20;  // 0b100000
  return spatial_mask;
}
...
//
static inline std::tuple<size_t, size_t> find_output_size_and_prepad(bool same_shape, size_t insize,
                                                                     size_t winsize,
                                                                     size_t stride) {
  size_t outsize;
  size_t prepad;
