...
class OpDefFlags {
  ...
  constexpr static uint32_t BIT_hidden =
      2;  // replaced,hidden; not necessarily pending removal (>= deleted).
  constexpr static uint32_t BIT_const = 4;  // is an OpDef_ConstBase, or an op with const output.
  constexpr static uint32_t BIT_volatile =
      8;                                      // is not invariant, even if all inputs are invariant.
  ...
  constexpr static uint32_t BIT_constbase =
      64;  // if and only if it's a OpDef_ConstBase. immutable.
  ...
  constexpr static uint32_t BIT_fake_unsigned =
      256;  // OpDef had dtype changed to unsigned during prepare
 ...
 protected:
  ...
  API_EXPORT OpDefFlags(hnnx::opname_tag_parm_t opstr, int n_in, int n_out)
      : flags((uint16_t)flag_init(opstr, n_in, n_out)),
        opstr_hashval((uint16_t)hnnx::find_opname_hash(opstr)) {}
 ...
 public:
  ...
  API_EXPORT bool set_fake_unsigned(bool val = true) {
    return set_flag_state<BIT_fake_unsigned>(val);
  }
...
/**
 * @brief: OutputDefPatch struct is used to update specific members of an OutputDef, rather than
 * replacing/copying an entire object
 */
...
class OpDef : public OpDefFlags {
 protected:
  ...
  API_EXPORT OpDef(GraphPrepare &graph_in, OpId my_id_in, hnnx::opname_tag_parm_t opstr_in,
                   OutputDef const &odef, ForConst const &)
      : OpDefFlags(opstr_in, true),
        splithist(),
        graphref(graph_in),
        id(my_id_in),
        opstr(opstr_in),
 ...
 public:
  ...
  API_EXPORT OpDef(GraphPrepare &graph_in, OpId my_id_in, hnnx::opname_tag_parm_t opstr_in,
                   ...
                   hnnx::splithist_t sl)
      : OpDefFlags(opstr_in, input_defs_in.size(), (output_def_in == nullptr) ? 0 : 1),
        splithist(sl),
        graphref(graph_in),
        id(my_id_in),
        opstr(opstr_in),
        input_defs(std::move(input_defs_in)),
        m_outputdef() {
  ...
  API_EXPORT OpDef make_output_exemplar(OutputDef const *size_from,
                                        OutputDef const *outp_from) const;
  ...
  API_EXPORT inline OpDef make_output_exemplar() const {
    return make_output_exemplar(nullptr, nullptr);
  }
  ...
  //    API_EXPORT OutputDef &get_outputdef() { return output_def; } //use when need to modify
  //    output_def
  API_EXPORT OutputDef const &get_outputdef() const {
    return m_outputdef;
  }  // return read-only output_def
  ...
  struct compare_less_ptr_functor {
    bool operator()(const OpDef *lhs, const OpDef *rhs) const {
      return OpDef::compare_less(*lhs, *rhs);
    }
  };
...
// used to order them.
//
// the ordering will be a little arbitrary - if two have different hashes, they will first be
// ordered according to the hashes; otherwise we will go through a multi-key compare of the OpDef
// attributes and finally call tensor_compare (if both are OpDef_Const). If a more rational ordering
// is needed, this can be added, but it will tend to be slower than the hashed compare if you have a
// lot of Const with matching shapes. If A and B are both OpDefConstBase, then
// OpDef::compare_less(A,B) will be the same as OpDef_ConstBase::compare_less(A,B).
//
// The content_hash is never 0; zero is used to mark 'unknown'; the next time get_content_hash() is
// called, find_content_hash() will be called to determine the hash, and then it will be stored in
// content_hash for next time.
//
class OpDef_ConstBase : public OpDef {
  mutable uint32_t content_hash = 0;

 protected:
  OpDef_ConstBase(GraphPrepare &graph_in, OpId my_id_in, opname_tag_parm_t opstr,
                  OutputDef const &output_def)
...
API_FUNC_EXPORT inline bool compare_constbase_eq(const OpDef_ConstBase &lhs,
                                                 const OpDef_ConstBase &rhs) {
...
class OpDef_Const : public OpDef_ConstBase {
 public:
  ...
  API_EXPORT OpDef_Const(GraphPrepare &graph_in, OpId my_id_in, OutputDef const &output_def,
                         const uint8_t *data_in, size_t len);
