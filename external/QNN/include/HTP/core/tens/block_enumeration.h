...
// ids.
//   Rules are:
//   - if 'supply_blocks_func' is called with memclass < 0, the memory class of the block is
//   unspecified;
//     if it is called with memclass >=0, the value  is  MemoryClass and the tensor guarantees that
//     all of the blocks are in that class.
//   - Tensor may in general make multiple calls to supply_blocks_func in one call to
//   enum_memory_blocks, and may
...
class MemBlockEnumerator {
 public:
  ...
  API_EXPORT virtual void supply_blocks_func(Tensor const *tensp, int memclass, void *const *ptr,
                                             size_t num) = 0;
  ...
  API_EXPORT inline void supply_blocks(Tensor const *tensp, MemoryClass mc, void *const *ptr,
                                       size_t num) {
...
class MemBlockEnumToSet : public MemBlockEnumerator {
 ...
 public:
  API_EXPORT explicit MemBlockEnumToSet(blockid_set_t &s, int mclass_sel = -1)
      : m_set(s), m_memclass_sel(mclass_sel) {}
  API_EXPORT MemBlockEnumToSet(blockid_set_t &s, MemoryClass mc)
      : m_set(s), m_memclass_sel(int(mc)) {}
  API_EXPORT virtual void supply_blocks_func(Tensor const *, int memclass, void *const *ptr,
                                             size_t num) override {
...
class MemBlockEnumWrapper : public MemBlockEnumerator {
  ENFUNC m_enfunc;
...
class MemBlockReplBlockWrapper : public MemBlockEnumerator {
  REPLFUNC m_replfunc;
