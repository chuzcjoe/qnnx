...
inline constexpr bool Float16::is_zero() const { return (exp_bits() | frac_bits()) == 0x0000; }

inline constexpr bool Float16::is_neg() const { return sign_bit(); }
...
inline constexpr bool Float16::is_finite() const { return is_norm() || is_subnorm(); }
...
inline constexpr Float16 Float16::zero(bool neg) { return Float16::from_raw(make_zero(neg)); }

inline constexpr Float16 Float16::qnan() { return Float16::from_raw(make_nan(true)); }

inline constexpr Float16 Float16::snan() { return Float16::from_raw(make_nan(false)); }

inline constexpr Float16 Float16::inf(bool neg) { return Float16::from_raw(make_inf(neg)); }
...
inline constexpr uint16_t Float16::sign_bit() const { return d & 0x8000u; }

inline constexpr uint16_t Float16::exp_bits() const { return d & 0x7C00u; }

inline constexpr uint16_t Float16::frac_bits() const { return d & 0x03FFu; }

inline constexpr uint16_t Float16::make_sign_bit(uint16_t s) {
  return static_cast<uint16_t>(!!s) << 15u;
}

inline constexpr uint16_t Float16::make_exp_bits(uint16_t e) { return (e & 0x001Fu) << 10u; }

inline constexpr uint16_t Float16::make_frac_bits(uint16_t f) { return f & 0x03FFu; }
...
constexpr Float16 operator""_f16(long double v) { return Float16(static_cast<float>(v)); }

PUSH_VISIBILITY(default)

template <>
class API_EXPORT std::numeric_limits<Float16> {
 public:
...
constexpr Float16 std::numeric_limits<Float16>::infinity() noexcept { return Float16::inf(false); }

constexpr Float16 std::numeric_limits<Float16>::quiet_NaN() noexcept { return Float16::qnan(); }

constexpr Float16 std::numeric_limits<Float16>::signaling_NaN() noexcept { return Float16::snan(); }
