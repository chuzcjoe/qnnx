...
template <typename T>
inline void ctor_hook(Graph &, T &ref) {
  return;
}
...
template <typename T>
inline void ctor_ophook(OpIoPtrs const &op_io_ptrs) {
  return;
}
...
#define CTOR_HOOK(FUNC, VAR, CODE)                                                         \
  template <>                                                                              \
  [[maybe_unused]] inline void hnnx::ctor_hook(Graph &graph_in,                            \
                                               typename DerivedType<(&FUNC)>::type &VAR) { \
    CODE                                                                                   \
  }
#endif
...
#define CTOR_OPHOOK(FUNC, HOOKCLASS)                                                               \
  template <>                                                                                      \
  inline void hnnx::ctor_ophook<typename DerivedType<(&FUNC)>::type>(OpIoPtrs const &op_io_ptrs) { \
