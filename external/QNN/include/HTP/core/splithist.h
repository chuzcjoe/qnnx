...
//
//   Now, for the first 1024 slices of that split [0..1023], the splithist_t upper bits contain the
//   index of the main record, and have slice_idx 0..1023; the next 1024 slices [1024..2047] have
//   upper bits referencing the extension record with recoff=1, also with slice_idx = 0..0123; and
//   the final 452 slices [2048..2499] have upper bits referencing the extension record with
//   recoff=2, and have slice_idx = 0..451. Extension records occupy one entry in the table, so when
//   an extension record is encountered when following a splithist_t, it is easy to locate the
//   'main' record it refers to, and to adjust the slice index to the proper value, by using the
//   'recoff' in the extension record.
//
// So, there are three types of records:
//   - Root record, contains a 3-bit 'dimno', and 32-bit 'original op id' (the lower 32 bits of the
//   OpId which is split)
//   - Subsplit record, contains a 3-bit dimno, and 'subsplit_t' which points one level towards the
//   root (and, it may point to
...
// are upper bits in the first word which signify what the record type is. Also, the records are
// often packed into one word, sometimes two, according to the magnitude of the values to be
// encoded. The 'extension' record always fits in one word. The details of the encoding are
// described in splithist.cc, near the three methods of SplitHistoryTable which need to know about
// them (e.g. cursor_from_splithist unpacks a record to 'Cursor' object, compensating for extension
// record when present).
...
class SplitHistoryTable {
 ...
 public:
  ...
  splithist_t make_new_split(uint32_t dimno, OpId nodeid, splithist_t oldhist, int nslices,
                             uint32_t slice_size, uint32_t first_sliceno = 0);
  ...
  // This differs from the values returned by splithist_t get_record() / get_sliceno() for extension
  // records.
