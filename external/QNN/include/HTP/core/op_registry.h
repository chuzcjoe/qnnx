...
/*
 * We register an op with the registry by giving it a name, and a std::unique_ptr to a generator for
 * that op. Returns a reference to the op once emplaced. Why? Because that allows us to create
 * static variables with the results, causing the functions to be loaded automatically...
 */
...
//
API_FUNC_EXPORT void register_optype_by_factory(OpFactory fp, hnnx::opname_tag_t opname_tag,
                                                std::type_info const &typ,
...
constexpr auto ConcatStr(const char *a, const char *b) {
...
template <typename T>
constexpr size_t ConstexprStrLen(T s) {
  return 0;
}

// LCOV_EXCL_START [SAFTYSWCCB-1736] constexprs resolved during compile time
template <>
constexpr size_t ConstexprStrLen<const char *>(const char *str) {
  size_t len = 0;
  while (*str != 0) {
    len++;
    str++;
