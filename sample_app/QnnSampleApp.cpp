...
sample_app::QnnSampleApp::QnnSampleApp(QnnFunctionPointers qnnFunctionPointers,
                                       ...
                                       void* backendLibraryHandle, std::string outputPath,
                                       bool debug, iotensor::OutputDataType outputDataType,
                                       iotensor::InputDataType inputDataType,
                                       sample_app::ProfilingLevel profilingLevel, bool dumpOutputs,
                                       std::string cachedBinaryPath, std::string saveBinaryName,
                                       unsigned int numInferences, bool serializeProfileLogs,
                                       std::string dlcPath)
...
sample_app::QnnSampleApp::~QnnSampleApp() {
  ...
  dlc_utils::freeDlcResources(m_qnnFunctionPointers.qnnSystemInterfaceHandle, m_dlcHandle,
                              m_dlcLogHandle);
}
...
sample_app::StatusCode sample_app::QnnSampleApp::initialize() {
  ...
  if (log::isLogInitialized()) {
    ...
    QNN_INFO("Initializing logging in the backend. Callback: [%p], Log Level: [%d]", logCallback,
             logLevel);
    if (QNN_SUCCESS !=
  ...
  if (ProfilingLevel::OFF != m_profilingLevel) {
    ...
    if (ProfilingLevel::BASIC == m_profilingLevel) {
      ...
      if (QNN_PROFILE_NO_ERROR !=
          m_qnnFunctionPointers.qnnInterface.profileCreate(m_backendHandle, QNN_PROFILE_LEVEL_BASIC,
...
sample_app::StatusCode sample_app::QnnSampleApp::registerOpPackages() {
  ...
  for (auto const& opPackagePath : m_opPackagePaths) {
    ...
    QNN_INFO("Registered Op Package: %s and interface provider: %s", opPackage[pathIdx].c_str(),
  ...
  if (!m_dlcPath.empty()) {
    ...
    if (qnn_wrapper_api::ModelError_t::MODEL_NO_ERROR !=
        m_qnnFunctionPointers.composeGraphsFnHandle(
            ...
            (const qnn_wrapper_api::GraphConfigInfo_t**)m_graphConfigsInfo, m_graphConfigsInfoCount,
            &m_graphsInfo, &m_graphsCount, m_debug, log::getLogCallback(), log::getLogLevel())) {
...
sample_app::StatusCode sample_app::QnnSampleApp::finalizeGraphs() {
  for (size_t graphIdx = 0; graphIdx < m_graphsCount; graphIdx++) {
    ...
    if (QNN_GRAPH_NO_ERROR !=
        m_qnnFunctionPointers.qnnInterface.graphFinalize((*m_graphsInfo)[graphIdx].graph,
                                                         m_profileBackendHandle, nullptr)) {
      return StatusCode::FAILURE;
    }
...
sample_app::StatusCode sample_app::QnnSampleApp::createFromBinary() {
  ...
  if (StatusCode::SUCCESS == returnStatus &&
      QNN_SUCCESS != m_qnnFunctionPointers.qnnSystemInterface.systemContextGetBinaryInfo(
                         sysCtxHandle, static_cast<void*>(buffer.get()), bufferSize, &binaryInfo,
                         &binaryInfoSize)) {
  ...
  if (StatusCode::SUCCESS == returnStatus &&
      m_qnnFunctionPointers.qnnInterface.contextCreateFromBinary(
          ...
          static_cast<void*>(buffer.get()), bufferSize, &m_context, m_profileBackendHandle)) {
...
sample_app::StatusCode sample_app::QnnSampleApp::saveBinary() {
  ...
  if (QNN_CONTEXT_NO_ERROR != m_qnnFunctionPointers.qnnInterface.contextGetBinary(
...
sample_app::StatusCode sample_app::QnnSampleApp::composeGraphsFromDlc() {
  ...
  auto dlcStatus = dlc_utils::createDlcHandle(m_qnnFunctionPointers.qnnSystemInterfaceHandle,
                                              m_dlcPath, log::getLogCallback(), log::getLogLevel(),
                                              m_dlcLogHandle, m_dlcHandle);
  ...
  dlcStatus = dlc_utils::composeGraphsFromDlc(
      m_qnnFunctionPointers.qnnSystemInterfaceHandle, m_dlcHandle, m_backendHandle, m_context,
      m_qnnFunctionPointers.qnnInterfaceHandle, m_graphsInfo, m_graphsCount);
...
sample_app::StatusCode sample_app::QnnSampleApp::extractBackendProfilingInfo(
  ...
  if (m_serializationTargetHandle != nullptr && profileData != nullptr) {
    ...
    if (QNN_SUCCESS != m_qnnFunctionPointers.qnnSystemInterface.systemProfileSerializeEventData(
                           m_serializationTargetHandle,
                           const_cast<const QnnSystemProfile_ProfileData_t**>(&profileData), 1)) {
...
sample_app::StatusCode sample_app::QnnSampleApp::extractProfilingSubEvents(
  ...
  for (size_t subEvent = 0; subEvent < numSubEvents; subEvent++) {
    ...
    extractProfilingSubEvents(*(profileSubEvents + subEvent), systemProfileSubEvent,
                              profilingSubEvents);
...
sample_app::StatusCode sample_app::QnnSampleApp::executeGraphs() {
  ...
  for (unsigned int run = 0; run < m_numInferences; run++) {
    for (size_t graphIdx = 0; graphIdx < m_graphsCount; graphIdx++) {
      ...
      if (!inputFileList.empty()) {
        ...
        while (inputFileIndexOffset < totalCount) {
          ...
          std::tie(iotReturnStatus, numInputFilesPopulated, batchSize) =
              m_ioTensor.populateInputTensors(graphIdx, inputFileList, inputFileIndexOffset, false,
...
#ifndef __hexagon__
              if (iotensor::StatusCode::SUCCESS !=
                  m_ioTensor.writeOutputTensors(graphIdx, inputFileIndexOffset, graphInfo.graphName,
                                                outputs, graphInfo.numOutputTensors,
                                                m_outputDataType, m_graphsCount, m_outputPath,
                                                numInputFilesPopulated, batchSize)) {
                returnStatus = StatusCode::FAILURE;
              }
#endif
            }
      ...
      m_ioTensor.tearDownInputAndOutputTensors(inputs, outputs, graphInfo.numInputTensors,
