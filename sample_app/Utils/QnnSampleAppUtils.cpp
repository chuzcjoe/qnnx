...
uint64_t sample_app::getTimeStampInUs() {
  return std::chrono::duration_cast<std::chrono::microseconds>(
             std::chrono::steady_clock::now().time_since_epoch())
      .count();
}

void sample_app::split(std::vector<std::string> &splitString, const std::string &tokenizedString,
...
sample_app::ReadInputListRetType_t sample_app::readInputList(const std::string inputFileListPath) {
  ...
  if (!fileListStream) {
    ...
    return std::make_tuple(std::vector<std::vector<std::string>>{},
                           std::unordered_map<std::string, uint32_t>{}, false);
  }
  ...
  if (QNN_TENSOR_GET_RANK(src) > 0) {
    ...
    if (QNN_TENSOR_GET_DIMENSIONS(dst)) {
      pal::StringOp::memscpy(
          QNN_TENSOR_GET_DIMENSIONS(dst), QNN_TENSOR_GET_RANK(src) * sizeof(uint32_t),
    ...
    if (QNN_TENSOR_GET_IS_DYNAMIC_DIMENSIONS(src)) {
      ...
      pal::StringOp::memscpy(
          QNN_TENSOR_GET_IS_DYNAMIC_DIMENSIONS(dst), QNN_TENSOR_GET_RANK(src) * sizeof(uint8_t),
          QNN_TENSOR_GET_IS_DYNAMIC_DIMENSIONS(src), QNN_TENSOR_GET_RANK(src) * sizeof(uint8_t));
    }
  }
...
bool sample_app::copyTensorsInfo(const Qnn_Tensor_t *tensorsInfoSrc, Qnn_Tensor_t *&tensorWrappers,
...
bool sample_app::copyGraphsInfoV1(const QnnSystemContext_GraphInfoV1_t *graphInfoSrc,
  ...
  if (graphInfoSrc->graphOutputs) {
    if (!copyTensorsInfo(graphInfoSrc->graphOutputs, graphInfoDst->outputTensors,
