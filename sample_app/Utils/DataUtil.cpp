...
datautil::StatusCode datautil::readDataFromFile(std::string filePath, std::vector<size_t> dims,
  ...
  if (length != l) {
    QNN_ERROR("Input file %s: file size in bytes (%d), should be equal to: %d", filePath.c_str(),
              length, l);
  ...
  while (true) {
    ...
    if ((tensorLength % fileSize) != 0 || fileSize > tensorLength || fileSize == 0) {
      QNN_ERROR(
          ...
          filePaths[fileIndex].c_str(), fileSize, tensorLength, tensorLength);
...
datautil::StatusCode datautil::readBinaryFromFile(std::string filePath, uint8_t* buffer,
                                                  size_t bufferSize) {
  if (nullptr == buffer) {
    QNN_ERROR("buffer is nullptr");
    return StatusCode::INVALID_BUFFER;
...
#ifndef __hexagon__
datautil::StatusCode datautil::writeDataToFile(std::string fileDir, std::string fileName,
...
datautil::StatusCode datautil::writeBinaryToFile(std::string fileDir, std::string fileName,
...
datautil::StatusCode datautil::floatToTfN(T_QuantType* out, float* in, int32_t offset, float scale,
                                          size_t numElements) {
  ...
  double encodingMin = (0 - static_cast<T_QuantType>(offset)) * scale;
...
template datautil::StatusCode datautil::floatToTfN<uint8_t>(uint8_t* out, float* in, int32_t offset,
                                                            float scale, size_t numElements);

template datautil::StatusCode datautil::floatToTfN<uint16_t>(uint16_t* out, float* in,
                                                             int32_t offset, float scale,
                                                             size_t numElements);

template <typename T_QuantType>
datautil::StatusCode datautil::tfNToFloat(float* out, T_QuantType* in, int32_t offset, float scale,
                                          size_t numElements) {
...
template datautil::StatusCode datautil::tfNToFloat<uint8_t>(float* out, uint8_t* in, int32_t offset,
                                                            float scale, size_t numElements);

template datautil::StatusCode datautil::tfNToFloat<uint16_t>(float* out, uint16_t* in,
                                                             int32_t offset, float scale,
                                                             size_t numElements);
...
template datautil::StatusCode datautil::castToFloat<uint8_t>(float* out, uint8_t* in,
                                                             size_t numElements);

template datautil::StatusCode datautil::castToFloat<uint16_t>(float* out, uint16_t* in,
                                                              size_t numElements);

template datautil::StatusCode datautil::castToFloat<uint32_t>(float* out, uint32_t* in,
                                                              size_t numElements);

template datautil::StatusCode datautil::castToFloat<uint64_t>(float* out, uint64_t* in,
                                                              size_t numElements);

template datautil::StatusCode datautil::castToFloat<int8_t>(float* out, int8_t* in,
                                                            size_t numElements);

template datautil::StatusCode datautil::castToFloat<int16_t>(float* out, int16_t* in,
                                                             size_t numElements);

template datautil::StatusCode datautil::castToFloat<int32_t>(float* out, int32_t* in,
                                                             size_t numElements);

template datautil::StatusCode datautil::castToFloat<int64_t>(float* out, int64_t* in,
                                                             size_t numElements);
...
template datautil::StatusCode datautil::castFromFloat<uint8_t>(uint8_t* out, float* in,
                                                               size_t numElements);

template datautil::StatusCode datautil::castFromFloat<uint16_t>(uint16_t* out, float* in,
                                                                size_t numElements);

template datautil::StatusCode datautil::castFromFloat<uint32_t>(uint32_t* out, float* in,
                                                                size_t numElements);

template datautil::StatusCode datautil::castFromFloat<uint64_t>(uint64_t* out, float* in,
                                                                size_t numElements);

template datautil::StatusCode datautil::castFromFloat<int8_t>(int8_t* out, float* in,
                                                              size_t numElements);

template datautil::StatusCode datautil::castFromFloat<int16_t>(int16_t* out, float* in,
                                                               size_t numElements);

template datautil::StatusCode datautil::castFromFloat<int32_t>(int32_t* out, float* in,
                                                               size_t numElements);

template datautil::StatusCode datautil::castFromFloat<int64_t>(int64_t* out, float* in,
                                                               size_t numElements);