...
iotensor::PopulateInputTensorsRetType_t iotensor::IOTensor::readDataAndAllocateBuffer(
    const std::vector<std::string>& filePaths, const size_t filePathsIndexOffset,
    const bool loopBackToStart, std::vector<size_t> dims, Qnn_DataType_t dataType,
  ...
  *bufferToCopy = nullptr;
  ...
  std::tie(status, numFilesPopulated, batchSize) =
      datautil::readBatchData(filePaths, filePathsIndexOffset, loopBackToStart, dims, dataType,
...
iotensor::PopulateInputTensorsRetType_t iotensor::IOTensor::populateInputTensor(
    const std::vector<std::string>& filePaths, const size_t filePathsIndexOffset,
    const bool loopBackToStart, Qnn_Tensor_t* input, iotensor::InputDataType inputDataType) {
  if (nullptr == input) {
  ...
  if (inputDataType == InputDataType::FLOAT &&
    ...
    std::tie(returnStatus, numFilesPopulated, batchSize) =
        readDataAndAllocateBuffer(filePaths, filePathsIndexOffset, loopBackToStart, dims,
                                  QNN_DATATYPE_FLOAT_32, &fileToBuffer);
    if (StatusCode::SUCCESS == returnStatus) {
...
iotensor::PopulateInputTensorsRetType_t iotensor::IOTensor::populateInputTensors(
    uint32_t graphIdx, const std::vector<std::vector<std::string>>& filePathsVector,
    const size_t filePathsIndexOffset, const bool loopBackToStart,
    const std::unordered_map<std::string, uint32_t>& inputNameToIndex, Qnn_Tensor_t* inputs,
    qnn_wrapper_api::GraphInfo_t graphInfo, iotensor::InputDataType inputDataType) {
  ...
  if (filePathsVector.size() != inputCount) {
    QNN_ERROR(
        ...
        "received: %d",
        graphIdx, inputCount, filePathsVector.size());
    return {StatusCode::FAILURE, 0, 0};
  }
  size_t numFilesPopulated = 0;
  ...
  for (size_t inputIdx = 0; inputIdx < inputCount; inputIdx++) {
    ...
    std::tie(returnStatus, currentInputNumFilesPopulated, currentInputNumBatchSize) =
        populateInputTensor(filePathsVector[inputNameIdx], filePathsIndexOffset, loopBackToStart,
                            &(inputs[inputIdx]), inputDataType);
    if (StatusCode::SUCCESS != returnStatus) {
      QNN_ERROR("populateInputTensorFromFiles failed for input %s with index %d",
                inputNodeName.c_str(), inputIdx);
    ...
    if (inputIdx == 0) {
      ...
      if (numFilesPopulated != currentInputNumFilesPopulated ||
        ...
        QNN_ERROR(
            ...
            inputNodeName.c_str(), inputIdx, currentInputNumFilesPopulated,
            currentInputNumBatchSize, numFilesPopulated, numBatchSize);
...
iotensor::StatusCode iotensor::IOTensor::setupTensors(Qnn_Tensor_t** tensors, uint32_t tensorCount,
...
iotensor::StatusCode iotensor::IOTensor::allocateBuffer(uint8_t** buffer, std::vector<size_t> dims,
...
iotensor::StatusCode iotensor::IOTensor::allocateBuffer(T** buffer, size_t& elementCount) {
  QNN_DEBUG("ElementCount: %d, sizeof(T): %d, total size: %d", elementCount, sizeof(T),
    ...
    case QNN_DATATYPE_UFIXED_POINT_8:
      if (datautil::StatusCode::SUCCESS !=
          datautil::tfNToFloat<uint8_t>(
              *out, reinterpret_cast<uint8_t*>(QNN_TENSOR_GET_CLIENT_BUF(tensor).data),
              QNN_TENSOR_GET_QUANT_PARAMS(tensor).scaleOffsetEncoding.offset,
              QNN_TENSOR_GET_QUANT_PARAMS(tensor).scaleOffsetEncoding.scale, elementCount)) {
    ...
    case QNN_DATATYPE_UFIXED_POINT_16:
      if (datautil::StatusCode::SUCCESS !=
          datautil::tfNToFloat<uint16_t>(
              *out, reinterpret_cast<uint16_t*>(QNN_TENSOR_GET_CLIENT_BUF(tensor).data),
              QNN_TENSOR_GET_QUANT_PARAMS(tensor).scaleOffsetEncoding.offset,
              QNN_TENSOR_GET_QUANT_PARAMS(tensor).scaleOffsetEncoding.scale, elementCount)) {
        QNN_ERROR("failure in tfNToFloat<uint8_t>");
        returnStatus = StatusCode::FAILURE;
    ...
    case QNN_DATATYPE_UINT_32:
      if (datautil::StatusCode::SUCCESS !=
          datautil::castToFloat<uint32_t>(
              *out, reinterpret_cast<uint32_t*>(QNN_TENSOR_GET_CLIENT_BUF(tensor).data),
              elementCount)) {
    ...
    case QNN_DATATYPE_UINT_64:
      if (datautil::StatusCode::SUCCESS !=
          datautil::castToFloat<uint64_t>(
              *out, reinterpret_cast<uint64_t*>(QNN_TENSOR_GET_CLIENT_BUF(tensor).data),
              elementCount)) {
    ...
    case QNN_DATATYPE_INT_8:
      if (datautil::StatusCode::SUCCESS !=
          datautil::castToFloat<int8_t>(
              *out, reinterpret_cast<int8_t*>(QNN_TENSOR_GET_CLIENT_BUF(tensor).data),
              elementCount)) {
    ...
    case QNN_DATATYPE_INT_16:
      if (datautil::StatusCode::SUCCESS !=
          datautil::castToFloat<int16_t>(
              *out, reinterpret_cast<int16_t*>(QNN_TENSOR_GET_CLIENT_BUF(tensor).data),
              elementCount)) {
    ...
    case QNN_DATATYPE_INT_32:
      if (datautil::StatusCode::SUCCESS !=
          datautil::castToFloat<int32_t>(
              *out, reinterpret_cast<int32_t*>(QNN_TENSOR_GET_CLIENT_BUF(tensor).data),
              elementCount)) {
    ...
    case QNN_DATATYPE_INT_64:
      if (datautil::StatusCode::SUCCESS !=
          datautil::castToFloat<int64_t>(
              *out, reinterpret_cast<int64_t*>(QNN_TENSOR_GET_CLIENT_BUF(tensor).data),
              elementCount)) {
...
iotensor::StatusCode iotensor::IOTensor::convertAndWriteOutputTensorInFloat(
  ...
  if (datautil::StatusCode::SUCCESS !=
      datautil::writeBatchDataToFile(outputPaths, fileName, dims, QNN_DATATYPE_FLOAT_32,
...
iotensor::StatusCode iotensor::IOTensor::writeOutputTensor(Qnn_Tensor_t* output,
  ...
  if (datautil::StatusCode::SUCCESS !=
      datautil::writeBatchDataToFile(outputPaths, fileName, dims, QNN_TENSOR_GET_DATA_TYPE(output),
                                     bufferToWrite, outputBatchSize)) {
    QNN_ERROR("failure in writeBatchDataToFile");
    returnStatus = StatusCode::FAILURE;
  }
...
iotensor::StatusCode iotensor::IOTensor::writeOutputTensors(
    uint32_t graphIdx, size_t startIdx, char* graphName, Qnn_Tensor_t* outputs, uint32_t numOutputs,
    iotensor::OutputDataType outputDatatype, uint32_t graphsCount, std::string outputPath,
    size_t numInputFilesPopulated, size_t outputBatchSize) {
  if (nullptr == outputs) {
  ...
  for (size_t outputIdx = 0; outputIdx < numOutputs; outputIdx++) {
    ...
    if (QNN_TENSOR_GET_DATA_TYPE(outputs[outputIdx]) == QNN_DATATYPE_FLOAT_32) {
      ...
      returnStatus = convertAndWriteOutputTensorInFloat(&(outputs[outputIdx]), outputPaths,
...
iotensor::StatusCode iotensor::IOTensor::allocateAndCopyBuffer(uint8_t** buffer,
  ...
  pal::StringOp::memscpy(*buffer, length * sizeof(uint8_t), QNN_TENSOR_GET_CLIENT_BUF(tensor).data,
                         length * sizeof(uint8_t));
  return StatusCode::SUCCESS;
}

iotensor::StatusCode iotensor::IOTensor::fillDims(std::vector<size_t>& dims, uint32_t* inDimensions,
                                                  uint32_t rank) {
