...
StatusCode readDataFromFile(std::string filePath, std::vector<size_t> dims, Qnn_DataType_t dataType,
                            uint8_t* buffer);
...
#ifndef __hexagon__
StatusCode writeDataToFile(std::string fileDir, std::string fileName, std::vector<size_t> dims,
                           Qnn_DataType_t dataType, uint8_t* buffer);

StatusCode writeBatchDataToFile(std::vector<std::string> fileDirs, std::string fileName,
                                std::vector<size_t> dims, Qnn_DataType_t dataType, uint8_t* buffer,
                                const size_t batchSize);

StatusCode writeBinaryToFile(std::string fileDir, std::string fileName, uint8_t* buffer,
                             size_t bufferSize);
#endif

template <typename T_QuantType>
datautil::StatusCode floatToTfN(T_QuantType* out, float* in, int32_t offset, float scale,
                                size_t numElements);

template <typename T_QuantType>
datautil::StatusCode tfNToFloat(float* out, T_QuantType* in, int32_t offset, float scale,
                                size_t numElements);
