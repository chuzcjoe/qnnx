...
sample_app::dlc_utils::StatusCode sample_app::dlc_utils::createDlcHandle(
    const QnnSystemInterface_t& systemInterface, const std::string& dlcPath,
    QnnLog_Callback_t logCallback, QnnLog_Level_t logLevel, Qnn_LogHandle_t& logHandle,
    QnnSystemDlc_Handle_t& dlcHandle) {
  QNN_INFO("Creating DLC handle from file: %s", dlcPath.c_str());
  QNN_INFO("Initializing logging in the System Library for DLC. Callback: [%p], Log Level: [%d]",
           logCallback, logLevel);

  // Initialize logging for DLC
  if (QNN_SUCCESS != systemInterface.QNN_SYSTEM_INTERFACE_VER_NAME.systemLogCreate(
                         logCallback, logLevel, &logHandle)) {
    QNN_ERROR("Unable to initialize logging in the system library for DLC.");
    return StatusCode::FAILURE;
  }

  // Create DLC handle from file
  if (QNN_SUCCESS != systemInterface.QNN_SYSTEM_INTERFACE_VER_NAME.systemDlcCreateFromFile(
                         logHandle, dlcPath.c_str(), &dlcHandle)) {
  ...
  QNN_INFO("Successfully created DLC handle");
...
sample_app::dlc_utils::StatusCode sample_app::dlc_utils::composeGraphsFromDlc(
    const QnnSystemInterface_t& systemInterface, QnnSystemDlc_Handle_t dlcHandle,
    Qnn_BackendHandle_t backendHandle, Qnn_ContextHandle_t context,
    const QnnInterface_t& qnnInterface, qnn_wrapper_api::GraphInfo_t**& graphsInfo,
  ...
  QNN_INFO("Calling systemDlcComposeGraphs");

  auto qnnStatus = systemInterface.QNN_SYSTEM_INTERFACE_VER_NAME.systemDlcComposeGraphs(
      dlcHandle, (const QnnSystemDlc_GraphConfigInfo_t**)graphConfigsInfo, numGraphConfigs,
      backendHandle, context, qnnInterface, QNN_SYSTEM_CONTEXT_GRAPH_INFO_VERSION_1,
      &systemGraphInfos, &numGraphs);
  ...
  QNN_INFO("Successfully composed %d graphs from DLC", numGraphs);
...
sample_app::dlc_utils::StatusCode sample_app::dlc_utils::retrieveGraphHandles(
    const QnnInterface_t& qnnInterface, Qnn_ContextHandle_t context,
  ...
  for (uint32_t graphIdx = 0; graphIdx < graphsCount; graphIdx++) {
    if (QNN_SUCCESS !=
        qnnInterface.QNN_INTERFACE_VER_NAME.graphRetrieve(
            context, (*graphsInfo)[graphIdx].graphName, &((*graphsInfo)[graphIdx].graph))) {
      QNN_ERROR("Unable to retrieve graph handle for graph: %s", (*graphsInfo)[graphIdx].graphName);
      return StatusCode::FAILURE;
    }
